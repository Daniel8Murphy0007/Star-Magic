...
Unique physics: Merger interaction term I(t)=I_0*exp(-t/tau_merger)
                applied as (1+I(t)) multiplier on BOTH term1 AND Ug
                (interaction boosts gravity during merger approach).

Run:  python gen_muge_antennae.py
Output: AntennaeGalaxies.h
"""
import os

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
OUTPUT_FILE = os.path.join(SCRIPT_DIR, "AntennaeGalaxies.h")
...
def main():
    print("gen_muge_antennae.py — Generating AntennaeGalaxies.h ...")
    content = get_content()
