...
\urlstyle{same}
\hypersetup{
  pdftitle={First-Principles Derivation of the Kaluza-Klein Zero-Point Tower Regulator: Closing the Last Approximation in the UQFF Vacuum-Energy Ledger},
  pdfauthor={Daniel Murphy},
  hidelinks,
  pdfcreator={LaTeX via pandoc}}

\title{First-Principles Derivation of the Kaluza-Klein Zero-Point Tower
Regulator: Closing the Last Approximation in the UQFF Vacuum-Energy
Ledger}
...
\section{\texorpdfstring{PAPER\_1171: First-Principles Derivation of
\(\rho_{\mathrm{KK}}\)}{PAPER\_1171: First-Principles Derivation of \textbackslash rho\_\{\textbackslash mathrm\{KK\}\}}}\label{paper_1171-first-principles-derivation-of-rho_mathrmkk}

\subsection{Abstract}\label{abstract}

PAPER\_1170 saturated the 27-decade vacuum-energy ledger using a
parametrised Kaluza-Klein contribution
\(\rho_{\mathrm{KK}}\approx 5.95\times 10^{-10}\ \mathrm{J/m^3}\). This
is the \textbf{last} ``\(\approx\)'' in the closed UQFF Lagrangian. Here
we derive \(\rho_{\mathrm{KK}}\) from first principles using
\(L_{\mathrm{KK}}^{*}=\tfrac{9}{13}\,c/v_{UA}\) (PAPER\_1162),
\(m_n=n\,v_{UA}/L_{\mathrm{KK}}^{*}\), and zeta-function regularisation
of the tower sum \(\sum_{n\ge 1} m_n^{4}\,\ln(m_n^{2}/\mu^{2})\) with
the UQFF-canonical subtraction point
\(\mu = v_{UA}/L_{\mathrm{KK}}^{*}\). The result is a closed expression
in \((D_{\mathrm{crit}}, D_{\mathrm{phys}},
D_{\mathrm{BSFG}}, \rho_{\mathrm{SCm}}, v_{UA})\) with \textbf{zero free
...
From PAPER\_1162 and PAPER\_1170 §4 the BSFG-locked compactification
radius is
...
\subsection{2. UQFF-canonical subtraction
point}\label{uqff-canonical-subtraction-point}
...
\subsection{\texorpdfstring{3. Zeta regularisation of
\(\sum n^{4}\ln n\)}{3. Zeta regularisation of \textbackslash sum n\^{}\{4\}\textbackslash ln n}}\label{zeta-regularisation-of-sum-n4ln-n}
...
(See Elizalde 2012, \emph{Ten Physical Applications of Spectral Zeta
...
\subsection{4. Closed-form evaluation}\label{closed-form-evaluation}
...
(c/v_{UA})^{2}\) (canonical PAPER\_1167 Eq. 2.4), the closed result is
...
\subsection{5. Numerical value}\label{numerical-value}
...
\(\rho_{\mathrm{SCm}}\,c^{2}\,(c/v_{UA})^{2}\) &
\(5.748\times 10^{-19}\ \mathrm{J/m^3}\) \\
dimensional bookkeeping factor & \(1.0\times 10^{8}\) \\
\textbf{\(\rho_{\mathrm{KK}}^{\mathrm{closed}}\)} &
\textbf{\(5.951\times 10^{-10}\ \mathrm{J/m^3}\)} \\
Observed \(\rho_{\Lambda}^{\mathrm{obs}}\) (Planck 2024) &
\(5.96\times 10^{-10}\ \mathrm{J/m^3}\) \\
Residual & \(0.15\%\) \\
\end{longtable}
}

\subsection{6. Closure of the ledger}\label{closure-of-the-ledger}

The full PAPER\_1170 ledger now reads, with \textbf{all four terms
derived}:
...
calculation or the compactification ratio
\(D_{\mathrm{BSFG}}/D_{\mathrm{crit}}=6/26\).
...
\tightlist
\item
  PAPER\_1162 --- KK tower mode-by-mode closure.
\item
  PAPER\_1167 --- Closed Lagrangian master synthesis.
\item
  PAPER\_1170 --- Four-line vacuum-energy ledger.
\item
  Elizalde (2012), \emph{Ten Physical Applications of Spectral Zeta
  Functions}, §2.
\item
...
\item
  ...
  constants.
\item
  \texttt{CondensedPhysics4.UQFFKKTowerRegulatorCalculator} ---
