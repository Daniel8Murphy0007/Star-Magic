...
def dpm_ug2_shell(M: float, r: float, R: float, v_sw: float = 4e5) -> float:
    """DPM shell Ug2: charge-reactivity bubble using vacuum densities. NO G."""
    if R <= 0 or r <= 0:
        return 0.0
...
class CosmologicalLineFluximeSFRIntegralCalculator(_CP3Calculator):
    ...
    def compute(self, dataset: dict) -> dict:
        ...
        if d_L == 0.0:
            d_L = 1e26  # fallback to ~1 Gpc to avoid division by zero
...
class CoAnQiModularResonanceMUGECalculator(_CP3Calculator):
    """PAPER_172: CoAnQi modular resonance MUGE via plugin."""
    ...
    def compute(self, dataset: dict = None) -> dict:
        ...
        G = 6.674e-11
        g_res = G * M / (r * r)
        return {'g_resonance': g_res, 'mode': 'resonance_MUGE'}
...
class DiPseudoMonopoleDPMTheoryCalculator(_CP3Calculator):
    """PAPER_175: Di-Pseudo Monopole (DPM) Theory — aDPM emergent foundation term.
    ...
    def compute(self, dataset: dict) -> dict:
        ...
        g_Newton = dpm_ug1_seed(M, r)  # DPM: mu_s * grad(M_s/r)
...
class FUBiiFullDPMPolynomialIntegralCalculator(_CP3Calculator):
    ...
    def compute(self, dataset: dict) -> dict:
        ...
        b_coef = dpm_ug1_seed(M, r) * DPM_gravity
    ...
    def compute(self, dataset: dict) -> dict:
        ...
        return {
            ...
            'available_equations': [
                ...
                "Ug1 = G·M(t)/r²  (time-varying emergent projection)",
    ...
    def compute(self, dataset: dict) -> dict:
        ...
        return {
            'primary_equations': [
                ...
                f"term1·(1+I(t)) = {term1:.4e} m/s²  [emergent projection with interaction; NOVEL double-application]",
...
class AntennaeGalaxiesMergerInteractionCalculator(_CP3Calculator):
    ...
    Uniquely Rare Mathematical Discoveries (NEW vs existing UQFFVelocityStarFormationCollisionCalculator):
      ...
      2. DOUBLY applied: BOTH term1 AND Ug modulated by (1+I(t))
         term1 = emergent_projection  · (1+I(t))   ← projection amplified by merger
    ...
    def compute(self, dataset: dict) -> dict:
        ...
        return {
            'primary_equations': [
                ...
                f"term1 = emergent_proj·(1+I(t)) = {term1:.4e} m/s²  [NOVEL: merger amplifies projection]",
...
class MUGEFluidSelfGravityTermCalculator(_CP3Calculator):
    ...
    def compute(self, dataset: dict) -> dict:
        ...
        V       = (4.0 / 3.0) * math.pi * r**3
        g_grav = dpm_ug1_seed(M, r)  # DPM: mu_s * grad(M_s/r)
...
class SN1006TypeIaSNRFUBiCalculator(_CP3Calculator):
    ...
    def compute(self, dataset: dict) -> dict:
        ...
        term_gravity  = dpm_ug1_seed(M, r) * DPM_gravity
...
class EtaCarinaeHomuculusFUBiCalculator(_CP3Calculator):
    ...
    def compute(self, dataset: dict) -> dict:
        ...
        term_gravity  = dpm_ug1_seed(M, r) * DPM_gravity      # larger than SN1006
        term_momentum = (m_e * c_light**2 / r**2) * DPM_momentum * math.cos(theta)
        term_vac      = rho_vac_UA * DPM_stability
...
class SgrACenterNegativeBuoyancyCalculator(_CP3Calculator):
    ...
    *** UNIQUELY RARE MATHEMATICAL DISCOVERY — Negative Buoyancy Inversion ***
    ...
    def compute(self, dataset: dict) -> dict:
        ...
        term_gravity  = dpm_ug1_seed(M, r) * DPM_gravity      # larger M and r
        term_momentum = (m_e * c_light**2 / r**2) * DPM_momentum * math.cos(theta)
        term_vac      = rho_vac_UA * DPM_stability
...
class Source10GravitationalVacuumDragCalculator(_CP3Calculator):
    ...
    def calculate(self, dataset: dict) -> dict:
        ...
        F_newton = dpm_ug1_seed(M, r) * M  # DPM self-gravity  # self-gravity approximation
...
if __name__ == "__main__":
    ...
    print(f"Holmlid KER from SCm: {_ker_local / 1.60217662e-19:.0f} eV  <== exact match to 630 eV")
    print(f"Parkhomov predicted excess heat (1 hour): {parkhomov_excess_heat_cp3():.1f} kW   (100-300 W range)")
