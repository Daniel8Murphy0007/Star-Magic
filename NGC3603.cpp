 ...
 *          pipeline (CP3/PAPER_198): term_Ubi=0.5×ug1_t (Tier-1 static half-gravity);
 *          term_F_UBii=−β_i×ug1_t×ω_g×(Mt/r)×U_UA×cos(π·t) (Tier-2 dynamic compact M(t));
 *          term_Ub_i=−β_i×ug1_t×ω_g×(M_GC/r_GC)×U_UA×cos(π·t) (Tier-3 outer-frame via
 *          Sgr A* ext body ~7 kpc from NGC 3603); β_i=0.61, ω_g=7.3e-16, U_UA=1e-11;
 *          10→13-term MUGE; FU_diag diagnostic; UQFF 2.0 (logging_enabled, dynamic_params,
 *          setEnableLogging, exportState("NGC3603_state.txt"), cross_validate<>).
...
private:
    ...
    // External gravitational body: Sgr A* (~7 kpc from NGC 3603, Carina arm)
    double M_GC;            // Sgr A* mass ~4e6 M_sun (kg)
    double r_GC;            // Distance NGC 3603 to GC ~7 kpc in meters
...
public:
    ...
    void initializeDefaults() {
        ...
        r_GC    = 2.16e20;       // NGC 3603 to GC ~7 kpc in meters
    ...
    double compute_g_NGC3603(double t) const {
        ...
        // ── UQFF buoyancy pipeline (CP3/PAPER_198 canonical) ─────────────────────────
        // Note: use ug1_t (time-evolving M(t)) throughout all 3 buoyancy tiers
        // Tier-1: term_Ubi = 0.5 * ug1_t (CP3/PAPER_198 static half-gravity buoyancy)
        double term_Ubi = 0.5 * ug1_t;
        // Tier-2: term_F_UBii — PAPER_198 dynamic compact, cosine modulation over M(t)
        double term_F_UBii = -beta_i * ug1_t * omega_g * (Mt / r) * U_UA * cos(M_PI * t);
        // Tier-3: term_Ub_i — CP1 outer-frame via Sgr A* (~7 kpc from NGC 3603, Carina arm)
        double term_Ub_i = -beta_i * ug1_t * omega_g * (M_GC / r_GC) * U_UA * cos(M_PI * t);
        // FU diagnostic (not summed into result)
        double FU_diag = -(term2 + term_Ubi) * ug1_t;

        // Total g_NGC3603 (13 terms: 10 original + 3 UQFF buoyancy tiers)
    ...
    void printParameters(std::ostream& os = std::cout) const {
        ...
        os << "M_GC: " << M_GC << " (Sgr A* ext body), r_GC: " << r_GC << " (~7 kpc)" << std::endl;
    }
    ...
    void exportState(const std::string& filename = "NGC3603_state.txt") const {
        std::ofstream ofs(filename);
        if (!ofs) {
            std::cerr << "Error: Cannot open '" << filename << "' for state export." << std::endl;
            return;
        }
        ofs << "# NGC3603 (UQFF 2.0) state export\n";
        ofs << std::scientific << std::setprecision(10);
        const char* names[] = {
            "G","M0","r","H0","B","B_crit","Lambda","c_light","q_charge","gas_v",
            "f_TRZ","M_dot_factor","tau_SF","rho_wind","v_wind","rho_fluid",
            "P0","tau_exp","rho_vac_UA","rho_vac_SCm","scale_EM","proton_mass",
    ...
    double cross_validate(const OtherModuleT& /*other*/, double t_seconds = 0.0) const {
        double g_here  = compute_g_NGC3603(t_seconds);
        double g_other = 0.0;
