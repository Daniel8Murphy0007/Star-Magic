# PAPER_1243 — Collatz Conjecture via F_TRZ Phase Lock and K_Mex Halving
...
**Calculator surface:** `calculate_paradox({"paradox": "collatz"})`
**Closure helper:** `_l96_uqff_axiom_collatz_conjecture_closure()`

## Abstract

This paper presents the UQFF derivation of the Collatz Conjecture problem using only the locked canonical primitives of the UQFF framework. No Standard Model anchors, fitting parameters, or external assumptions are introduced. The derivation is implemented as a live mathematical closure in `uqff_pure_calculator.py` and dispatches through the `calculate_paradox` public surface.
...
F_TRZ Phase Lock and K_Mex Halving

The closure helper `_l96_uqff_axiom_collatz_conjecture_closure()` evaluates this derivation. Output dict carries:
...
result = u.calculate_paradox({"paradox": "collatz"})['value']
```
...
- Closure location: `uqff_pure_calculator.py` → `_l96_uqff_axiom_collatz_conjecture_closure`
- Dispatch: `PARADOX_TO_CLOSURE["collatz"]`
