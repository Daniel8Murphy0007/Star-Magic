# PAPER_1248 — Smooth Poincare 4D via K_Mex × D_phys = 25/3 Exotic R4
...
**Calculator surface:** `calculate_paradox({"paradox": "smooth_poincare_4d"})`
**Closure helper:** `_l96_uqff_axiom_smooth_poincare_4d_closure()`

## Abstract

This paper presents the UQFF derivation of the Smooth Poincare 4D problem using only the locked canonical primitives of the UQFF framework. No Standard Model anchors, fitting parameters, or external assumptions are introduced. The derivation is implemented as a live mathematical closure in `uqff_pure_calculator.py` and dispatches through the `calculate_paradox` public surface.
...
K_Mex × D_phys = 25/3 Exotic R4

The closure helper `_l96_uqff_axiom_smooth_poincare_4d_closure()` evaluates this derivation. Output dict carries:
...
result = u.calculate_paradox({"paradox": "smooth_poincare_4d"})['value']
```
...
- Closure location: `uqff_pure_calculator.py` → `_l96_uqff_axiom_smooth_poincare_4d_closure`
- Dispatch: `PARADOX_TO_CLOSURE["smooth_poincare_4d"]`
