...
**Author:** Daniel T. Murphy  
**Framework:** UQFF Star-Magic ($\kappa$ = 0.0005/day, [SCm] $\approx$ 0.99, [SSq] = 0.57)  
**Date:** March 7, 2026  
**Source Data:** MAIN_{1\_CoAnQi}.cpp Batch 22 (Astrophysical Transients Module)  
...
| Peak L_bol | 2.4 $\times$ 104 erg/s | Nicholl+2020 |
...
$$L_{\mathrm{peak}}^{\mathrm{UQFF}} = \epsilon_{\mathrm{eff}} \dot{M}_{\mathrm{fb}} c^2$$

Where:
$$\epsilon_{\mathrm{eff}} = \epsilon_{\mathrm{GR}} \cdot [{\mathrm{SCm}}] \cdot \left(1 + \frac{U_{g2}}{c^2 \dot{M}}\right)$$
...
$$\dot{M}_{\mathrm{fb}}(t) = \frac{M_\star}{3 t_{\mathrm{fb}}} \left(\frac{t}{t_{\mathrm{fb}}}\right)^{-5/3}$$

Characteristic fallback time:
$$t_{\mathrm{fb}} = 2\pi \left(\frac{R_t^3}{G M_{\mathrm{BH}}}\right)^{1/2}$$
...
$$U_{g3}(r, t) = \frac{B_{\mathrm{dip}}^2 R_{\mathrm{eff}}^3}{4} \cdot \frac{\omega_{\mathrm{orb}}(t)}{\omega_{\mathrm{crit}}}$$
...
$$L_{\mathrm{peak}}^{\mathrm{UQFF}} = L_{\mathrm{Edd}} \cdot \eta_{\mathrm{UQFF}}$$

With $\eta_{\mathrm{UQFF}} = \eta_{\mathrm{GR}} \times [{\mathrm{SCm}}] = 0.1 \times 0.99 = 0.099$ (slightly sub-Eddington efficiency).

For M_BH = 10645 M? = 2.82 $\times$ 106 M?:
$$L_{\mathrm{Edd}} = 3.6 \times 10^{44} \text{ erg/s}$$
$$L_{\mathrm{peak}}^{\mathrm{UQFF}} = \eta_{\mathrm{UQFF}} \dot{M}_{\mathrm{max}} c^2 = 2.20 \times 10^{43} \text{ erg/s}$$

Observed: 2.4 $\times$ 104 erg/s ? **UQFF deviation: -8.3%** (within uncertainty range).

### Phase 3: Temporal ?-Decay

$$L_{\mathrm{opt}}(t) = L_{\mathrm{peak}} e^{-\kappa_{\mathrm{opt}} t}$$

Where ?_opt  $\kappa$ = 0.0005/day ? half-life = ln2/? = 1,386 days; but observed half-life is 60 days.
...
|------------|------|------------|-------|
| Peak luminosity | 2.20 $\times$ 104 erg/s | 2.4 $\times$ 104 erg/s | 91.7% |
...
*Source: `MAIN_{1\_CoAnQi}`.cpp Batch 22 (AT2019qizUQFFTerm) | Nicholl+2020 reference | [SCm]=0.99 |
[SSq]=0.57*
...
$$\text{PAPER\_877 Axioms} \xrightarrow{\text{DPM + ACP}} \rho_{\mathrm{vac}} = \rho_{\mathrm{UA}} + \rho_{\mathrm{SCm}} \xrightarrow{\text{Stage 5}} U_{b,\mathrm{seed}} \xrightarrow{\text{4 forces}} F_U_Bi_i \xrightarrow{\text{sector E-L}} \delta S/\delta \phi_{\mathrm{outflow}} = 0$$
...
8. Hawking, S.W. (1975). *Particle Creation by Black Holes.* Commun. Math. Phys. **43**, 199 — doi:10.1007/BF02345020
9. Bekenstein, J.D. (1973). *Black Holes and Entropy.* Phys. Rev. D **7**, 2333 — doi:10.1103/PhysRevD.7.2333
10. Unruh, W.G. (1976). *Notes on Black-Hole Evaporation.* Phys. Rev. D **14**, 870 — doi:10.1103/PhysRevD.14.870
