...
private:
    ...
    double omega_g;         // Galactic rotation rate (rad/s)
    double U_UA;            // Unit charge aether parameter (C)
    // External gravitational body: Galactic Center (Sgr A*, ~6.85 kpc from Pillars/M16)
    double M_GC;            // Sgr A* mass ~4e6 M_sun (kg)
    double r_GC;            // Distance Pillars to GC ~6.85 kpc in meters
...
public:
    ...
    void initializeDefaults() {
        ...
        r_GC = 2.11e20;              // Pillars to GC ~6.85 kpc in meters
    ...
    double compute_g_Pillars(double t) const {
        ...
        // Note: buoyancy acts on the same pillar mass sculpted by erosion E(t)
        double term_Ubi = 0.5 * ug1_t;
        // CP3/PAPER_198 Tier-2: dynamic compact F_UBii — cosine modulation over pillar oscillation
        double term_F_UBii = -beta_i * ug1_t * omega_g * (Mt / r) * U_UA * cos(M_PI * t);
        // CP1 Tier-3: outer-frame Ub_i via Galactic Center (Sgr A*, ~6.85 kpc from M16)
    ...
    void printParameters(std::ostream& os = std::cout) const {
        ...
        os << "M_GC: " << M_GC << " (Sgr A* ext body), r_GC: " << r_GC << " (~6.85 kpc)" << std::endl;
    }
    ...
    void exportState(const std::string& filename = "PILLARS_OF_CREATION_state.txt") const {
        std::ofstream ofs(filename);
        if (!ofs) {
            std::cerr << "Error: Cannot open '" << filename << "' for state export." << std::endl;
            return;
        }
        ofs << "# PillarsOfCreation state export\n";
        ofs << std::scientific << std::setprecision(10);
        const char* names[] = {
            "G","M_initial","r","H0","B","B_crit","Lambda","c_light",
            "q_charge","gas_v","f_TRZ","M_dot_factor","tau_SF","E_0","tau_erosion",
    ...
    // Cross-validate against Westerlund2 at t=500k yr — fractional |gPillars − gWd2| / |gPillars|
    template<typename Westerlund2T>
    double cross_validate(const Westerlund2T& wd2, double t_years = 5.0e5) const {
        double t_s     = t_years * 3.156e7;
        double g_pil   = compute_g_Pillars(t_s);
        double g_wd2   = wd2.compute_g_Westerlund2(t_s);
        double frac    = (g_pil != 0.0)
                         ? std::abs(g_wd2 - g_pil) / std::abs(g_pil)
                         : std::numeric_limits<double>::quiet_NaN();
        if (logging_enabled)
            std::cout << "[XVAL] Pillars g(" << t_years << "yr)=" << g_pil
                      << "  Wd2 g=" << g_wd2
