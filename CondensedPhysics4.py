...
class UQFFBlackHoleInversionCalculator:
    ...
    ENTRY = 247
    PAPER = "PAPER_663"
    CPP_MODULE = "UQFFBlackHoleInversion"
    UQFF_CONSTANTS = {
        "rho_UA": 7.09e-36, "rho_SCm": 7.09e-37, "f_TRZ": 0.1,
        "kappa": 0.0005, "SSq": 0.57, "mu_j": 3.38e23, "gamma": 5e-5,
    }
    PARAMETERS = {
        "M_kg": {"type": "float", "default": 8.55e+36, "desc": "Black hole mass in kg"},
        "n_MC": {"type": "int", "default": 5000, "desc": "Monte Carlo samples"},
    }
    PRIMARY_OUTPUT = "Theta_inv"
    EQUATIONS = [
        "UQFF BH Inversion Probability — Theta_inv = P_inv*Phi_inv*exp(U_m/k_B T_H). P_inv = f_TRZ*exp(-E_inv/k_B T_H), Phi_inv = (rho_UA/rho_SCm)*(GM/c)*(1+f_TRZ). Monte Carlo log-normal distribution. Sgr A*:",
    ]
    ...
    def compute(self, params: dict = None) -> dict:
        ...
        return {
            ...
            "Um_J":            Um,
            "stability_factor": tau_uqff/max(tau_std,1e-300),
            "Theta_inv":   tau_uqff if "tau" in "Theta_inv" else (
                               L_uqff if "L_" in "Theta_inv" else
                               T_uqff if "T_" in "Theta_inv" else 0.0),
            "equations": [
                "tau_UQFF = tau_std/(1-f_TRZ)*(rho_UA/rho_SCm)*exp(U_m/k_B T_H)",
                "L_UQFF = L_H*(1-rho_SCm/rho_UA)*exp(-U_m/k_B T_H)",
                "T_UQFF = T_H*(1+f_TRZ)*(1-rho_SCm/rho_UA)",
...
class UQFFDMDtDerivationCalculator:
    ...
    PRIMARY_OUTPUT = "dM_dt_UQFF_kgs"
    EQUATIONS = [
        "UQFF dM/dt Full Derivation — Step 1: dM/dt*=(1-f_TRZ); Step 2: *=(rho_SCm/rho_UA); Step 3: *=exp(-U_m/k_B T_H). Suppression factor ~0.033 vs standard. Analytic M(t)=(M0^3-3A t)^1/3. Full Euler evapora",
    ]
    ...
    def compute(self, params: dict = None) -> dict:
        ...
        return {
            ...
            "Um_J":            Um,
            "stability_factor": tau_uqff/max(tau_std,1e-300),
            "dM_dt_UQFF_kgs":   tau_uqff if "tau" in "dM_dt_UQFF_kgs" else (
                               L_uqff if "L_" in "dM_dt_UQFF_kgs" else
                               T_uqff if "T_" in "dM_dt_UQFF_kgs" else 0.0),
            "equations": [
                "tau_UQFF = tau_std/(1-f_TRZ)*(rho_UA/rho_SCm)*exp(U_m/k_B T_H)",
                "L_UQFF = L_H*(1-rho_SCm/rho_UA)*exp(-U_m/k_B T_H)",
                "T_UQFF = T_H*(1+f_TRZ)*(1-rho_SCm/rho_UA)",
...
class UQFFAdvancementsAndTHzHolesCalculator:
    ...
    ENTRY = 257
    PAPER = "PAPER_673"
    CPP_MODULE = "UQFFAdvancementsAndTHzHoles"
    UQFF_CONSTANTS = {
        "rho_UA": 7.09e-36, "rho_SCm": 7.09e-37, "f_TRZ": 0.1,
        "kappa": 0.0005, "SSq": 0.57, "mu_j": 3.38e23, "gamma": 5e-5,
    }
    PARAMETERS = {
        "M_kg": {"type": "float", "default": 1.0e+36, "desc": "BH mass for THz analogy in kg"},
        "T_c_K": {"type": "float", "default": 100.0, "desc": "Superconductor critical temp in K"},
        "n_papers": {"type": "int", "default": 673, "desc": "Number of UQFF papers (FAS)"},
    }
    PRIMARY_OUTPUT = "FAS"
    EQUATIONS = [
        "UQFF THz Holes + Red Dwarf Reactor Meta-Module — f_THz=k_B T_c/(2pi hbar)~2 THz. L_THz_UQFF=L_H*(f_THz/f_H)^4*(rho_UA/rho_SCm). tau_RD_UQFF=tau_std*(rho_UA/rho_SCm)*(1+f_TRZ)~1.1e14 yr. FAS=N_papers*(",
    ]
    ...
    def compute(self, params: dict = None) -> dict:
        ...
        return {
            ...
            "Um_J":            Um,
            "stability_factor": tau_uqff/max(tau_std,1e-300),
            "FAS":   tau_uqff if "tau" in "FAS" else (
                               L_uqff if "L_" in "FAS" else
                               T_uqff if "T_" in "FAS" else 0.0),
            "equations": [
                "tau_UQFF = tau_std/(1-f_TRZ)*(rho_UA/rho_SCm)*exp(U_m/k_B T_H)",
                "L_UQFF = L_H*(1-rho_SCm/rho_UA)*exp(-U_m/k_B T_H)",
                "T_UQFF = T_H*(1+f_TRZ)*(1-rho_SCm/rho_UA)",
