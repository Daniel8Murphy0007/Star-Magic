...
double {cls}::F_env(double t) const {{
    double F_tidal   = dpm_ug1_seed(M_spiral, d_spiral);
...
double {cls}::U_g3_prime() const {{
    return dpm_ug1_seed(M_spiral, d_spiral);
}}
...
double {cls}::g_NGC1316(double r, double t) const {{
    ...
    double g_core = (dpm_ug1_seed(M, rt)) * (1.0 + H) * (1.0 - B_AGN / B_crit) * (1.0 + F);
...
double {cls}::g_cluster_UQFF(double r) const {{
    double g0 = dpm_ug1_seed(M_cluster, r);
...
double {cls}::g_M51_UQFF(double r) const {{
    ...
    double g0    = dpm_ug1_seed(M_tot, r);
...
double {cls}::SFE_UQFF() const {{
    double g_grav = dpm_ug1_seed(M_main, R_spiral);
    double F_t    = tidal_force(R_spiral);
