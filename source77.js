class UGC10214UQFFModule {
    constructor() {
        ...
        this.variables.set("SFR", 4.67 * M_sun_val / this.variables.get("year_to_s")); // kg/s
        this.variables.set("r", 55e3 * kpc_val);                // m
        this.variables.set("z", 0.032);                         // Redshift
        this.variables.set("M_dwarf", 3.5e9 * M_sun_val);       // kg
        this.variables.set("d_dwarf", 110e3 * kpc_val);         // m
        this.variables.set("v_tail", 400e3);                    // m/s
        this.variables.set("tau_merge", 2.5e8 * this.variables.get("year_to_s")); // s
        this.variables.set("t", 2.5e8 * this.variables.get("year_to_s")); // Default t=250 Myr s
        ...
        this.variables.set("omega", 1e-15);                     // rad/s for tail waves
        this.variables.set("x", 0.0);
        this.variables.set("v", this.variables.get("v_tail"));  // m/s
        this.variables.set("sigma", 10e3 * kpc_val);            // m for Gaussian
    ...
    // M_merge(t)
    computeMmerge(t) {
        return this.variables.get("M_dwarf") * Math.exp(-t / this.variables.get("tau_merge"));
    }
    ...
    // F_env(t)
    computeFenv(t) {
        const F_tidal = (this.variables.get("G") * this.variables.get("M_dwarf")) / (this.variables.get("d_dwarf") * this.variables.get("d_dwarf"));
        const F_SF = this.variables.get("k_SF") * this.variables.get("SFR") / 1.989e30;  // Normalize to m/s^2
        const F_tail = this.variables.get("rho_fluid") * Math.pow(this.variables.get("v_tail"), 2);
    ...
    // Ug3': external
    computeUg3prime(t) {
        return (this.variables.get("G") * this.variables.get("M_dwarf")) / (this.variables.get("d_dwarf") * this.variables.get("d_dwarf"));
    }
    ...
    printVariables() {
        console.log('UGC 10214 Variables:');
    ...
    // Install UGC10214 UQFF module (for compatibility)
    install_uqff_module() {
        console.log("UGC10214UQFFModule installed for Tadpole Galaxy (UGC 10214) Evolution");
        console.log("Supports gravitational dynamics, minor merger with VV 29c, tidal tail ejection, star formation in disk/tail, gas densities, and dark matter");
