...
\begin{enumerate}
    ...
    \item Newtonian gravity emergent as zeroth-order limit of $F_U$ (corrections $\lesssim 10^{-8}$)
\end{enumerate}
...
\begin{equation}
F_U(r, t, t_n, \theta) = \underbrace{-\frac{G M}{r}}_{\text{Newtonian}} + \underbrace{\delta U_{g1}(r) + \delta U_{g2}(r) + \delta U_{g3}(r, t) + \delta U_{g4}(r, t_n)}_{\text{UQFF corrections}} + U_{bi}(r) + U_m(r)
\label{eq:unified_field}
\end{equation}

where $-GM/r$ is the Newtonian gravitational potential and each $\delta U_{gi}$ represents a distinct UQFF correction mechanism:

\textbf{1. Magnetic Dipole Correction} ($\delta U_{g1}$):
\begin{equation}
\delta U_{g1}(r) = -\frac{G B_{\text{mag}}^2 r}{2 \mu_0 c^2}
\label{eq:dug1}
\end{equation}
Accounts for magnetic field contributions to gravitational potential. Equivalent to the correction factor $-\frac{GM}{r} \cdot \frac{B_{\text{mag}}^2 r^2}{2\mu_0 M c^2}$.

\textbf{2. Charge-Reactivity Correction} ($\delta U_{g2}$):
\begin{equation}
\delta U_{g2}(r) = -\frac{G M \alpha [\text{SSq}]}{r^3}
\label{eq:dug2}
\end{equation}
Represents electromagnetic-gravitational coupling with calibrated $[\text{SSq}] = 0.57$.

\textbf{3. String Rotation Correction} ($\delta U_{g3}$):
\begin{equation}
\delta U_{g3}(r, t) = -\frac{G M \beta \sin(\omega t)}{r}
\label{eq:dug3}
\end{equation}
Time-dependent oscillations with $\omega = 2\pi/(1\,\text{day})$.

\textbf{4. Vacuum Concentration Correction} ($\delta U_{g4}$):
\begin{equation}
\delta U_{g4}(r, t_n) = -\frac{G M \gamma \exp(-\kappa t_n)}{r}
\label{eq:dug4}
\end{equation}
...
\textbf{Theorem (Newtonian Emergence).} In the limit where all UQFF corrections vanish---zero magnetic field ($B_{\text{mag}} \to 0$), zero charge coupling ($\alpha \to 0$), zero string rotation ($\beta \to 0$), zero vacuum concentration ($\gamma \to 0$), zero vacuum energy density ($\lambda_{\text{UA}} \to 0$), and no magnetic dipole moments ($m_1 \cdot m_2 \to 0$)---the unified field $F_U$ reduces to the Newtonian gravitational potential:
\begin{equation}
...
\begin{align}
\delta U_{g1} &= -\frac{G B_{\text{mag}}^2 r}{2\mu_0 c^2} \xrightarrow{B \to 0} 0 \label{eq:lim1} \\
\delta U_{g2} &= -\frac{G M \alpha [\text{SSq}]}{r^3} \xrightarrow{\alpha \to 0} 0 \label{eq:lim2} \\
\delta U_{g3} &= -\frac{G M \beta \sin(\omega t)}{r} \xrightarrow{\beta \to 0} 0 \label{eq:lim3} \\
\delta U_{g4} &= -\frac{G M \gamma e^{-\kappa t_n}}{r} \xrightarrow{\gamma \to 0} 0 \label{eq:lim4} \\
...
\end{equation}
recovering Newton's inverse-square law. This confirms that Newtonian gravity is \textit{emergent} from the UQFF framework---it is the zeroth-order term with all quantum-aether corrections turned off.
...
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Term} & \textbf{Ratio to Newtonian} & \textbf{Scale} \\
\midrule
$|\delta U_{g1}|/|GM/r|$ & $B^2 r^3 / (2\mu_0 M c^2)$ & $\sim 10^{-24}$ (Solar $B$) \\
$|\delta U_{g2}|/|GM/r|$ & $\alpha [\text{SSq}] / r^2$ & $\sim 10^{-22}$ \\
$|\delta U_{g3}|/|GM/r|$ & $\beta$ & $\sim 10^{-10}$ (calibrated) \\
$|\delta U_{g4}|/|GM/r|$ & $\gamma e^{-\kappa t_n}$ & $\sim 10^{-8}$ (calibrated) \\
$|U_{bi}|/|GM/r|$ & $16\pi \lambda_{\text{UA}} r^3 / (3 M c^4)$ & $\sim 10^{-11}$ \\
\bottomrule
\end{tabular}
\caption{UQFF correction magnitudes relative to Newtonian potential for Solar system scales. All corrections are perturbatively small, validating the classical limit.}
\label{tab:newtonian_corrections}
\end{table}

The UQFF corrections are at most $\sim 10^{-8}$ of the Newtonian base for typical astrophysical systems, confirming that Newtonian gravity is the dominant contribution and UQFF effects are perturbative corrections.
...
\begin{enumerate}
    ...
    \item Newtonian gravity proven emergent (zeroth-order limit of $F_U$, Sec.~2.2)
\end{enumerate}
