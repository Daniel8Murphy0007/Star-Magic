...
**Significance:** The phonon Lagrangian extends the E(t) variational framework to include frequency-dependent phonon modulation. This is the highest-level Lagrangian in the Session 209 hierarchy: L_phonon contains L_{E(t)} (PAPER_888) as the omega -> omega_SCm limit where Phi -> Phi_max.
...
This paper maps to **phonon-variational sector** of the 9-sector UQFF Lagrangian (see `uqff_lagrangian_derivation.py`).
...
For this system, the local VDS sub-ratio is $0.079$ (deep sub-threshold regime), placing it in the zone where exponential damping dominates. This threshold behavior connects to the PAPER_877 cosmogenesis Stage 1 vacuum density initialization: $\rho_{\rm vac} = \rho_{\rm UA} + \rho_{\rm SCm} = 7.799 \times 10^{-36}$ kg/m³.
...
| DVP prime | $p_k \in$ {2,3,...,113} | $p_{\rm DVP} = 79$ | ✓ Super-threshold |
