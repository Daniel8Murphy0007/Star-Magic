...
**Significance:** The 66.7% GW damping fraction is a quantitative prediction that distinguishes UQFF from GR. While GR predicts h_GR, UQFF predicts h_observed = h_GR (1 - 0.667) = 0.333 h_GR. This factor arises from the buoyancy-erosion of gravitational wave energy in the SCm vacuum.
...
This paper maps to **GW-erosion sector** of the 9-sector UQFF Lagrangian (see `uqff_lagrangian_derivation.py`).
...
For this system, the local VDS sub-ratio is $0.073$ (deep sub-threshold regime), placing it in the zone where exponential damping dominates. This threshold behavior connects to the PAPER_877 cosmogenesis Stage 1 vacuum density initialization: $\rho_{\rm vac} = \rho_{\rm UA} + \rho_{\rm SCm} = 7.799 \times 10^{-36}$ kg/m³.
