...
| Redshift of lens | $z_{\mathrm{lens}}$ | 0.5 | dimensionless |
| Angular distance ratio | $L_{\mathrm{factor}} = D_{LS}/D_S$ | 0.67 | dimensionless |
| Magnetic field | $B$ | $10^{-5}$ | T |
| Gas velocity | $v_{\mathrm{gas}}$ | $10^5$ | m/s |
| Oscillatory amplitude | $A_{\mathrm{osc}}$ | $10^{-12}$ | m/s2 |
...
$$L_t = \underbrace{\frac{GM}{c^2 r}}_{\text{DPM mass gradient}} \cdot L_{\mathrm{factor}}, \quad L_{\mathrm{factor}} = \frac{D_{LS}}{D_S} = 0.67$$

$${\mathrm{corr}}_L = 1 + L_t$$
...
$$H(z=0.5) = H_0\sqrt{\Omega_m(1+z)^3 + \Omega_Lambda} = H_0\sqrt{0.3 \cdot (1.5)^3 + 0.7}$$
...
|---------|----------------------|-------------------------|
| Lensing factor | $L(t) = L_0\,e^{-t/\tau}\cos(\omega_{\mathrm{lens}}t)$ | $L_t = (GM/c^2r)\cdot(D_{LS}/D_S)$ |
...
| Parameter | $L_0=0.15$, $\tau_{\mathrm{lens}}$, $\omega_{\mathrm{lens}}$ | $L_{\mathrm{factor}}=0.67$ (distance ratio) |
| Oscillation | None | Two-mode standing + traveling wave |
| DM pert2 | Not present | $3\mu_s\nabla(M_s/r)/r$ tidal correction |
| Wind term | Not present | $\rho_w v_w^2/\rho_{\mathrm{fl}}$ |
...
$${\mathrm{corr}}_L \approx 1.0016 \qquad (\sim 0.16\%\,{\mathrm{amplification}})$$
...
*PAPER_242 \| Session 60 \| CP3 class 111 (RingsOfRelativityEinsteinLensingMUGECalculator) \| UQFF
v4.10*
