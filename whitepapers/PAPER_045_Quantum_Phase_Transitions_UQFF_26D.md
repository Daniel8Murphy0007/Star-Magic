...
**Validator:** `test_{phase2\_validation}.py` Test Suite 1 (Quantum Level 26 Framework): 10/11 PASS  
...
but the lookup function `get_{level\_by\_scale}(r)` is implemented as finding the nearest level where
...
The `get_{level\_info}(10)` call returns complete metadata:
...
*Validator: `t`est_{phase2\_validation}`.py` 10/11 PASS (91%) | $\kappa$ = 0.0005/day | [SSq] = 0.57*
