...
Mode 1 (standing wave): `g_osc1 = 2A·cos(kx)·cos(ωt)` — the classic interference pattern of two equal-amplitude counter-propagating waves. Mode 2 (traveling wave): `g_osc2 = (2π/T_H_gyr)·A·cos(kx − ωt)` — a unidirectional propagating disturbance whose amplitude is suppressed by the inverse Hubble time in gigayears, `(2π/T_H_gyr)`, connecting gravitational oscillations to the cosmological expansion rate.

The key resonance condition — `ω_local = 2π/t_Hubble` — places the system at the threshold where Mode 2 dominates the superposition because `(2π/T_H_gyr) → 1` for `T_H_gyr = 2π`. Away from resonance, Mode 1 dominates for Hubble times much larger than 2π Gyr. The time-averaged result `⟨g_osc⟩ = 0` ensures no net secular drift — oscillatory gravity is a zero-mean perturbation to the static MUGE field.
...
| Angular frequency | ω | 2πc/r | rad/s | Relativistic frequency at scale r |
...
g_osc1 = 2 · A · cos(k·x) · cos(ω·t)

Mode 2 (Hubble-normalised traveling wave):
g_osc2 = (2π / T_H_gyr) · A · cos(k·x − ω·t)
...
```
g₊ = A · cos(kx − ωt)   [forward]
g₋ = A · cos(kx + ωt)   [backward]
g_standing = g₊ + g₋ = 2A · cos(kx) · cos(ωt)   [trig identity]
```

This mode is spatially modulated by `cos(kx)` — nodes at `kx = (n+½)π`, antinodes at `kx = nπ`. At nodes, gravity is unaffected by the standing wave; at antinodes, the oscillation reaches full amplitude 2A.
...
The factor `(2π / T_H_gyr)` has units of `1/Gyr` when T_H_gyr is in gigayears, but since A is already in m/s², the result is dimensionally consistent only if T_H_gyr is treated as a dimensionless ratio `T_H / (1 Gyr)`. This convention is standard in cosmological normalisation within MUGE.

**Physical interpretation:** Gravitational disturbances traveling at cosmological speeds are attenuated by the Hubble expansion. The factor `(2π/T_H_gyr)` is the instantaneous angular expansion rate in units of `(Gyr)⁻¹`, analogous to the Hubble parameter H₀ = 1/t_Hubble but expressed in the natural angular frequency unit.
...
```
(2π / T_H_gyr) = 1  →  T_H_gyr = 2π ≈ 6.28 Gyr
```

For the current Universe (`T_H_gyr = 13.8`), Mode 2 amplitude factor ≈ 0.455 — Mode 2 carries about 45% of Mode 1 amplitude. At early cosmic times (`T_H_gyr → 2π ≈ 6.3 Gyr`, i.e., z ≈ 0.5 in ΛCDM), the two modes were equal in amplitude. At Mode 2 resonance (`T_H_gyr = 2π`), the interference pattern is maximally complex.
...
```
⟨cos(kx)·cos(ωt)⟩_t = cos(kx) · ⟨cos(ωt)⟩_t = 0
⟨cos(kx − ωt)⟩_t   = 0
∴  ⟨g_osc⟩_t = 0
```

This result ensures that oscillatory gravity is a **perturbative zero-mean correction** to the static MUGE field — it modulates gravity on timescale `2π/ω = r/c` (light-crossing time of the system) but produces no secular drift in the total gravitational potential.

### 2.5 Wavenumber and Frequency at Astrophysical Scales

For a system of radius r, the natural wavenumber and frequency are `k = 1/r` and `ω = 2πc/r`. This choice ties the oscillation period to the light-crossing time:
...
- **Early Universe enhancement:** At z ≈ 0.5 (T_H_gyr ≈ 2π), the standing and traveling waves were equal — enhanced gravitational perturbations at this epoch may leave an imprint in the large-scale galaxy power spectrum at `k ≈ 0.1 h/Mpc`.
