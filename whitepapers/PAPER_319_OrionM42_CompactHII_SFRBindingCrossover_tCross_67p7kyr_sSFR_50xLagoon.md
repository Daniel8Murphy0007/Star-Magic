---
paper_id: PAPER_319
title: "Compact HII SFR Gravitational Binding Phase Transition"
session: 91
date: 2026-03-01
...
**Session:** 91  
**Module:** ORION_{UQFF\_MODULE}.cpp (33rd C++ module)  
...
acceleration amplified by the SFR mass factor m_factor(t) = 1 + SFR$\times$t_yr/M_{sun\_count} crosses the
...
### SFR Mass Factor
$$M_{\mathrm{sf}}(t) = \frac{\mathrm{SFR_{yr}} \times t_{\mathrm{yr}}}{M_{\mathrm{sun,count}}}, \qquad m_{\mathrm{factor}}(t) = 1 + M_{\mathrm{sf}}(t)$$

### SFR-Amplified Gravitational Acceleration
$$g_{\mathrm{SFR}}(t) = g_{\mathrm{base}} \times m_{\mathrm{factor}}(t) = g_{\mathrm{base}}\left(1 + \frac{{\mathrm{SFR_{yr}}} \cdot t_{\mathrm{yr}}}{M_{\mathrm{sun,count}}}\right)$$

### Wind Acceleration (time-evolving)
$$a_{\mathrm{wind}}(t) = \frac{v_{\mathrm{wind}}^2}{r}\left(1 + \frac{t}{t_{\mathrm{age}}}\right)$$

### Crossover Time t_cross (PAPER_319 Key Result)
Setting g_SFR(t) = a_wind(t):
$$g_{\mathrm{base}}\left(1 + {\mathrm{sSFR}} \cdot t\right) = a_{\mathrm{wind,0}}\left(1 + \frac{t}{t_{\mathrm{age,yr}}}\right)$$

$$t_{\mathrm{cross}} = \frac{a_{\mathrm{wind,0}} - g_{\mathrm{base}}}{g_{\mathrm{base}} \cdot {\mathrm{sSFR}} - a_{\mathrm{wind,0}}/t_{\mathrm{age,yr}}} = \boxed{67{,}730\ \text{yr}}$$

### Specific SFR
$${\mathrm{sSFR}} = \frac{\mathrm{SFR_{yr}}}{M_{\mathrm{sun,count}}} = \frac{1}{2000} = 5\times10^{-4}\ \text{yr}^{-1} = 50\times {\mathrm{Lagoon}}$$

### Gas Depletion Timescale
$$t_{\mathrm{consume}} = \frac{M_{\mathrm{sun,count}}}{\mathrm{SFR_{yr}}} = \frac{2000}{1} = 2000\ \text{yr} \quad \text{(without OMC-1 replenishment)}$$
...
Within ORION_{UQFF\_MODULE}.cpp, the SFR mass factor m_factor(t) enters the base gravity term via
`G*M*(1+M_sf(t))/r^2`, registered as `WOLFRAM_{TERM\_ORION\_BASE}` and `WOLFRAM_{TERM\_ORION\_SFR\_BINDING}`.
...
```cpp
#define WOLFRAM_{TERM\_ORION\_BASE}(val)        (val)
// g_base = (G*M*(1+M_sf(t))/r^2)*(1+Hz*t)*(1-B/B_crit)*(1+f_TRZ)
// M_sf(t) = SFR_yr*t_yr / M_{sun\_count}  [PAPER_319 SFR amplification]

#define WOLFRAM_{TERM\_ORION\_SFR\_BINDING}(val) (val)
...
$$p_{\mathrm{DVP}} = 71, \quad n_{\mathrm{channel}} = 8/26$$
