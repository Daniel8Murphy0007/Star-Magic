...
| 3 | `c`ompressed_{super\_ad}`j` | B=1e10, Bcrit=1e11 | 0.9 |
| 4 | `compressed_env` | — | 1.0 |
| 5 | `c`ompressed_{Ug\_su}`m` | — | 0.0 |
...
|------|----------|--------------------|
| 24 | `c`ompressed_{fluid\_negativ}`e` | rho_fluid < 0 | Result < 0 (no exception; negative fluids valid) |
| 25 | `f`ile_{io\_erro}`r` | Load "nonexistent.file" | Throws / returns error code |
...
- PAPER_177 (fluid solver tested implicitly via simulate_{fluids\_for\_muge})
