...
GW150914, the first direct detection of gravitational waves, was produced by a binary black hole merger with component masses 36 + 29 M? at luminosity distance d\_L = 410 Mpc (z $\sim$ 0.09). The LIGO detectors measured a peak strain of \textasciitilde{}10?21 consistent with GR predictions.
...
\begin{enumerate}
  \item \textbf{String rotation coupling (\ss{}\_string):} String-tension-mediated coupling between the GW field
\end{enumerate}

and the quantum vacuum. Calibrated as \ss{}\_string = 0.37.
...
At d = 410 Mpc for a short-duration event (0.2 s chirp), ?\_aether is negligible and U\_A = 1.0000.
...
The string coupling is the dominant damping channel. UQFF string tension \ss{}\_string = 0.37 gives:
...
String (\ss{}\_string) & 0.3700 & 0.3330 \tabularnewline
\bottomrule
\end{longtable}
\end{center}

\textbf{Combined damping: D = 0.333 ? 66.7\% amplitude reduction}
...
\begin{equation}
\begin{aligned}
  & ?f = 2p \times \beta_string_correction \times N_cycles \\
...
Component masses & 36 + 29 M? & GR inference \tabularnewline
Final mass & \textasciitilde{}62 M? & Energy conservation \tabularnewline
...
underestimate H0 by \textasciitilde{}D $\sim$ 0.33 relative to electromagnetic methods.

\begin{enumerate}
  \item \textbf{Template bank coverage:} LIGO matched-filter pipelines using GR templates will recover UQFF
\end{enumerate}

signals at reduced efficiency; a UQFF template bank covering D ? [0.30, 0.40] would improve detection rate.
