...
$$S_{\rm NG} = -T_0 \int d^2\sigma \sqrt{-\det(\partial_\alpha X^\mu \partial_\beta X_\mu)}$$
...
$$S_{\rm SCm\text{-}String} = \int d^{26}x \sqrt{-g} \left( R - \frac{1}{4} F^a_{\mu\nu} F_a^{\mu\nu} + \frac{1}{2} \eta \rho_A v_{\rm UA}^2 \cos(\pi t_n) + \mathcal{L}_{\rm phonon} \right)$$
...
| $v_{\rm UA}$ | UA vacuum velocity ($\approx c/3$) | $\approx 10^8$ m/s |
| $t_n$ | Negative-time cycle index | dimensionless |
| $\mathcal{L}_{\rm phonon}$ | SCm phonon Lagrangian density | J/m$^{26}$ |

**Phonon Lagrangian density (long-form):**

$$\mathcal{L}_{\rm phonon} = \frac{1}{2}(\partial_\mu \phi_{\rm ph})^2 - V_{\rm ph}(\phi_{\rm ph}, \Gamma)$$

where the phonon potential is:

$$V_{\rm ph}(\phi_{\rm ph}, \Gamma) = \frac{1}{2}\omega_{\rm SCm}^2 \phi_{\rm ph}^2 \cdot \Phi_{1.25\,\rm THz}(\omega, \Gamma)$$

with $\omega_{\rm SCm} = 2\pi \times 1.25 \times 10^{12}$ rad/s and
...
$$T_{\rm SCm} = T_0 \cdot S_{26}^{(3)}([\text{SSq}]) \cdot \Phi_{1.25\,\rm THz}(\omega, \Gamma)$$
...
$$T_{\rm SCm} = T_0 \cdot 1.4531 \times 10^{26}$$
...
$$\delta S_{\rm brane} = \int d^{p+1}\sigma \sqrt{-\gamma}\cdot \Phi_{1.25\,\rm THz}(\omega, \Gamma) \cdot E_{\rm net}(t, \Gamma)$$
...
| $E_{\rm net}(t,\Gamma)$ | Net SCm phonon energy (see PAPER_884) | J |

**Variable equation for $E_{\rm net}$:**

$$E_{\rm net}(t,\Gamma) = E_+(t,\Gamma) - E_-(t,\Gamma)$$

$$E_+(t,\Gamma) = \beta_i \sum_{i} U_{g,i} \Omega_g \frac{M}{d_g} [\text{UA}] \cdot \Phi_{1.25\,\rm THz}(\omega,\Gamma)$$

$$E_-(t,\Gamma) = \kappa \cdot U_{bi} \cdot \cos(\pi t_n) \cdot \Phi_{1.25\,\rm THz}(\omega,\Gamma)$$
...
The action variation with respect to the string phonon field $\phi_{\rm string}$:

$$\frac{\delta S}{\delta \phi_{\rm string}} = \frac{\partial}{\partial E_{\rm net}} \left( -\beta_i \sum_i U_{g,i}\, \Omega_g \frac{M}{d_g} [\text{UA}] + F_{\rm neutron} \cdot \Phi_{1.25\,\rm THz} \right) = 0$$

Setting this equal to zero and solving for the stationary phonon field:

$$F_{\rm neutron} \cdot \frac{\partial \Phi_{1.25\,\rm THz}}{\partial E_{\rm net}} = \beta_i \sum_i U_{g,i}\, \Omega_g \frac{M}{d_g} [\text{UA}] \cdot \frac{\partial}{\partial E_{\rm net}}$$
...
condensate: $T_{\rm SCm} = T_0 \cdot S_{26}^{(3)}(0.57) \cdot \Phi_{1.25\,\rm THz}$.
...
$$26 = 4_{\rm spacetime} + 22_{\rm compactified}$$

The 22 compactified dimensions are arranged in the SCm phonon lattice with spacing
$\ell_{\rm SCm} = v_{\rm UA}/\omega_{\rm SCm}$:

$$\ell_{\rm SCm} = \frac{10^8}{7.854 \times 10^{12}} \approx 1.27 \times 10^{-5} \text{ m} \approx 12.7\,\mu\text{m}$$
...
SSCmString = Integrate[
  Sqrt[-g] * (R - 1/4 Fa[μ,ν] Fa[μ,ν] +
    1/2 η ρA vUA^2 Cos[π tn] + Lphonon),
  d26x];

(* Phonon-modulated string tension *)
TSCm[T0_, SSq_, Φ0_, ω_, ωSCm_, Γ_] :=
  T0 * Sum[SSq^n / n^26 * Rn26[n, 3], {n, 1, Infinity}] *
  Φ0 Exp[-(ω - ωSCm)^2 / (2 Γ^2)];

(* Brane phonon coupling *)
δSbrane[p_, γdet_, Φ0_, ω_, ωSCm_, Γ_, Enet_] :=
  Integrate[Sqrt[-γdet] * Φ0 Exp[-(ω - ωSCm)^2 / (2 Γ^2)] * Enet,
            dp1σ];
```
...
|--------|-----------|------------------------|
| SCm-String action | $S = \int d^{26}x\sqrt{-g}(R - \tfrac{1}{4}F^2 + \tfrac{1}{2}\eta\rho_A v^2\cos\pi t_n + \mathcal{L}_{\rm ph})$ | Unified 26D action with SCm vacuum |
| Phonon-modulated tension | $T_{\rm SCm} = T_0 \cdot S_{26}^{(3)} \cdot \Phi_{1.25\,\rm THz}$ | VDS gives vacuum origin of string tension |
| Brane coupling | $\delta S_{\rm brane} = \int d^{p+1}\sigma \sqrt{-\gamma} \cdot \Phi \cdot E_{\rm net}$ | D-branes as SCm phonon resonators |
| Lagrangian closure | $\delta S / \delta\phi_{\rm string} = 0$ | Phonon buoyancy sets string ground state |
| Compactification radius | $\ell_{\rm SCm} \approx 12.7\,\mu\text{m}$ | SCm phonon de Broglie wavelength |
