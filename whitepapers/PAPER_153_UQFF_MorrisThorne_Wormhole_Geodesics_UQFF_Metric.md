# PAPER #153 — UQFF Morris-Thorne Wormhole Geodesics: UQFF Metric Integration
...
The **UQFF framework reduces the exotic matter requirement by ~93%** relative to GR alone, with the remaining 7.2×10^29 J/m³ provided by the SCm energy density (ρ_SCm·v_SCm² = 10^31 J/m³ > required).
...
- This 90/10 split mirrors the Ω_Λ/Ω_m ratio in ΛCDM (0.685/0.315 ≈ 2.2) at the order-of-magnitude level
...
| Shape function decay rate | κ = 5×10^-4/day → spatial | exponential falloff |
...
| Throat equilibrium condition | ρ_SCm·v²_SCm = c²/(8πGr₀²) | — |
...
2. The equilibrium throat radius predicted by UQFF is r_0 ≈ 2.32 mm — set entirely by the SCm energy density parameters (ρ_SCm, v_SCm) with no free parameters.
