...
| Sun | 1.989e30 | 6.96e8 | 1.496e13 | 5778 | 2.5e-6 | 1e-4 | 1e15 | 1e-11 | 2p/(11\times3.156e7) | 
| Earth | 5.972e24 | 6.371e6 | 1e7 | 288 | 7.292e-5 | 3e-5 | 1e12 | 1e-12 | 2p/(11\times3.156e7) | 
| Jupiter | 1.898e27 | 6.9911e7 | 1e8 | 165 | 1.758e-4 | 4e-4 | 1e13 | 1e-11 | 2p/(11\times3.156e7) |

| Neptune | 1.024e26 | 2.4622e7 | 5e7 | 72 | 1.083e-4 | 2e-5 | 1e12 | 1e-12 | 2p/(11\times3.156e7) | 
--- 
...
- Larger values of \compute_Ereact: Ereact = (SCm_density \times v_SCm2/rho_A) \times exp(-?t) 
- Higher \mu_s (DPM moment) via `SCm_contrib = 1e3` (placeholder constant) 
- Stronger Ug3 and Um through \compute_Bj and \compute_{mu\_j} 
...
Ug2 ? (QA_global + body.QUA) \times Ms / r2
This ensures each body contributes uniquely to its own heliosphere bubble 
geometry, with the Sun carrying ~10\times more QUA than Earth. 
--- 
### 6. omega_c — Stellar Magnetic Cycle 
All bodies currently share the Solar magnetic cycle period:
omega_c = 2p / (11 \times 365.25 \times 86400) ˜ 1.81e-8 rad/s
```

This drives temporal modulation in:
- mu_s(t): 0.4 $\times$ sin(omega_c $\times$ t) term
- Bj(t): same modulation on the string field
- omega_{s\_t}(t): 0.4e-6 $\times$ sin(omega_c $\times$ t) on spin rate

---

### 7. I/O Support

CelestialBody includes `output_{json\_params}()` and `load_bodies()` supporting
