...
F_U_Bi_i = ∑_{k=1}^{N} [ k^k
          ...
          + k^4 · ρ_vac,[SCm] · M_BH / r
          · e^{-αt} cos(πt_n) · (1 + f_feedback) ]
```
...
```
∑_{k=1}^{∞} k/k^k = ∑_{k=1}^{∞} k^{1-k} ≈ 1.2913 (Komornik-Loreti constant vicinity)
∑_{k=1}^{∞} 1/k^k = ∑_{k=1}^{∞} k^{-k} ≈ 1.2913 (Sophomore's dream integral)
```
...
For calibrated values: f_UA'=0.999, f_SCm=0.001 → product = 9.99×10⁻⁴
With REB1/REB2 ~ 1 (unit resonant coupling): bilinear = 9.99×10⁻⁷ per state pair

### 2.5 Compact/Galactic Results (Vela/Crab vs. NGC 1365)

```
[compact, x_2=2.9 kly]:  F_U_Bi_i ≈ −2.09×10²¹² N
[galactic, x_2=60.7 Mly]: F_U_Bi_i ≈ −8.32×10²¹⁷ N
```
...
```
f_Ub = k_Ub · Δk_η · (ρ_vac,[UA] / ρ_vac,[SCm]) · (V_little / V_big) ~ 0.1
```

| Symbol | Value | Description |
|--------|-------|-------------|
| k_Ub | ~0.1 | Buoyancy coupling constant |
| Δk_η | incremental η correction | η flux differential per state |
| ρ_vac,[UA]/ρ_vac,[SCm] | ~1000 (f_SCm=0.001 → ratio=1000) | Vacuum ratio |
| V_little/V_big | ~10⁻⁴ | Volume of compact core / total volume |
| Product f_Ub | ~0.1 | Final buoyancy fraction |

**Physical significance:** V_little/V_big is the volume fraction of the compact high-density core to the total system volume. For a neutron star in a SNR: V_NS/V_SNR = (10⁴ m)³/(10¹⁶ m)³ = 10⁻³⁶ (very small → real f_Ub < 0.1 for NS+SNR). The calibrated f_Ub = 0.1 applies to the Vela/Crab geometry where V_little is the pulsar wind region.

### 3.3 H_k Geometry Kernel

```
H_k(ν_THz, U_b, geometry_k) = H_k,0 · [ν_THz/ν_ref] · U_b · Ω_k
```
- ν_THz = 10¹² Hz (THz reference frequency)
- U_b = buoyancy energy per state
- Ω_k = solid angle factor for k-th geometry
...
2. **FIRST F_U_Bi explicit H_k geometry-kernel function** — H_k(ν_THz, U_b, geometry_k)
3. **FIRST f_Ub volume ratio definition** — k_Ub·Δk_η·(ρ_UA/ρ_SCm)·(V_little/V_big)~0.1
...
F_U_Bi_i = ∑_{k=1}^{N} [k^k · (f_UA'1·f_SCm1·REB1)·(f_UA'2·f_SCm2·REB2)/r²
                         · G_k(UA,Ub,ν_THz,geometry_k)
                         + k^4·ρ_vac,[SCm]·M_BH/r·e^{-αt}cos(πt_n)(1+f_feedback)]

F_U_Bi = ∑_{k=1}^{N} [k_Ub,k·f_UA'·f_SCm·REB/r²·H_k(ν_THz,U_b,geometry_k)·f_Ub]

f_Ub = k_Ub·Δk_η·(ρ_vac,[UA]/ρ_vac,[SCm])·(V_little/V_big) ~ 0.1

f_UA' = 0.999  [calibrated]; f_SCm = 0.001  [calibrated]; α = 5×10⁻⁵ day⁻¹

[compact]  F_U_Bi_i ≈ −2.09×10²¹² N; F_U_Bi ≈ +9.79×10⁻³³ N
[galactic] F_U_Bi_i ≈ −8.32×10²¹⁷ N
```
