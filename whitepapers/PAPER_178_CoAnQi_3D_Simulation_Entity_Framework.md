...
unsigned int Texture::loadTexture(const std::string& path) {
    ...
    // Creates OpenGL texture with GL_{UNSIGNED\_BYTE}
    // Generates mipmaps: glGenerateMipmap(GL_{TEXTURE\_2D})
    // Wraps: GL_REPEAT; Filters: GL_{LINEAR\_MIPMAP\_LINEAR}
}
```
...
### 10. populate_{simulation\_entities}() — MUGE?Entity Mapping
...
void populate_{simulation\_entities}(
    ...
    for (auto& sys : systems) {
        ...
        e.model = load_{or\_generate\_mesh\_for}(sys.name);
