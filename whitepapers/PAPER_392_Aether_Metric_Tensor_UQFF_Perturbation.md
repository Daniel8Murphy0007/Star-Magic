...
**Source:** grok_{share\_cfdcad2f5}.txt, lines ~200–1400 (C++ UQFF simulation code)  
**Section:** `compute_{A\_mu\_nu}()` function in CelestialBody.cpp / main.cpp  
...
```cpp
std::vector<std::vector<double>> compute_{A\_mu\_nu}(double tn, double eta, double Ts00) {
    std::vector<std::vector<double>> A = g_{mu\_nu};  // diag(1,-1,-1,-1)
...
// Output: tr(A) = -2 + 4*eta*Ts00*cos(pi*tn) \approx -2 at t_n=0
```
