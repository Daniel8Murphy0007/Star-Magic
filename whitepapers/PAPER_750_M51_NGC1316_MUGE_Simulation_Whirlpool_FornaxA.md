...
def U_i():
    return lambda_I * (rho_{vac\_SCm} / rho_{vac\_UA}) * omega_i * 1.0 * (1 + F_RZ)
...
plt.savefig('m51_{gravity\_profile}.png', dpi=150)
plt.close()
print("M51 simulation complete. Profile saved to m51_{gravity\_profile}.png")
```
...
def U_i():
    rho_{vac\_UA} = 7.09e-36
    return 1.0 * (rho_{vac\_SCm} / rho_{vac\_UA}) * 1e-8 * 1.0 * 1.01
...
plt.legend()
plt.grid(True, alpha=0.3)
plt.savefig('ngc1316_{gravity\_profile}.png', dpi=150)
plt.close()
print("NGC 1316 simulation complete. Profile saved to ngc1316_{gravity\_profile}.png")
```
