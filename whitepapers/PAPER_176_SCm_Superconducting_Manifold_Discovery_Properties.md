...
```
E_react = (SCm_density × v_SCm² / ?_A) × exp(-?t)

v_SCm = 0.99c   (SCm flows at relativistic speed within magnetic strings)
?_A   = 1e-23 kg/m³  (ambient Aether density)
?     = 0.0005/day   (decay rate calibrated to Sun-SgrA*)
...
µ_s(t) = (Bs + 0.4×sin(?_c×t) + SCm_contrib) × Rs³
...
The UQFF calibration constant ?=0.0005/day is derived from requiring that
`E_react(t=t_sun_age)` matches the observed solar luminosity:

```
? = -ln(L_observed / L_initial) / t_sun_age
