...
#### 2.2 In DPM Moment \mu_s (Ug1)
```
SCm_contrib = 1e3  (constant contribution to stellar DPM moment)
\mu_s(t) = (Bs + 0.4\timessin(?_c\timest) + SCm_contrib) \times Rs3
...
$$
\begin{aligned}
  & Bj(t) = 1e-3 + 0.4\times\sin(?_c\times t) + SCm_contrib \\
...
`E_react(t=t_{sun\_age})` matches the observed solar luminosity:
...
| SCm_contrib | 1e3 (field proxy) | Contribution to \mu_s and Bj |
...
- CelestialBody.cpp: compute_Ereact, compute_{mu\_s}, compute_Bj
