...
**Observable:** Charge multiplicity dN_ch/dη vs. √s with centrality

**UQFF connection:**
```
dN_ch/dη = η · k_η · exp(-[SSq] · n/26)   at n=18, √s^{0.156} power law
```

| Parameter | Value | Description |
|-----------|-------|-------------|
| k_η | 10¹³ cm⁻²/s (BESIII) | Flux-coupling constant |
| n | 18 | Heavy state level (ATLAS vector-like regime) |
| √s^{0.156} | power-law index | Collision energy scaling |
| exp(-[SSq]·18/26) | exp(-0.507×0.692) = 0.702 | Level-18 suppression |

**Constraint:** At n=18, the ratio dN_ch/dη(√s)/dN_ch/dη(ref) ∝ exp(-[SSq]·18/26) × √s^{0.156} — this directly calibrates the [SSq]×centrality product.
...
**UQFF connection:**
```
Um ∋ b_p · sin(m_a · t + φ)   [axion coupling through Um magnetism bilinear]
```

| Parameter | Value | Description |
|-----------|-------|-------------|
| b_p | axion coupling strength | nm coupling from Um bilinear |
| m_a | axion mass | Angular frequency m_a·c²/ℏ |
| φ | initial phase | Spatial phase |
...
**Observable:** Tau anomalous magnetic moment a_τ ~ 10⁻³

**UQFF connection:**
```
a_τ → μ_j · cos(πt_n)   [tau dipole maps to Um magnetic moment with t_n modulation]
```

| Parameter | Value | Description |
|-----------|-------|-------------|
| a_τ | ~10⁻³ | Tau anomalous magnetic moment |
| μ_j | depends on state j | Magnetic moment per UQFF state |
| cos(πt_n) | time modulation | UQFF temporal coupling |

**Constraint:** a_τ ~ 10⁻³ sets the scale for μ_j when integrated over all states. Super Tau-Charm Factory limits constrain the product `μ_j × P_SCm × E_react` in Um.

### 2.5 Experiment 5 — JUNO (Jiangmen Underground Neutrino Observatory)

**Observable:** PMT dark count rate (DCR), gain ~10⁷, spikes in noise
...
| DCR gain | 10⁷ | PMT dark count amplification factor |
...
**Observable:** Double-Cabibbo-Suppressed (DCS) decay branching ratio BR ~ 10⁻⁴

**UQFF connection:**
```
BR_DCS ~ 10⁻⁴ → η ~ 10¹³ cm⁻²/s   [light quark sector flux calibration]
```

| Parameter | Value | Description |
|-----------|-------|-------------|
| BR_DCS | ~10⁻⁴ | DCS branching ratio |
| η | 10¹³ cm⁻²/s | Particle flux (same as ALICE k_η) |

**Constraint:** The DCS BRs at BESIII provide a light-quark sector calibration of k_η, independently confirming the ALICE result from a different energy regime.

### 2.7 Experiment 7 — LHCb (Lepton Flavor Violation)

**Observable:** Lepton flavor violating decay BR < 10⁻⁶

**UQFF connection:**
```
BR_LFV < 10⁻⁶ → t_n < 0   [Um reversal trigger; negative time-zone boundary]
```

| Parameter | Value | Description |
|-----------|-------|-------------|
| BR_LFV | < 10⁻⁶ | LFV branching ratio upper limit |
| t_n | < 0 | Negative time zone (T-reversal) |
| Um reversal | (1−e^{−γt}cos(πt_n))→flip | Signal of time-zone crossing |

**Physical significance:** LFV requires lepton number violation, which in UQFF occurs when t_n < 0 triggers a sign flip in the Um bilinear. The BR < 10⁻⁶ limit constrains the frequency of t_n < 0 events in the integration path.

### 2.8 Experiment 8 — ATLAS (Vector-Like Quarks)

**Observable:** Vector-like quark coupling κ = 0.14–0.52

**UQFF connection:**
```
κ = 0.14–0.52 → SC_m at heavy state n=18
```
...
**Constraint:** The κ range 0.14–0.52 encompasses the UQFF prediction for SC_m at level n=18. The geometric mean √(0.14×0.52) ≈ 0.27 coincides with the UQFF-predicted SC_m in the heavy-quark limit.

### 2.9 Experiment 9 — ECFA (Higgs/Electroweak Studies)

**Observable:** Higgs coupling modifier κ_Higgs approaching unity at high precision

**UQFF connection:**
```
κ_Higgs = 47.34 (UQFF)   →   κ_Higgs,level18 ≈ 1.0
```

| κ_Higgs value | Level | Physical meaning |
|---------------|-------|-----------------|
| 47.34 | Level 1 | UQFF fundamental coupling |
| 1.0 | Level 18 | Standard-model-normalized at n=18 |
| Scaling | κ(n) = 47.34/n^β | Power-law level scaling |
...
b = 9.96
κ_Higgs = 47.34
τ_dev = 5×10⁻⁸ at r = 0.3 fm  (<5% error vs. Super Tau-Charm limits)
```

### 2.10 Experiment 10 — NOMAD (Near Oscillation Magnetic Axial Detector)

**Observable:** Monophoton events from ν polarizability
...
|---|-----------|-----------|--------------|-----------------|
| 1 | EDM SO(10) | d_e~10⁻²⁵ e·cm | Fu += d_e·e/(2m_e c)·exp(-[SSq]n/26) | Constrains Im([SSq]) |
| 2 | ALICE | dN_ch/dη, √s^{0.156} | η·k_η·exp(-[SSq]·18/26) | k_η = 10¹³ cm⁻²/s |
| 3 | Comagnetometer | 75% error @20 Hz | Um ∋ b_p·sin(m_a t+φ) | m_a to refine |
| 4 | Tau dipole | a_τ~10⁻³ | μ_j·cos(πt_n) | Super Tau-Charm fit |
| 5 | JUNO PMT | DCR gain 10⁷ | SC_m → Qs=0 | SC_m=1 boundary |
| 6 | BESIII DCS | BR~10⁻⁴ | η~10¹³ cm⁻²/s | k_η confirmed |
| 7 | LHCb LFV | BR<10⁻⁶ | t_n<0 Um reversal | TRZ boundary |
| 8 | ATLAS VLQ | κ=0.14–0.52 | SC_m heavy n=18 | SC_m~0.27 |
| 9 | ECFA Higgs | κ_Higgs~1 @n=18 | κ_Higgs=47.34 | g-2: a=4.74e-5 |
| 10 | NOMAD | ν polarizability | [SSq] n=13 | exp(-[SSq]/2)=0.776 |
...
3. **FIRST UQFF axion comagnetometer coupling** via Um: `Um ∋ b_p·sin(m_a t+φ)`
...
dN_ch/dη = η·k_η·exp(-[SSq]·18/26)           [ALICE level-18 multiplicity]
Um ∋ b_p·sin(m_a t+φ)                         [comagnetometer axion]
a_τ~10^{-3} → μ_j·cos(πt_n)                   [tau dipole]
SC_m=1 → Qs=0 (JUNO PMT DCR)                  [JUNO identification]
BR_DCS~10^{-4} → η~10^{13} cm^{-2}/s          [BESIII k_η]
BR_LFV<10^{-6} → t_n<0 Um reversal            [LHCb boundary]
κ_VLQ=0.14-0.52 → SC_m heavy n=18             [ATLAS VLQ]
κ_Higgs=47.34; g-2: a=4.74e-5, b=9.96         [ECFA/g-2 fit]
...
- ALICE centrality: Pb-Pb √s=5.02 TeV
