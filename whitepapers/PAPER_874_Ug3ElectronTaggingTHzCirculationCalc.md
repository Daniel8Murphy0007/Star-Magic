...
U_g3 = U_i + U_m in motion. U_i individually tags each electron via the THz hole pipeline from
nucleus (Point A) to electron (Point B), counteracting the strong-force to position electrons at
projected quantum energy shell balance. Electron DPM uses limited U_i for coherent circulation in
U_g2 imaginary orbit shell. U_g2 monitors and adjusts shell position.

          ## Key Equations

            1. `F_Ug3 = (F_Ui + F_Um + F_{DPM\_e}) * G_geo / r_shell^2`
2. `F_Ui = k_i * fUA'_nuc * nu_THz * R_EB   (repulsive tagging)`
3. `F_Um = k_m * fSCm_nuc * nu_res   (SCm-driven motion)`
4. `F_{DPM\_e} = k_e * (fUA'_e * fSCm_e) * nu_THz   (electron circulation)`
