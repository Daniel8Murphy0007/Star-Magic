...
**Author:** Daniel T. Murphy  
**WOLFRAM_TERM:** `RSC_UQFF:a_{sc\_freq}=hbar*f_super*f_DPM*a_DPM/(E_vac*c); A_sc=6.994e21;
...
| B (T) | B/B_crit | SCm | `g_{res\_sc}` / a_res |
|--------|----------|-----|-----------------|
...
At B = 0: g_{res\_sc} = 1.1 $\times$ a_{res\_total} (maximum resonance contribution).
...
| SC frequency term | `a_{sc\_freq}` | 2.479$\times$104 m/s2 |
