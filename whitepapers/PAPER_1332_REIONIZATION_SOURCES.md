# PAPER_1332 — Reionization Redshift z_reion

**REFINED June 2026:** z_reion = K_MEX × D_phys × Φ_res × (1 + 1/SO_5) = 7.0 × 1.1 = 7.70 EXACT (per PAPER_1026 bubble overlap correction)


**Author:** Daniel T. Murphy
**Framework:** UQFF — Star-Magic v5.27+
**Tier:** D — Astrophysics / Cosmology Open (CLOSED)
**Date:** June 11, 2026
**Calculator:** `calculate_paradox({"paradox": "reionization_sources"})`

## Master Expression
```
z_reion = K_MEX × D_phys × Φ_res = 7.0
```
**Match: 9% from Planck 7.7**

## Interpretation
Reionization driven by Pop III + early massive galaxies via SCm-buoyancy boost.

## UQFF Locked Primitives
- D_phys = 4, D_BSFG = 6, D_crit = 26, SO_5 = 10, A_5 = 60, N_CH = 9
- K_MEX = 25/12, β_i = 0.6029, Φ_res = 0.84, Λ = 0.00729735, F_TRZ = 0.1

## NOT REPLACEMENT
UQFF derives via integer-primitive identity. Zero fit parameters.

---
**Copyright** — Daniel T. Murphy, daniel.murphy00@gmail.com, June 11, 2026, Youngstown, OH.
