---
paper_id: PAPER_015b
title: "PAPER_015b: Multi-Band Gravitational Wave Astronomy: LISA+LIGO Synergy Under UQFF"
session: 0
date: 2026-03-07
author: "Daniel T. Murphy"
status: production
cvw: "v2.0.0"
tags: [GW, merger, gravitational-wave, vacuum, SMBH, LIGO, damping, UQFF]
...
We quantify the impact of UQFF vacuum damping on multi-band gravitational wave detection, combining
...
Multi-band gravitational wave astronomy — detecting the same compact binary system separately at low
frequencies (years before merger with LISA) and at high frequencies (at merger with LIGO/ET/CE) — is
one of the most powerful probes of GW physics. Jointly, the two frequency bands measure:
...
In UQFF, the amplitude reduction factor is frequency-independent above ~20 Hz, allowing a clean
test: the same UQFF factor D should suppress both the LISA and LIGO observations of the same source
equally.
...
UQFF factor applied: D = 0.622 (note: this is the LISA-regime factor from validate_multiband.py).
For the LIGO BBH regime at z < 0.3, the pure factor should be D = 0.333, but the multiband
simulation uses D = 0.622 as a cross-band average.
...
| UQFF | 167 | D $\times$ GR = 0.622 $\times$ 268 |
...
| UQFF | 694 | D $\times$ GR = 0.622 $\times$ 1116 |

In both cases, the UQFF factor 0.622 is consistent: multi-band observations of the same source would
reveal a coherent, frequency-independent suppression — the hallmark of UQFF vacuum propagation
rather than a source-property effect.
...
**The UQFF-accessible detection volume is 24% of the GR volume.** This has major implications for GW
source rate predictions:
...
**Forward link (this paper):** Multi-band (LISA + LIGO) synergy is the cleanest falsifier of the v5.78 closure: both bands inherit the SAME $\beta_i = 0.603$ and $F_{TRZ}=1/10$. PAPER_1175 (P11, LIGO O5 ringdown $R_{21}/R_{22} \approx 0.144$) and the LISA-band EMRI counterpart (cf. PAPER_014b) must agree on these constants; this paper's multi-band consistency check IS the synergy test.
