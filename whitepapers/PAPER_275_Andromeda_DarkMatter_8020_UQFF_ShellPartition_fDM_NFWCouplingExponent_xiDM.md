---
paper_id: PAPER_275
title: "UQFF Dark Matter 80/20 Shell Partition  f_DM^(1/3) NFW Coupling Exponent and the xi_DM
Interaction Term"
session: 75
date: 2026-03-01
...
**Keywords:** dark matter, NFW profile, 80/20 partition, f_DM, xi_DM, coupling exponent, shell
partition, Andromeda M31, gravitational DM interaction, cube-root exponent
...
**UQFF dark matter coupling constant** ?_DM = 0.80^(1/3) = 0.9283. We demonstrate that this
f_DM^(1/3) scaling is consistent with the NFW halo profile's central density behavior (? ? r^(-1)
core) and provides a better UQFF representation of the gravitational interaction between the DM halo
and the visible disk than linear superposition. The total DM gravitational term is g_{DM\_total} = g_dm
+ ?_DM  g_vis.
...
In UQFF, simply adding these linearly produces a g_total that matches only the combined mass, losing
information about the structural coupling between the two components. The 80/20 Shell Partition
retains this structural information through the coupling exponent.
...
$$\xi_text{DM} = f_\text{DM}^{1/3}$$

$$g_\text{int} = \xi_text{DM} \times g_\text{vis} = f_\text{DM}^{1/3} \times \frac{G(1-f_\text{DM})M}{r^2}$$
...
$$\rho_text{NFW}(r) \approx \frac{\rho_s r_s}{r} \propto r^{-1}$$
...
This is exactly the scaling that naturally emerges from the NFW mass distribution when the
fractional mass is raised to the 1/3 power. The UQFF coupling ?_DM = f_DM^(1/3) **reproduces the NFW
radial scaling of the DM-visible coupling** without requiring explicit knowledge of the NFW profile 
only the global DM fraction f_DM is needed.
...
matter gravity  some is "screened" by the DM shell geometry.

---

## 5. The UQFF Dark Matter Coupling Constant ?_DM

$$\boxed{\xi_text{DM} = f_\text{DM}^{1/3} = 0.9283}\ \text{(for Andromeda with } f_\text{DM}=0.80\text{)}$$
...
$$\boxed{\xi_text{DM} = f_\text{DM}^{1/3} = 0.9283}\ \text{(Andromeda)}$$

Key discoveries:
1. The 1/3 exponent on f_DM is **not arbitrary**  it reproduces the NFW profile radial scaling from
only the global DM fraction
...
4. The formula generalizes to any galaxy: compute f_DM from observations ? get ?_DM ? apply UQFF DM
partition

---

*Derived from ANDROMEDA_{UQFF\_MODULE}.cpp, UQFF 2.0, Session 75. PAPER_273275 complete the Andromeda
...
$$p_{\mathrm{DVP}} = 13, \quad n_{\mathrm{channel}} = 16/26$$
