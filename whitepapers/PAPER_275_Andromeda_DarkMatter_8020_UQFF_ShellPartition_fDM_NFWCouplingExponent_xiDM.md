...
3. g_int = ξ_DM × g_vis (DM-visible coupling via NFW exponent)
...
- **Visible matter** (stars, gas, dust): concentrated in the disk and bulge, following a Sérsic or exponential profile
- **Dark matter**: distributed in an extended NFW halo with ρ_NFW ∝ (r/r_s)^(−1)(1 + r/r_s)^(−2)
...
With f_DM = 0.80, G = 6.674×10⁻¹¹, M = 1.989×10⁴² kg, r = 1.04×10²¹ m:
...
This is exactly the scaling that naturally emerges from the NFW mass distribution when the fractional mass is raised to the 1/3 power. The UQFF coupling ξ_DM = f_DM^(1/3) **reproduces the NFW radial scaling of the DM-visible coupling** without requiring explicit knowledge of the NFW profile — only the global DM fraction f_DM is needed.

### 3.2 Physical Interpretation of ξ_DM

ξ_DM = f_DM^(1/3) is the **UQFF dark matter shell coupling constant**. Its physical meaning is:

- f_DM = 1 (100% DM, no visible matter): ξ_DM = 1, g_int = 1 × g_vis = 0 (since g_vis = 0). No coupling contribution — pure DM.
- f_DM = 0 (100% visible, no DM): ξ_DM = 0, g_int = 0. No coupling — pure visible.
- f_DM = 0.80 (Andromeda): ξ_DM = 0.9283. The DM shell couples to 93% of the visible matter's gravitational contribution.
- f_DM = 0.5 (equal DM/visible): ξ_DM = 0.7937. Symmetric coupling at 79%.

| f_DM | ξ_DM = f_DM^(1/3) | Physical regime |
|------|-------------------|----------------|
...
**Difference:** Δg = −1.7×10⁻¹² m/s² (~1.4% reduction)

The UQFF partition predicts a slight *reduction* in effective gravitational acceleration compared to the naive sum, because ξ_DM < 1 means the DM-visible coupling does not fully transfer the visible matter gravity — some is "screened" by the DM shell geometry.

---

## 5. The UQFF Dark Matter Coupling Constant ξ_DM
...
2. ξ_DM = 0.9283 is the **UQFF dark matter coupling constant** for M31
3. The partition predicts a 1.4% reduction in effective gravity compared to linear superposition
4. The formula generalizes to any galaxy: compute f_DM from observations → get ξ_DM → apply UQFF DM partition
