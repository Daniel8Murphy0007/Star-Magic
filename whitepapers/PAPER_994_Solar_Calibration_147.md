...
The 99-system $\Gamma$ sweep shows that this solar calibration value is stable across all 7 linewidth values tested ($\Gamma \in \{0.01, ..., 10.0\}\text{ THz}$), confirming that the solar calibration is $\Gamma$-independent (as expected for a static surface gravity measurement).

## 3. Implementation

File: `fubi_{inside\_outside}.py`, class `SolarCalibration147Calc`. CP4 class #578.
