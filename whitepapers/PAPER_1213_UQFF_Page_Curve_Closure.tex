...
\title{UQFF Page-Curve Closure: Black-Hole Entropy Recovery \\
       from the $N_{ch}=9$ Aether Channel Structure}
...
($\mathcal{L}_{\mathrm{aether}}$, sector~7 of PAPER\_1210); each channel
...
\section*{Hawking-radiation entropy closure}
...
\section*{Information-paradox resolution map}
\begin{center}
\begin{tabular}{lll}
\toprule
...
Why does evaporation halt at $M_{\mathrm{Pl}}$? & SCm/UA pair stabilises remnant & $\rho_{\mathrm{SCm}},\rho_{\mathrm{UA}}$ \\
Why Page time $=10/27\,t_{\mathrm{evap}}$? & Half $\times$ $\Phires$ $\times$ $8/9$ & locked rationals \\
Why finite remnant entropy? & $\KMex=25/12$ fixes residual modes & $\mathcal{L}_\phi$ \\
\bottomrule
\end{tabular}
\end{center}
...
\texttt{\_session305d\_page\_curve\_closure.py} computes both the Page
