...
2. **Universal Aether (UA')** — Non-local connectivity medium, vacuum energy carrier, ρ_UA' = 7.09e-36 J/m³
3. **Super Conductive Material (SCm)** — Superconductive magnetism mediator, ρ_SCm = 7.09e-37 J/m³
...
```
U_i → U_m strings  (U_mag_i = magnetic string sequences)
```
...
State i (i=1..26): pre-mass nuclear configuration level
  ...
  - Superconductive magnetism: [SCm]_i = 1e-5 * i² T
...
$$\mathcal{L}_{\rm sector} = \frac{1}{2}(\partial_\mu \phi_{\rm cosmo})(\partial^\mu \phi_{\rm cosmo}) - V(\phi_{\rm cosmo}) + \mathcal{L}_{\rm cosmo}$$
