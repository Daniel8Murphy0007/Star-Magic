...
**Date:** November 2025
**Related files:** source176_{auto\_full\_uqff}.cpp, add_{wolfram\_terms}.py
...
#define WOLFRAM_TERM "g_{SGR1745\_compressed} = (G*1.4e31/9.0e8^2)*(1 + 0.99*0.0005) + 1.114e-52*c^2/3"
```

The Python injector `add_{wolfram\_terms}.py` automates insertion for all existing source files without
one:
...
Starting scan...
PASS source42.cpp → g_{SGR1745\_compressed} = (G*1.4e31/9.0e8^2)*(1 + 0.99...
...
Scanned 10 files...
...
build_time      \approx N_terms \times mean_{term\_length} / (mem_bandwidth)  [seconds]
transfer_time   \approx expression_{size\_bytes} / WSTP_bandwidth  [seconds]
simplify_time   \approx f(N_terms, depth_{of\_cancellations})      [hours to days]
```
...
Related: source176_{auto\_full\_uqff}.cpp, add_{wolfram\_terms}.py
