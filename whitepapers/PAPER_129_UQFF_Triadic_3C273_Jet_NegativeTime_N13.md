...
## 2. UQFF Triadic Mode: Negative Time and F_{U\_tri}
...
130/45 $\times$ 2.9. This factor arises from F_{U\_tri}(+t_n)/F_{U\_tri}(-t_n)  2.9, consistent with the
...
t_{n\_counter} = -0.10  # slightly negative

# Triadic F_U ratio
F_jet = 1 + abs(np.cos(np.pi * 0.0))   # constructive
F_counter = 1 + abs(np.cos(np.pi * t_{n\_counter}))
