...
```
H_res = A_res · sin(2πf_res · t) + U_dp · SC_m · k_nuc + S_shell
```
...
| δ_pair | +0.1 even-Z,N / −0.1 odd-Z,N | Pairing energy correction |
...
| φ_dp | phase angle | Dipole orientation angle |
...
| δ_pair | ±0.1 | Same pairing correction as A_res |
...
- Double magic (e.g., ²⁰⁸Pb: Z=82, N=126): S_shell = 0.1·(82+126) = 20.8
...
```
H_res = A_res · sin(2πf_res · t)    [oscillatory amplitude term]
...
```
δ_pair = +0.1 (Z=1 odd → δ_pair = -0.1, but by convention: even-even = +0.1)
...
H_res = 0.9·k_A·sin(2πf_res·t) + 0 + 0.4
```

**Iron-56 (Z=26, A=56, N=30):**
```
δ_pair = +0.1 (Z=26 even, N=30 even)
...
From PAPER_328, the alpha-BEC LENR enhancement uses `A_res·(1+δ_pair)` and `f_res·(1+S_shell)` factors. Now H_res provides the complete container:

```
LENR_rate ∝ H_res · N_B(T_BEC) · σ_CS(E_CS)
where N_B = 1/(exp(ΔE/kT_BEC) - 1) = 29.76 at T_BEC=14.52 MeV, ΔE=0.48 MeV
```

The U_dp term adds a new channel: dipole-dipole pre-alignment couples nuclear pairs before BEC onset, reducing the effective ΔE required for BEC formation.

**Dipole enhancement estimate:**
```
ΔE_eff = ΔE - U_dp · SC_m · k_nuc ≈ 0.48 - U_dp·k_nuc MeV
N_B_enhanced = 1/(exp(ΔE_eff/kT_BEC) - 1) > N_B = 29.76
```
...
2. **FIRST U_dp dipole-dipole coupling** in UQFF nuclear framework — `k(A₁A₂/f_dp²)cos(φ_dp)`
