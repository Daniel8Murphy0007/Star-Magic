...
exactly DPM-seeded gravity, making the UQFF framework locally undetectable.
...
- **Anti-gravity zone (f_TRZ < -1):** Relevant to dark energy domination epochs (high-z supernovae acceleration). If f_TRZ tracks the equation of state w: `f_TRZ \approx -(1 + w)` for w near -1, the TRZ zero-point corresponds exactly to de Sitter expansion (w = -1).
