...
$$\Delta g_r = \frac{\varepsilon'(r)}{2} = -\frac{3\eta\cos(\pi t_n)\,C_{\mathrm{num}}}{2r^4}$$

with orbital velocity correction $v^2_{\mathrm{orbit}} = \mu_s\nabla(M_s/r) + r\,c^2\,\varepsilon'(r)/2$. At $r = R_\odot$: $|\Delta g_r| \approx 2.73 \times 10^{-11}\ {\mathrm{m/s}}^2 \approx 10^{-13}\,g_{
...
$$\Delta g_r^{(\mathrm{Aether})} = \frac{\varepsilon'(r)}{2} = -\frac{3\eta\cos(\pi t_n)\,C_{\mathrm{num}}}{2r^4}$$
...
$$\frac{v^2_{\mathrm{orbit}}}{r} = \underbrace{\frac{GM}{r^2}}_{\mu_s\nabla(M_s/r)} - \frac{\varepsilon'(r)}{2}$$
...
| $\Delta g_r^{(\mathrm{Aether})}$ | $-2.73 \times 10^{-11}\ {\mathrm{m/s}}^2$ |
| $g_{
m DPM}(R_\odot)$ | $+274\ {\mathrm{m/s}}^2$ |
| Ratio | $\approx 10^{-13}$ |
| $\delta v_{\mathrm{orbit}} = v_{\mathrm{orbit}} - v_{\mathrm{Newton}}$ | $\approx 2.1 \times 10^{-2}\ {\mathrm{m/s}}$ at $r = R_\odot$ |
...
$$p_{\mathrm{DVP}} = 53, \quad n_{\mathrm{channel}} = 10/26$$
...
The geodesic correction $\Delta g_r^{(\mathrm{Aether})} \propto r^{-4}$ falls off faster than the $r^{-2}$ DPM-seeded force, ensuring it is negligible at astronomical distances. However, it becomes meaningful in the near-stellar regime, providing:
...
3. **An intrinsic coupling to SCm field density** — through $C_{\mathrm{num}} \propto M_s c^2$ (stellar rest energy drives the aether gradient).

**Connection to existing UQFF framework:** The term $\Delta g_r^{(\mathrm{Aether})}$ is the geometric origin of the correction term in the MUGE Compressed framework (PAPER_395, SOURCE4), where the effective gravity $g_{\mathrm{eff}} = g_N \cdot (1 + \mathrm{corrections})$ gains contributions from the SCm aether background.
