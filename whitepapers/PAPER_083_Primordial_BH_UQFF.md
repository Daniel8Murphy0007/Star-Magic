---
paper_id: PAPER_083
title: "Primordial Black Hole Mass Distribution: UQFF Corrections to Formation Thresholds and
Present-Day Density"
session: 0
date: 2026-03-07
...
status: production
cvw: "v2.0.0"
tags: [dark-matter, Hawking, vacuum, black-hole, UQFF]
...
**Date:** March 7, 2026  
**Source Data:** validate_{hawking\_temperature}.py (primordial BH test, Test 2), validate_phase3.py  
**Index Slot:** §1.11 Black Hole Physics & Hawking Radiation,  

## Abstract

Primordial black holes (PBHs) form during radiation domination from density perturbations exceeding
dc ~ 0.45. The UQFF modifies the critical density threshold dc_UQFF through the Buoyant vacuum
pressure correction and extends PBH survival above the GR threshold mass M_threshold by 3.5%. The
evaporation temperature of surviving PBHs is T_UQFF = 0.99  T_H, slightly reducing their current
gamma-ray flux. PBHs with M ~ 10-5$\times$10-7 g may constitute part of dark matter  the UQFF predicts the
dark matter fraction f_PBH is unaffected (the 3.5% mass threshold shift does not move PBHs into or
out of the observationally allowed window).
...
$$P_{\mathrm{UQFF}} = P_{\mathrm{radiation}} + P_{\mathrm{vacuum}} = \frac{\rho_{\mathrm{rad}}c^2}{3} + \rho_{\mathrm{vac}} c^2 \times [UA]$$

The fractional correction:

$$\delta_c^{\mathrm{UQFF}} = \delta_c \times \left(1 - \frac{P_{\mathrm{vacuum}}}{P_{\mathrm{rad}}}\right) = 0.45 \times (1 - [UA] \times z_{\mathrm{formation}}^{-4})$$

At z_formation = 106 (typical PBH epoch): P_vacuum/P_rad = 0.0001 $\times$ 10?4 ? negligible.
...
### Standard GR
$$M_{\mathrm{threshold}}^{\mathrm{GR}} = 5.70 \times 10^{11} \text{ kg} \quad (t_{\mathrm{evap}} = t_{\mathrm{universe}} = 4.35 \times 10^{17} \text{ s})$$

### UQFF Threshold
$$M_{\mathrm{threshold}}^{\mathrm{UQFF}} = M_{\mathrm{threshold}}^{\mathrm{GR}} \times (T_{\mathrm{UQFF}}/T_H)^{-4/3} = 5.70 \times 10^{11} \times 0.99^{-4/3} = 5.73 \times 10^{11} \text{ kg}$$

**UQFF threshold: 5.73$\times$10 kg (vs GR 5.70$\times$10 kg)** $\approx$ 0.5% increase.

---

## 3. Present-Day PBH Gamma-Ray Flux

For PBHs near threshold (M ~ M_threshold), the Hawking radiation contributes to the diffuse
gamma-ray background:

$$\frac{d^2\Phi_gamma}{dE\,d\Omega} = \frac{1}{4\pi} \int_0^\infty n_{\mathrm{PBH}}(M) \frac{d^2N_\gamma}{dE\,dt}(M, T_{\mathrm{UQFF}}) \, dM$$

The UQFF T_UQFF = 0.99 T_H reduces peak gamma-ray energy by 1%:

$$E_{\mathrm{peak}}^{\mathrm{UQFF}} = 2.82 k_B T_{\mathrm{UQFF}} = 2.82 k_B \times 0.99 T_H = 0.99 E_{\mathrm{peak}}^{\mathrm{GR}}$$

**Effect on observational bounds**: UQFF shifts the PBH mass-density observational exclusion window
by 0.5% in M. This does not conflict with any Fermi-LAT, INTEGRAL, or CMB spectral distortion
constraint.
...
For M_PBH in the asteroid belt window (10-5$\times$10-7 g, avoiding microlensing and CMB):

$$f_{\mathrm{PBH}}^{\mathrm{UQFF}} = f_{\mathrm{PBH}}^{\mathrm{GR}} \times \frac{M_{\mathrm{threshold}}^{\mathrm{UQFF}}}{M_{\mathrm{threshold}}^{\mathrm{GR}}} \times \frac{T_{\mathrm{UQFF}}^4}{T_H^4}$$

$$= f_{\mathrm{PBH}} \times 1.005 \times 0.96 = f_{\mathrm{PBH}} \times 0.965$$

**UQFF reduces f_PBH by 3.5%**  within the 10-20% uncertainty on current PBH dark matter
constraints.
...
| Survival mass threshold | 5.70$\times$10 kg | 5.73$\times$10 kg | +0.5% |
...
*Source: `validate_{hawking\_temperature}`.py primordial BH tests | $\kappa$ = 0.0005/day | [SSq] = 0.57*
