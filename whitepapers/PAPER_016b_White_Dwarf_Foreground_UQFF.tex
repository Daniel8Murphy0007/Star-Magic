...
where the sum is over all systems contributing to frequency bin f, and $\Delta$f is the frequency resolution.
...
\begin{enumerate}
  ...
  \item \textbf{Foreground reduction:} P\_noise $\rightarrow$ P\_noise $\times$ D\_local2 = P\_noise $\times$ 0.614 (61.4\% noise reduction)
\end{enumerate}
...
For sources at very high redshift (z > 3), D\_cosmo $\neq$ D\_local because Aether compensation saturates:
...
$\beta_i$ & 0.60--0.61 & Buoyancy coupling coefficient \tabularnewline
...
I & 10$^{-22}$ & Inertia tensor scale \tabularnewline
...
-$\Sigma\lambda_i\cdot$U$_i\cdot$E\_react & 4th dissipation term (PAPER\_420) & \texttt{c}ompute\_{FU\_SOURCE}\texttt{4} / full pipeline \tabularnewline
\bottomrule
\end{tabularx}
\end{table}

\textbf{4th dissipation term parameters (PAPER\_420):} \\  $\lambda_1$=10$^{-10}$, $\lambda_2$=10$^{-12}$, $\lambda_3$=10$^{-11}$, $\lambda_4$=10$^{-13}$ (free parameters, not yet empirically calibrated)
...
Symbol & Value & Description \tabularnewline
\midrule
$\rho_c$ & 1015 kg/m3 & SCm critical superconducting density \tabularnewline
...
\textbf{Buoyant} & $\beta_i$ $\times$ Ubi & Expanding nebulae, stellar winds \tabularnewline
...
Proton decay rate & $\kappa$ = 0.0005/day $\rightarrow$ G\_p suppression & < 4.17$\times$ 10$^{-35}$/yr & Super-K 2024 & PASS Consistent \tabularnewline
...
\textbf{New physics claim:} UQFF introduces buoyancy-based gravitational corrections (F\_{U\_Bi\_i}) that produce measurable deviations from GR at scales where vacuum condensate density $\rho_{SCm}$ becomes significant, offering a falsifiable prediction beyond the Standard Model.
