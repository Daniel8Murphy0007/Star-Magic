...
UQFF Erosion-Buoyancy Co-action:
  E(t) = E₀ · (1 − e^{−t/τ_erosion})   [monotonically increasing: 0→E₀]

  E(t) acts on term1 AND simultaneously:
    term_Ubi, term_F_UBii, term_Ub_i are evaluated with ug1_base (static M)

  Both E(t) and Σ_buoy share the same kernel ug1_base = G·M/r²
```
...
- M(t) = M₀·(1 + Ṁ_factor·e^{-t/τ_SF}) — DYNAMIC (star formation ongoing inside pillars)
...
- E(t) mechanism: UV photons from σ Orionis erode the western edge of the dark lane → photoevaporative flow from a curved surface, not a pillar tip
...
This depends on:
- `Φ_UV`: incident UV photon flux (set by the illuminating star, independent of geometry)
- `ρ₀`: ambient neutral gas density
- `c_s`: sound speed at the ionization front
- `G·M`: gravitational binding (same in all PDR geometries)

The geometry (pillar vs. dark lane) affects the **projected area** and **shadowing correction**, but these are sub-leading-order effects that modify E₀ and τ_erosion — NOT the functional form `(1 − e^{-t/τ})`. Therefore E(t) is **universal in form** across all PDR morphologies.
...
| UV source | Internal OB stars | Trapezium OB | External σ Orionis |
...
The Horsehead Nebula MUGE therefore produces a **monotonically decreasing gravitational confinement** (as E(t) → E₀, the suppression term → 1 − E₀ = 0.9) while the buoyancy oscillations remain constant. This is the **asymmetric erosion-buoyancy regime** unique to static-M PDRs.
...
|----------------|--------|-----------|-------|-----------|
| Elongated pillars | Pillars of Creation (M16) | PILLARS_OF_CREATION.cpp | Yes | E(t) = E₀(1−e^{−t/τ}) |
| Pillar-less dark lane | Horsehead Nebula (B33) | HorseheadNebula.cpp | **No** | E(t) = E₀(1−e^{−t/τ}) |
| (Future) Cometary globule | CG 4, Bok globule | TBD | No | E(t) = E₀(1−e^{−t/τ}) |
| (Future) Elephant trunk | IC 1396A | TBD | Yes | E(t) = E₀(1−e^{−t/τ}) |

**Proven invariant:** E(t) form is the same in all cases. Geometry modifies {E₀, τ_erosion} only.
...
1. **Erosion timescale:** With τ_erosion = 5 Myr, the Horsehead dark lane reaches 63% erosion (E(t) = 0.63·E₀) at t = 5 Myr from now, and 95% erosion at t = 15 Myr. Proper motion measurements of the western ionization front (Pound & Bania 1993; Habart et al. 2005) constrain v_erosion ~ 0.3 km/s → τ_erosion ≈ 4–6 Myr, consistent with τ_erosion = 5 Myr used here.

2. **Gravitational confinement declining:** The MUGE term1 factor `E(t) → E₀ = 0.1` over 15–20 Myr. This predicts the dark lane will complete photoionization dispersal without triggering star formation (consistent with the lack of embedded YSOs in Barnard 33 compared to the adjacent Horsehead PDR layer, Habart et al. 2005).

3. **Buoyancy amplitude independent of erosion:** Since `ug1_base` is constant, the amplitude of Tier-2 and Tier-3 buoyancy oscillations is constant while E(t) grows — creating a **measurable asymmetry** between the buoyancy-oscillation timescale (ω_g⁻¹ ~ 43 Gyr) and the erosion completion timescale (τ_erosion ~ 5 Myr). Both operate simultaneously, confirming co-action on vastly different timescales.

4. **Sgr A* outer frame tidal signature:** Tier-3 uses Sgr A* at ~8.5 kpc (Orion arm). This contributes tidal acceleration `−β_i·ug1_base·ω_g·(M_GC/r_GC)·U_UA ~ O(10⁻²⁸)` m/s² — below current detection limit but predictable at next-generation astrometry precision.
