...
interaction function `I(t) = I0 \cdot exp(-t/t_merger)`.

The modulated gravity `g_merger = g_base \cdot (1 + I(t))` peaks at (1 + I0) ˜ 1.1 times the base MUGE
gravity at the moment of closest approach (t = 0) and relaxes exponentially to unity on the merger
timescale t_merger. Two key characteristic times emerge: `t_half = t_merger \cdot ln(2)` (half-decay
time) and `t_relax = t_merger \cdot ln(I0/0.01)` (the epoch when the modulation drops below 1%).

The base gravity `g_base = (Ug1 + Ug4) \cdot (1 + f_TRZ)` is itself built from the UQFF magnetic dipole
...
```
Ug1    = G\cdotM/r2                    [DPM-seeded-equivalent dipole term]
Ug4    = Ug1\cdot(1-B/B_crit)         [vacuum-field reduction: Ug4 < Ug1 for B > 0]

g_base = (Ug1 + Ug4)\cdot(1 + f_TRZ)
       = Ug1\cdot(2 - B/B_crit)\cdot(1 + f_TRZ)
```

For B « B_crit (galactic fields ~10?1° T): `Ug4 ˜ Ug1`, so `g_base ˜ 2\cdotUg1\cdot(1 + f_TRZ)`.
The TRZ factor f_TRZ = 0.1 adds a 10% triadic resonance contribution, giving `g_base ˜ 2.2\cdotUg1`.
...
**Half-life:** `t_half = t \cdot ln(2)`. For t = 400 Myr: `t_half ˜ 277 Myr`. Half the initial merger
boost is dissipated in ~277 Myr — the timeframe over which the Antennae system's star-burst peaks
and begins to fade.

**1% relaxation:** `t_relax = t \cdot ln(I0/0.01) = 400\cdotln(10) ˜ 920 Myr`. The galaxy pair is
effectively unperturbed after ~1 Gyr, consistent with the dynamical friction timescale for major
mergers.

**Instantaneous merger rate:** `dI/dt = -I0/t \cdot exp(-t/t)` — most rapid change at t = 0 (peak
...
For the default Antennae parameters: integrated boost ˜ `g_base \times 0.1 \times 400 Myr = 40 Myr\cdotg_base`.
...
document.
6. grok_{share\_8d951e12} validation session — merger modulation term (Antennae + HUDF modules).
...
$$p_{\mathrm{DVP}} = 19, \quad n_{\mathrm{channel}} = 14/26$$
