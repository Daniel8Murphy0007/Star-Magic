...
Phonon modulation factor Φ_{1.25THz}(ω) = Φ₀·exp[-(ω-ω_SCm)2/(2Γ2)]·S₂₆([SSq]) with quality factor Q
= ω_SCm/Γ = 6.25 (sharp resonance) and FWHM = 2Γ√(2ln2) ≈ 1.49 THz linewidth. Characterizes the SCm
...
For this system, the local VDS sub-ratio is $0.078$ (deep sub-threshold regime), placing it in the zone where exponential damping dominates. This threshold behavior connects to the PAPER_877 cosmogenesis Stage 1 vacuum density initialization: $\rho_{\rm vac} = \rho_{\rm UA} + \rho_{\rm SCm} = 7.799 \times 10^{-36}$ kg/m3.
...
| DVP prime | $p_k \in$ {2,3,...,113} | $p_{\rm DVP} = 71$ | PASS Super-threshold |
