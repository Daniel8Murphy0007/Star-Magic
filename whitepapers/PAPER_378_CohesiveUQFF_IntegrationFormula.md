...
dominated by the fluid frequency term ($a_{fluid\_freq} = 1.773 \times 10^{-9}$ m/s2) while
...
double compute_{cohesive\_MUGE}(const MUGESystem& sys, const ResonanceParams& res, double alpha_r,
double t) {
    double g_compressed = compute_{compressed\_MUGE}(sys);
    double g_resonance   = compute_{resonance\_MUGE}(sys, res);
