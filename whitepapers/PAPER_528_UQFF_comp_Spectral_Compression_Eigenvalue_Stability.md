---
paper_id: PAPER_528
title: "UQFF_comp Spectral Compression Eigenvalue Stability"
session: 142
date: 2026-03-25
...
This paper presents a UQFF analysis of UQFF_comp Spectral Compression Eigenvalue Stability, deriving
...
|-----------|-------|--------|-------------|
| $\lambda_text{stable}$ | $P/3$ | Stable (our existence) | 2 |
| $\lambda_text{destruct}$ | $2P/3$ | Destructive | 1 |

Key relationships:

$$\lambda_text{destruct} = 2\,\lambda_text{stable}$$
...
|--------------------|-------------------|
| $A_\text{stable}$ fraction (1/3) | $\lambda_text{stable} = P/3$ |
| $D_\text{repel}$ fraction (2/3) | $\lambda_text{destruct} = 2P/3$ |
...
1. **Eigenvalue vs Prob_order sweep:** Plot $\lambda_text{stable}$ and
   $\lambda_text{destruct}$ as functions of $P$ over $[0, 1.5]$ — observe
...
# result['lam_stable']   — \lambda_min = P/3
# result['lam_destruct'] — \lambda_max = 2P/3
