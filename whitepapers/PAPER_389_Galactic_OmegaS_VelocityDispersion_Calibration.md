---
paper_id: PAPER_389
title: "Galactic \omega_s Calibration from Stellar Velocity Dispersion"
session: 106
date: 2025-01-01
...
parameters such as `\omega_s` (system angular frequency) and `\omega_g` (galactic angular velocity).
...
calibration formula for the galactic angular frequency input `\omega_{s\_galactic}`:
...
This is consistent with the canonical `\omega_g = 7.3e-16` rad/s used in the UQFF pipeline
...
The formula directly calibrates `\omega_s` in the resonance MUGE:

```python
def compute_{omega\_s\_galactic}(sigma_{km\_s}: float, R_{bulge\_m}: float) -> float:
    """
    Calibrate galactic angular frequency from observational parameters.
    
    Args:
        sigma_{km\_s}: stellar velocity dispersion in km/s
        R_{bulge\_m}: bulge radius in meters
    
    Returns:
        omega_s: angular frequency in rad/s
    """
    return (sigma_{km\_s} * 1e3) / R_{bulge\_m}
```
...
```
Spectroscopic measurement: \sigma (SDSS/VLT/Keck line broadening)
         ↓
Photometric measurement: R_bulge (HST/ELT effective radius)
         ↓
Formula: \omega_{s\_galactic} = (\sigma \times 103) / R_bulge
         ↓
MUGE resonance input: \omega_s fed into 12-term co-sum
         ↓
PAPER_390 M-\sigma: log(M_BH) calibrates SMBH mass input M
```
...
$$p_{\mathrm{DVP}} = 113, \quad n_{\mathrm{channel}} = 26/26$$
