---
paper_id: PAPER_878
title: "SCm Gaussian Activation B-Field Suppression"
session: 209
date: 2026-04-08
...
SCm Gaussian activation function comparing linear exp(-B/B_crit) vs Gaussian exp[-(B/B_crit)2]
magnetic field suppression in the SCm manifold. Gaussian form matches BCS gap behavior with flatter
response at low B (preserving coherence) and sharper cutoff near B_crit. At B=0.5$\cdot$B_crit: linear
yields 0.607, Gaussian yields 0.779 — 28% higher coherence retention.
...
$$
\begin{aligned}
  & \beta(B) = exp(-B/B_crit)                         [linear] \\
  & A_SCm(B) = exp[-(B/B_crit)2]                  [Gaussian] \\
  & \alpha(B) = w\cdot\beta + (1-w)\cdot A_SCm                      [blended]
\end{aligned}
$$
...
| B/B_crit | beta_linear | `A_{SCm\_Gaussian}` | alpha_blended |
|---|---|---|---|
...
At B = 0.5 B_crit, linear suppression yields 0.607 while Gaussian yields 0.779 — a 28% coherence
advantage. The Gaussian form preserves superconducting coherence across the low-B regime, matching
BCS gap phenomenology. This implies that SCm-mediated gravitational effects persist deeper into
magnetar-strength fields than linear models predict.
...
**Significance:** SCm Gaussian activation replaces the simple exponential B-field suppression with a
BCS-inspired Gaussian that better preserves coherence in moderate magnetic fields — critical for
neutron star and magnetar environments where UQFF buoyancy corrections compete with extreme
B-fields.
...
The SCm activation framework is derived from the **SCm Superconductivity Axiom**: the vacuum
...
The standalone module `scm_{activation\_function}.py` implements this in:

- **Engine 2 (26-State Progression):** BCS-gap at each quantum state n
- **Engine 3 (Cosmogenesis):** Three-assumption framework (DPM, ACP, four U_g forces)
- **Engine 4 (Lagrangian):** SCm-magnetic sector couples to the UQFF variational principle

### Standalone Calculator

```bash
python scm_{activation\_function}.py        # Full report
python scm_{activation\_function}.py —json  # Machine-readable
```
...
- **VDS/DVP/BSH:** PRESENT (BCS-gap with VDS vacuum density, DVP prime encoding, BSH saturation harmonics)
...
This paper maps to **SCm-magnetic sector** of the 9-sector UQFF Lagrangian (see
...
The BSH saturation timescale for this sector is **10^6 yr** (B-field suppression):
...
1. PAPER_840 -- Kozima LENR F_neutron UQFF (SCm suppression context)
2. PAPER_870 -- DPM Extended Periodic Table (f_SCm fraction)
3. PAPER_877 -- Three-Assumption Cosmogenesis (SCm axiom)
4. BCS Theory: Bardeen-Cooper-Schrieffer gap equation
