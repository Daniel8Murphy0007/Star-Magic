---
paper_id: PAPER_082
title: "UQFF-Corrected Black Hole Evaporation Timescales: Stellar Mass Through Primordial Black
Holes"
session: 0
date: 2026-03-07
...
status: production
cvw: "v2.0.0"
tags: [Hawking, vacuum, black-hole, UQFF]
...
**Date:** March 7, 2026  
**Source Data:** validate_{hawking\_temperature}.py (Tests 2, 6), CondensedPhysics.py
simulate_evaporation()  
**Index Slot:** §1.11 Black Hole Physics & Hawking Radiation,  

## Abstract

Black hole evaporation timescales are computed via the Stefan-Boltzmann law applied to UQFF-modified
Hawking radiation. The standard result t_evap = 5120pGM/(?c4) is modified by the T_UQFF/T_H = 0.99
ratio. This changes evaporation rates by a factor of (T_UQFF/T_H)4 = 0.96, extending evaporation
timescales by ~4% for all black holes. The `validate_{hawking\_temperature}.py` evaporation simulation
...
$$t_{\mathrm{evap}}^{\mathrm{GR}} = \frac{5120 \pi G^2 M_0^3}{\hbar c^4}$$

### UQFF-Corrected

$$t_{\mathrm{evap}}^{\mathrm{UQFF}} = \frac{t_{\mathrm{evap}}^{\mathrm{GR}}}{(T_{\mathrm{UQFF}}/T_H)^4} = \frac{t_{\mathrm{evap}}^{\mathrm{GR}}}{0.99^4} = t_{\mathrm{evap}}^{\mathrm{GR}} \times 1.041$$

**UQFF evaporation timescale is 4.1% longer than GR**  black holes are slightly more stable in the
UQFF vacuum.
...
| System | M0 | `t_{evap\_GR}` | `t_{evap\_UQFF}` | Survives Universe |
|--------|-----|-----------|-------------|-------------------|
| Sgr A* | 4$\times$106 M? | 8.7$\times$108 s | 9.1$\times$108 s | ? Yes |
| M87* | 6.5$\times$10? M? | 3.8$\times$10-5 s | 4.0$\times$10-5 s | ? Yes |
| Stellar BH | 10 M? | 2.1$\times$1074 s | 2.2$\times$1074 s | ? Yes |
| Primordial BH | 5.7$\times$10 kg | 4.35$\times$10-7 s = t_U | 4.52$\times$10-7 s | Borderline |
| Primordial BH | 1$\times$10 kg | 2.3$\times$10 s (73 kyr) | 2.4$\times$10 s | ? Evaporated |

The validate_{hawking\_temperature}.py Test 2 confirms:
- Stellar BH (10 M?): `survives_universe = True` ?
- Test 6 simulation: M_initial = 10 kg, 100 steps, mass_{lost\_fraction} computed ?
...
$$\frac{dM}{dt} = -\frac{k_{\mathrm{UQFF}}}{M^2}, \quad k_{\mathrm{UQFF}} = \frac{\hbar c^4 (T_{\mathrm{UQFF}}/T_H)^4}{15360 \pi G^2}$$

With T_UQFF/T_H = 0.99: k_UQFF = 0.96  k_GR

At t = 100 $\times$ 10 s = 10 s:
- M_final – M_initial  (1 - t/t_evap)^{1/3} = 10  (1 - 10/2.4$\times$10)^{1/3}
- M_final  10 $\approx$ 0.583^{1/3}  8.35$\times$10? kg
...
The UQFF modifies the mass loss rate through the vacuum buoyancy correction. During late-stage
evaporation (M ? M_Planck):

$$\frac{dM_{\mathrm{UQFF}}}{dt} = -\frac{k_{\mathrm{UQFF}}}{M^2} + \frac{g_{\mathrm{Buoyant}} \times V_{\mathrm{BH}}}{c^2}$$

The buoyancy term: g_Buoyant – V_BH / c = ?_vac  1055  (4/3)p r_S / c ~ 10?8 kg/s ? negligible vs
the thermal term at all masses above Planck mass.
...
| Primordial threshold mass | 5.7$\times$10 kg | 5.5$\times$10 kg | -3.5% |
| Test 2 | `survives = True` | Confirmed | ? PASS |

*Source: `validate_{hawking\_temperature}`.py Tests 2+6, simulate_evaporation() | $\kappa$ = 0.0005/day | [SSq]
= 0.57*
