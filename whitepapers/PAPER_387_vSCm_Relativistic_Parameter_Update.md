...
**Source:** grok_{share\_cfdcad2f5}.txt, lines ~9600–10122 (main.cpp global constants)  
...
Prior UQFF implementations used a preliminary value `v_SCm = 1\times108 m/s` for the
...
- $\rho_A$ = ambient density (kg/m3), default `\rho_A = 1\times10-23 kg/m3`
- $\kappa$ = decay constant, default `\kappa = 0.0005 day-1`
- $t$ = time elapsed
...
double beta_i = 0.603;             // Buoyancy coupling (\approx0.6 calibrated)
...
The calibrated constant `\kappa=0.0005/day` (from PAPER_341) remains unchanged — the decay
envelope is independent of the velocity amplitude.

The calibrated `\beta_i\approx0.603` (PAPER_375) also remains valid as it governs buoyancy coupling,
