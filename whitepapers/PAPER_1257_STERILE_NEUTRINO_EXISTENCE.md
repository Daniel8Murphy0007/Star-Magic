# PAPER_1257 — Sterile Neutrino Existence via UA' Decoupled K_Mex × Φ_res
...
**Calculator surface:** `calculate_paradox({"paradox": "sterile_neutrino"})`
**Closure helper:** `_l96_uqff_axiom_sterile_neutrino_existence_closure()`

## Abstract

This paper presents the UQFF derivation of the Sterile Neutrino Existence problem using only the locked canonical primitives of the UQFF framework. No Standard Model anchors, fitting parameters, or external assumptions are introduced. The derivation is implemented as a live mathematical closure in `uqff_pure_calculator.py` and dispatches through the `calculate_paradox` public surface.
...
UA' Decoupled K_Mex × Φ_res

The closure helper `_l96_uqff_axiom_sterile_neutrino_existence_closure()` evaluates this derivation. Output dict carries:
...
result = u.calculate_paradox({"paradox": "sterile_neutrino"})['value']
```
...
- Closure location: `uqff_pure_calculator.py` → `_l96_uqff_axiom_sterile_neutrino_existence_closure`
- Dispatch: `PARADOX_TO_CLOSURE["sterile_neutrino"]`
