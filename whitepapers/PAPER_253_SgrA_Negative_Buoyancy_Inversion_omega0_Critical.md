...
|-----------|--------|-------|-------|--------|
| Black hole mass | M_BH | 4.1 × 106 M_sun = 7.956e36 kg | kg | GRAVITY collaboration 2020 |
| Probe radius | r | 6.17e18 | m (~200 ly) | GC thermal region |
...
```
F_rel = k_rel × (E_cm_astro_eff / E_cm_LEP)² = 4.30e33 N
```

F_rel is constant across all systems (independent of ?0). At ?0 = 10?¹², F_rel/F_LENR ˜ 10?7 — negligible. At ?0 = 10?¹5, F_rel/F_LENR ˜ 10?¹³ — still formally small, but its absolute magnitude (4.30e33 N) becomes significant relative to the vacuum-corrected integrand through the quadratic root evaluation.
...
```
F_U_Bi (Sgr A*) ˜ -8.31e211 N   [NEGATIVE — repulsive stabilisation]
```
...
The UQFF F_U_Bi = -8.31e211 N — an enormous repulsive force that, integrated over the GC volume, can drive gas outflow against the gravitational well of the bulge. The magnitude and sign are consistent with a centralised UQFF buoyancy acting as the inflation mechanism for the Fermi Bubbles.
...
- **Fermi Bubble morphology:** The UQFF F_U_Bi = -8.31e211 N predicts bubble inflation timescale: t_bubble = 2 × 25 kpc / v_gas = 2 × 7.7e2° / 106 ˜ 50 Myr — consistent with the Fermi Bubble age estimate of 6–50 Myr (Zubovas & King 2012).
