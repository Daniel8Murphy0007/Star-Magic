...
$$
\begin{aligned}
  & Um,X(arg) = [S_j µ_j(t, ?_vac,[SCm])/r_j] \times (1 - e^{-?t} \cdot cos(pt_n)) \times F_X \\
  & or (single j variant): \\
  & Um,X(arg) = µ(?_vac) \times (1 - e^{-?t}) \times F_X \\
  & where: \\
  & µ_j = magnetic moment density at scale j \\
  & ?_vac,[SCm] = superconductive vacuum density \\
  & ? = exponential decay rate \\
  & t_n = normalized time: t_n = t/t_Hubble\cdot(1+H(z)\cdott0) \\
...
| Um,orbdec(e) | dE/dt = -L, E = -GM$\cdot$µ/(2a) | Orbital decay by GW quadrupole radiation |
