---
paper_id: PAPER_055
title: "NGC 4676 \"The Mice\": UQFF Compressed Gravity Enhancement During Galaxy Merger –
High-Velocity Tidal Bridge and 10 Compression Factor"
session: 0
date: 2026-03-07
author: "Daniel T. Murphy"
status: production
cvw: "v2.0.0"
tags: [merger, galaxy, SCm, UQFF]
...
**Date:** March 7, 2026  
**Validator:** `validate_{all\_models}.py`  NGC4676Model: **4/4 PASS** ?  
**Source Module:** `CondensedPhysics.py` (NGC4676Model), `validate_{all\_models}.py`  
**Index Slot:** §1.7 arXiv Cross-Validation Framework,  

## Abstract

NGC 4676 (A and B), "The Mice," are two colliding spiral galaxies at ~87 Mpc undergoing first
pericenter passage. The UQFF model reveals a 10 enhancement of both g_compressed and R_amplitude
compared to isolated spirals, directly attributable to the violent compression of the inter-galactic
[SCm] medium during the collision. The gravitational field g_grav = 2.9500$\times$10? m/s is the highest
among the interacting-galaxy subset. All 4 tests pass.
...
- g_compressed = **1.0533$\times$10?** (10 larger than the standard 1.0533$\times$10?)
- R_amplitude = **1.1586$\times$10?** (10 larger than the standard 1.1586$\times$10?)

This 10 enhancement is the UQFF signature of a **major merger event**. In the 26-layer compressed
gravity framework:
$$g_{\mathrm{compressed}}^{\mathrm{merger}} = g_{\mathrm{compressed}}^{\mathrm{isolated}} \times \left(1 + \frac{\Delta M_{\mathrm{overlap}}}{M_{\mathrm{total}}}\right)^n$$

where ? M_overlap is the mass in the overlapping region and n  2.3 for head-on collisions. For The
Mice, with ~30% mass overlap during pericenter: (1 + 0.3)^2.3 $\times$ 1.7. The remaining factor ~6 arises
from the [SCm] compression: as the two galactic [SCm] halos merge, the [SCm] density in the
interaction zone spikes, boosting the buoyancy compression.

Combined: 1.7  ~6 $\times$ 10 ? consistent with the observed 10 g_compressed enhancement.
...
- g_grav = **2.9500$\times$10?** m/s (2 that of NGC3372 Carina, 37.5 that of UGC10214)
...
The exponential time decay e^(-?t) modulates BH-mediated forces (Ug4) but not the direct [SCm]
compression. The Mice merger timeline:
...
The UQFF model captures the snapshot at t  -160 Myr (post-pericenter relaxation) where the 10 factor
is appropriate.
...
1. NGC4676 shows a 10 enhancement in UQFF compressed gravity and resonance amplitude, the signature
of a major galaxy merger
2. The 2.95$\times$10? m/s gravitational field is the second-highest in the validation suite (after M42)
3. The factor-10 compression arises from [SCm] halo overlap during pericenter, boosting the buoyancy
compression term
4. UQFF distinguishes minor mergers (Ug3-dominated, one-sided tails) from major mergers
([SCm]-dominated, symmetric double tails)  a key prediction testable with IFU spectroscopy of merger
shock zones

*Validator: `v`alidate_{all\_models}`.py` NGC4676Model 4/4 PASS | $\kappa$ = 0.0005/day | [SSq] = 0.57*
