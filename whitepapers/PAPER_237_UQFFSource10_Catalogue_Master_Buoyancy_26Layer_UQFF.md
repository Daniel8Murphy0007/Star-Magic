---
paper_id: PAPER_237
title: "UQFFSource10 Catalogue — Master Buoyancy Integral F_{U\_Bi\_i} and 26-Layer Triadic g_UQFF"
session: 59
date: 2026-03-01
author: "Daniel T. Murphy"
status: production
cvw: "v2.0.0"
tags: [AGN, dark-energy, F_{U\_Bi\_i}, buoyancy, LENR, UQFF]
sm_anchor: "CVW v2.0.0 — G6 SM Anchor Gate compliant"
---

# PAPER_237: UQFFSource10 Catalogue — Master Buoyancy Integral F_{U\_Bi\_i} and 26-Layer Triadic g_UQFF

**Author:** Daniel T. Murphy
**Framework:** UQFF v4.9 (Star-Magic)
**Session:** 59 (grok_{share\_8d951e12}.txt second-pass — Source10 Text Module)
**Date:** March 2026
**Classification:** Novel UQFF Catalogue — Master Buoyancy Force + 26-Layer Vectorised Triadic
Gravity
...
This paper documents the UQFFSource10 central catalogue module, encoding the complete master buoyancy integral $F_{U\_Bi\_i}$ and 26-layer Triadic UQFF gravity $g_{\mathrm{UQFF}}(r,t)$. The Source10 module serves as the primary reference implementation for all five UQFF force classes: low-energy nuclear reaction (LENR), dark energy expansion, magnetic resonance, relativistic buoyancy, and the 26-layer gravitational hierarchy. Validation example: Eta Carinae yields $F_{U\_Bi\_i} \approx 2.11\times10^{208}$ N. The module also introduces a configurable `scaling_factors` map and `mt19937` RNG batch compute architecture for multi-system catalogue runs.

---

## 1. Physical System

Source10 is a general-purpose **astrophysical catalogue module** (not tied to a single object). The
validation example uses Eta Carinae parameters:
...
## 2. Master Buoyancy Force F_{U\_Bi\_i}

$$\boxed{F_{U\_Bi\_i} = I_{\mathrm{grav}}\cdot x_2 + F_{\mathrm{LENR}} + F_{\mathrm{DE}} + F_{\mathrm{res}} + F_{\mathrm{rel}}}$$

where:
$$I_{\mathrm{grav}} = \frac{G\,M}{r^2}, \quad x_2 = \text{buoyancy balance parameter}$$

### 2.1 LENR Component

$$F_{\mathrm{LENR}} = s_{\mathrm{LENR}}\cdot\left(\rho_f v^2\right)\cdot\left[1 + \frac{E_{\mathrm{act}}}{k_B T}\right]\cdot e^{-t/\tau_{\mathrm{LENR}}}$$

Low-energy nuclear reaction term: kinetic pressure scaled by nuclear activation threshold and
temporal decay.

### 2.2 Dark Energy Expansion Component

$$F_{\mathrm{DE}} = s_{\mathrm{DE}}\cdot\frac{\Lambda c^2}{3}\cdot r$$

Cosmological constant $\Lambda$ produces a radially-growing force term (de Sitter expansion analogy).

### 2.3 Magnetic Resonance Component

$$F_{\mathrm{res}} = s_{\mathrm{res}}\cdot\frac{B^2 V}{2\mu_0}\cdot\frac{\rho_n}{\rho_{\mathrm{ref}}}$$

Magnetic energy density modulated by neutron matter fraction $\rho_n/\rho_{\mathrm{ref}}$.

### 2.4 Relativistic Buoyancy Component

$$F_{\mathrm{rel}} = s_{\mathrm{rel}}\cdot\frac{Mc^2}{r}\cdot(1 + f_{\mathrm{TRZ}})$$

Rest-mass energy divided by radius, enhanced by time-reversal zone factor $f_{\mathrm{TRZ}}$.

---

## 3. 26-Layer Triadic UQFF Gravity

$$\boxed{g_{\mathrm{UQFF}}(r,t) = \sum_{i=1}^{26}\left(U_{g1,i} + U_{g2,i} + U_{g3,i} + U_{g4,i}\right) + \frac{\Lambda c^2}{3} + \frac{\hbar}{\sqrt{\Delta x\,\Delta p}}\cdot\int\psi;d^3x\cdot\frac{2\pi}{t_H}}$$
...
**Cosmological correction:** $\Lambda c^2/3$ (same as $F_{\mathrm{DE}}$ per unit length)
...
- **`scaling_factors` map** — per-system overrides for $s_{\mathrm{LENR}}$, $s_{\mathrm{DE}}$, $s_{\mathrm{res}}$, $s_{\mathrm{rel}}$
...
1. **Unified 5-component master buoyancy** — LENR + DE + resonance + relativistic + base all in one
formula
...
# result['F_{U\_Bi\_i}'] — master buoyancy
# result['g_UQFF']   — 26-layer Triadic gravity
# result['g_{layers\_26}'] — sum of all layer contributions
```
...
- grok_{share\_8d951e12}.txt, Source10 Text Module, lines 5903–6662
