...
The HYDROGEN_{PTOE\_RESONANCE} module is the FIRST module where f_DPM = f_THz = f_{quantum\_orbital} =
**1.0$\times$1015 Hz** — all three resonance channels simultaneously locked to the Lyman-alpha UV
frequency. This **triple Lyman-alpha frequency resonance lock** (freq_{lock\_ratio} = **1.000**)
...
| `f_{quantum\_orbital}` | 1.0$\times$1015 | Hz | Quantum orbital: Lyman-alpha UV |
...
The quantum orbital resonance uses the same pipeline with f_{quantum\_orbital} replacing f_THz:

$$a_{\text{qorb}} = \frac{10 \times f_{\text{qorb}} \times v_{\text{exp}}}{c} \times a_{\text{DPM}}$$

Since f_{quantum\_orbital} = f_THz = 1e15 Hz:
...
| `freq_{lock\_ratio}` | **1.000** | — | **[PAPER_303] FIRST unity lock** |
...
Gamma_{THz\_cache}       = 10.0 * f_THz * v_exp / C_LIGHT;    // 7.298e13
a_{THz\_cache}           = Gamma_{THz\_cache} * a_{DPM\_cache};      // 4.895e10
freq_{lock\_ratio\_cache} = f_THz / f_DPM;                      // 1.000 [P303]
a_{qorb\_cache}          = 10.0 * f_{quantum\_orbital} * v_exp / C_LIGHT * a_{DPM\_cache};
// a_qorb == a_THz (degenerate pair when f_qorb = f_THz) [P303]

WOLFRAM_{TERM\_PTOE\_THZ\_LOCK} = "f_THz/f_DPM=1.000; Gamma_THz=7.298e13; a_THz=a_qorb=4.895e10
[PAPER_303]"
```
...
$$p_{\mathrm{DVP}} = 7, \quad n_{\mathrm{channel}} = 18/26$$
