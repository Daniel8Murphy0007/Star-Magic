...
Full unified Lagrangian for the E(t) framework. L_{E(t)} = E_net(t)$\cdot$V_filament$\cdot$S26([SSq]) combining
both expansion and erosion branches with buoyancy sign-flipping and cosmological $\Lambda$ link. The
...
$$
\begin{aligned}
  & L_{E(t)} = E_net(t)\cdot V\cdot S26 \\
  & E_net = E0\cdot\exp(\kappa t+[SSq]t/26)\cdot S26\cdot(2R-1) \\
  & \Lambda = 8\pi G\cdot0.692\cdot\rho_crit/c2
\end{aligned}
$$
...
$$p_{\mathrm{DVP}} = 37, \quad n_{\mathrm{channel}} = 23/26$$
...
| DVP prime | $p_k \in$ {2,3,...,113} | $p_{\mathrm{DVP}} = 37$ | PASS Super-threshold |
