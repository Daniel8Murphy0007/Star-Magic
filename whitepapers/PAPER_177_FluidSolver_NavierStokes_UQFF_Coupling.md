...
### 7. integrate_{fluids\_for\_muge}() — System-Level Integration

```cpp
void simulate_{fluids\_for\_muge}(MUGESystem& sys, int N_steps=100) {
    FluidSolver fs;
    for int step=0; step<N_steps; step++:
        double g = compute_{resonance\_MUGE}(sys);  // UQFF gravity
        fs.add_{jet\_force}();
        fs.step(g);
    fs.print_{velocity\_field}();
}
```

This function is called from `populate_{simulation\_entities}()` for each
...
| `add_{jet\_force}` | SCm expulsion ignition event |
