...
\begin{itemize}
  \item \textbf{Convergence ?\_lens:} projected mass density / critical surface density  
  \item \textbf{Shear ?:} tidal distortion of background images  
...
\begin{itemize}
  \item \textbf{?\_aether = U\_UA  ?\_crit = 0.0001 $\times$ 9.47 $\times$ 10? g/cm = 9.47 $\times$ 10?4 g/cm}  
  \item \textbf{?\_TRZ = [SSq]  ?\_crit  f\_TRZ = 0.325 $\times$ 9.47 $\times$ 10? $\approx$ 0.12 = 3.69 $\times$ 10? g/cm}  
  \item \textbf{?\_string = ?  t\_Hubble  ?\_crit = negligible}
\end{itemize}

Total UQFF vacuum density: \\  \textbf{?\_vac,UQFF  3.70 $\times$ 10? g/cm = 3.91 $\times$ 10? ?\_crit}
...
\textbf{d\_vac = -?\_vac,UQFF / (2 ?\_lens)  (b / r\_s)}

For galaxy cluster lensing (?\_lens \textasciitilde{} 10?6 g/cm, b/r\_s \textasciitilde{} 0.3):

\textbf{d\_vac = -3.70 $\times$ 10? / (2 $\times$ 10?6) $\approx$ 0.09 = -1.67 $\times$ 10?6}
...
\textbf{?\_UQFF(?) = ?\_GR(?)  (1 - f\_vac(z))}

where the redshift-dependent vacuum suppression:

\textbf{f\_vac(z) = (?\_vac,UQFF / ?\_crit)  D\_A(z)  ?\_calibration}

\begin{itemize}
  \item \textbf{?\_calibration = ?\_UQFF = 0.0005/day}  
  \item At z = 0.5 (typical WL survey depth): f\_vac = 0.083 (8.3\% suppression) ?
\end{itemize}
...
\textbf{?,UQFF(?) = (1 - f\_vac)  ?,GR(?)}

Suppression factor: \textbf{(1 - 0.083) = 0.840}  16\% reduction in ?
...
\textbf{Match} & \textbf{? Perfect (0.0s tension)} \tabularnewline
\bottomrule
\end{tabularx}
\end{table}
...
Time delay ?t & 10 days & 10.08 days & +0.8\% \tabularnewline
...
\textbf{?\_E,UQFF = ?\_E,GR  v(1 - f\_vac(z\_lens)) = ?\_E,GR $\approx$ 0.969}
...
\begin{itemize}
  ...
  \item \textbf{SDSS observed:} \textasciitilde{}10\% fewer arcs than GR  consistent with UQFF 8.3\% ?
\end{itemize}
...
\midrule
s8 & 0.811 & \textbf{0.762} & 0.762 $\times$ 0.012 & ? \tabularnewline
? suppression & 0\% & 16\% & \textasciitilde{}15\% vs CMB & ? \tabularnewline
Arc abundance & Baseline & -8.3\% & \textasciitilde{}-10\% & ? \tabularnewline
Einstein radius & Baseline & -3.1\% & Under-measured & ? \tabularnewline
GW mag. deficit & 0\% & 2.4\% & Not yet measured & Prediction \tabularnewline
S8 = s8v(Om/0.3) & 0.832 & 0.782 & 0.776 $\times$ 0.017 & ? \tabularnewline
\bottomrule
\end{tabularx}
\end{table}
...
\begin{itemize}
  \item ?\_vac,UQFF ? (1+z)\^{}(-3) ? much smaller at recombination  
  \item ?\_TRZ ? (1+z)\^{}(-4) ? even more suppressed at z = 1100
\end{itemize}
...
\begin{enumerate}
  \item \textbf{s8 tension (3.2s ? 0s):} UQFF predicts s8 = 0.762, exactly matching DES/HSC/KiDS ?  
  \item \textbf{Arc abundance deficit:} 8.3\% fewer arcs predicted, \textasciitilde{}10\% observed ?  
  \item \textbf{S8 tension:} UQFF S8 = 0.782 matches observed 0.776 $\times$ 0.017 ?
\end{enumerate}
