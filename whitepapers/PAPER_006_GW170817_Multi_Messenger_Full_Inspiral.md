...
each messenger. UQFF predicts a 66.7% strain reduction (h_UQFF = 1.80 × 10-22 vs h_GR = 5.42 ×
10-22) and reduces the matched-filter SNR from 32.4 to 10.8, while GW propagation speed |Δc/c| < 3 ×
10-15 holds in both GR and UQFF. The event establishes tight UQFF parameter bounds: string factor =
0.37, TRZ = 0.90, and SCm ≈ 1.0 for B_NS << B_crit.
...
|---------|------------|--------------|------------------|
| GW (LIGO) | Peak strain h | 5.42 × 10-22 | 1.80 × 10-22 (−66.7%) |
| GW (LIGO) | Matched-filter SNR | 32.4 | 10.8 (still detectable) |
| GW speed | \|Δc/c\| | < 3 × 10-15 | Same (UQFF preserves GW speed) |
...
| NS B-field | B_NS | 108 G | B/B_crit = 2.27 × 10-10 (SCm inactive) |
...
| SCm | 1.000000 | B_NS = 1 × 104 T vs B_crit = 4.4 × 1013 T → inactive |
...
SNR_UQFF = 1.08e1, |Δc/c| < 3e-15

**UQFF Modified Strain:**

**h_UQFF = F_combined × h_GR = 0.333 × 5.4176 × 10-22 = 1.8041 × 10-22**
...
| UQFF from h_obs | 3.33 × 10-23 | Below observed by 3× |
...
efficiency ε = F_combined × (detector coupling), not directly by F_combined alone.
...
The kilonova produces ejecta M_ej ≈ 0.04–0.05 MM_sun of r-process material. UQFF does not modify the EM
sector in this scenario. However, the reduced GW-radiated energy under UQFF (ΔE_UQFF < ΔE_GR)
...
| Observable | GR | UQFF |
|------------|----|----|
| Peak GR strain | 5.4176 × 10-22 | — |
| Peak UQFF strain | — | 1.8041 × 10-22 |
...
0.333), shrinking the detection volume by ~27×. This is an implicit prediction for O4/O5 BNS rate
estimates.
...
3. **SCm inactive** for typical NS B-fields, confirming B_crit = 4.4×1013 T threshold
