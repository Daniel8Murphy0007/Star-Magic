...
HiRes & $10^{-8}$ $\times$ 10 eV & GZK feature at 6 $\times$ 10? eV \tabularnewline
...
\begin{enumerate}
  ...
  \item CMB photopion production (GZK process, E > 5 $\times$ 10? eV)
...
\textbf{G\_aether(E) = ?  (E / E\_ref)\^{}$\kappa$\_aether}

where:

\begin{itemize}
  \item \textbf{$\kappa$ = 0.0005/day = 5.79 $\times$ 10?? $\mathrm{s}^{-1}$}
...
\begin{itemize}
  \item \textbf{G\_aether = 0.0005  (10/$10^{-8}$)\^{}0.37 = 0.0005 $\times$ 8.51 = 4.26 $\times$ 10?/day}
...
\begin{itemize}
  \item \textbf{s0 = 3.2 $\times$ 10?6 cm} (TRZ cross-section at peak)
...
\begin{longtable}{@{}lll@{}}
\toprule
Energy (eV) & s\_TRZ (cm) & ?\_TRZ (Mpc) \tabularnewline
\midrule
\endhead
$10^{-8}$ & 2.1 $\times$ 10?7 & 2,900 \tabularnewline
8 $\times$ $10^{-8}$ & 3.2 $\times$ 10?6 & 190 \tabularnewline
10? & 2.8 $\times$ 10?6 & 217 \tabularnewline
10 & 4.1 $\times$ 10?8 & 148,000 \tabularnewline
\bottomrule
\end{longtable}
\end{center}
...
\begin{itemize}
  \item f\_string(10 eV) \textasciitilde{} $10^{-58}$ ? effectively zero
\end{itemize}
...
\textbf{t\_GZK(E) ? exp(E\_GZK / E),  E\_GZK = 5 $\times$ 10? eV}

UQFF modifies the effective energy loss length:

\textbf{L\_eff(E) = [1/L\_GZK(E) + 1/L\_aether(E) + 1/L\_TRZ(E)]?}
...
\midrule
10? & 1,000 & 570 & 217 & 148 \tabularnewline
5 $\times$ 10? & 100 & 340 & 8,200 & 82 \tabularnewline
...
\begin{itemize}
  \item Spectral softening ?? \textasciitilde{} 0.3 due to TRZ scattering peak
  \item Detectable by Auger with 10 years of data
\end{itemize}

\textbf{Feature 2  GZK + Aether Combined Suppression at E \textasciitilde{} 5 $\times$ 10? eV:}
...
\textbf{Feature 3  Aether Pile-up at E \textasciitilde{} 2 $\times$ 10? eV:}

\begin{itemize}
  \item Slight spectral hardening ?? \textasciitilde{} -0.15 from aether energy redistribution
...
\toprule
Energy Range & GR Index ? & UQFF Index ? & Difference ?? \tabularnewline
\midrule
$10^{-8}$ $\times$ 3  $10^{-8}$ eV & 3.30 & 3.28 & -0.02 \tabularnewline
3 $\times$ $10^{-8}$ $\times$ 8  $10^{-8}$ eV & 2.60 & 2.58 & -0.02 \tabularnewline
8 $\times$ $10^{-8}$ $\times$ 10? eV & 2.60 & 2.90 & +0.30 (TRZ break) \tabularnewline
10?  5 $\times$ 10? eV & 2.60 & 2.62 & +0.02 \tabularnewline
> 5 $\times$ 10? eV & 5.00 & 5.19 & +0.19 (aether) \tabularnewline
\bottomrule
\end{tabularx}
\end{table}
...
\textbf{s\_TRZ,aniso = s\_TRZ,iso  (1 + A\_TRZ cos?)}
...
\begin{itemize}
  ...
  \item Centaurus A sits at a TRZ filament node ? reduced scattering in that direction
\end{itemize}
...
\textbf{G\_aether(E, Z) = ?  Z\^{}(1/3)  (E/A / E\_ref)\^{}$\kappa$\_aether}
...
\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X@{}}
\toprule
Energy (eV) & GR ?lnA? & UQFF ?lnA? & Observed (Auger Xmax) \tabularnewline
\midrule
$10^{-8}$ & 1.5 & 1.6 & 1.5\S{}2.0 \tabularnewline
3 $\times$ $10^{-8}$ & 2.0 & 2.2 & 2.0\S{}2.5 \tabularnewline
10? & 2.5 & 2.8 & 2.5\S{}3.0 \tabularnewline
...
\midrule
GZK cutoff energy & 5 $\times$ 10? eV & 4.8 $\times$ 10? eV & \textasciitilde{}5 $\times$ 10? eV & ? \tabularnewline
GZK cutoff sharpness & Standard & 3.7\% sharper & Slightly sharp & ? \tabularnewline
Cen A anisotropy & Requires B < 1 nG & B \textasciitilde{} 5 nG sufficient & 5s excess & ? \tabularnewline
Secondary break at 8 EeV & Not predicted & ?? \textasciitilde{} +0.3 & Tentative (2s) & ? \tabularnewline
?lnA? at 10? eV & 2.5 & 2.8 & 2.5\S{}3.0 & ? \tabularnewline
Proton fraction > 10 eV & \textasciitilde{}30\% & \textasciitilde{}22\% & \textasciitilde{}2030\% & ? \tabularnewline
\bottomrule
\end{tabularx}
\end{table}
...
GRAND (200,000 km) & Detect aether pile-up at 2 $\times$ 10? eV & 20302035 \tabularnewline
...
\begin{enumerate}
  \item \textbf{GZK sharpness:} Aether drag adds 3.7\% additional suppression above 10 eV ?
...
All results derived from pre-calibrated constants $\kappa$ = 0.0005/day and [SSq] = 0.57. A new prediction -- TRZ secondary spectral break at E \textasciitilde{} 8 $\times$ $10^{-8}$ eV with ?? \textasciitilde{} +0.3  is testable by AugerPrime within 23 years.
