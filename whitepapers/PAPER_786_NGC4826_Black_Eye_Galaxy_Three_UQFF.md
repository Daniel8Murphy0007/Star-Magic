---
paper_id: PAPER_786
title: "NGC 4826 Black Eye Galaxy — Three-UQFF Warped Inner Disk"
session: 181
date: 2026-01-01
author: "Daniel T. Murphy"
status: production
cvw: "v2.0.0"
tags: [galaxy, merger, Three-UQFF, buoyancy, UQFF]
...
NGC 4826, the "Black Eye Galaxy" (also "Evil Eye Galaxy"), is a remarkable spiral ~17 million
light-years distant (z $\approx$ 0.0014) in Coma Berenices. Its distinctive feature is a massive dark dust
band across the nucleus and an extraordinary dynamical anomaly: the inner disk gas rotates in the
opposite direction to the outer disk. This counter-rotation indicates a past merger that supplied
the inner disk with retrograde gas. Using the Three-UQFF framework (simultaneous computation of
Compressed, Resonant, and Buoyancy UQFF modes), NGC 4826's counter-rotating dynamics yield g_primary
$\approx$ 1.053$\times$10-3 m/s2 across all three modes.

---

## 1. Introduction

NGC 4826's counter-rotating inner/outer disk is one of the most remarkable kinematic structures in
the nearby universe. The inner disk (r < 1 kpc) rotates retrograde relative to the outer stellar
disk, the result of a gas-rich minor merger ~1 Gyr ago. The shear layer between the two rotating
components generates localized star formation and turbulence. The Three-UQFF framework applies all
three UQFF operational modes simultaneously, testing the robustness of the gravitational field
...
$$
g_comp = (G \times M) / r2 \times (1 + H(z)\times t) \times (1 + M_sf) \times (1 + f_TRZ) + \text{a\_EM\_comp}
$$

### Mode 2: Resonant UQFF
$$
g_res = g_comp \times (1 + \kappa \times [SSq]) \times R_freq
$$
...
\begin{aligned}
  & g_buoy = g_comp + a_Ubi \\
  & where a_Ubi = \rho_UA \times V \times g_local / m_p
\end{aligned}
$$
...
|-----------|--------|-------|--------|
| Galaxy mass | M | 1.0$\times$1011 MM_sun = 1.989$\times$1041 kg | NED |
| Effective radius | r | 2.83$\times$1020 m (~30 kly) | NED |
| Counter-rotation gap | — | 1 kpc shear zone | Buta+1994 |
| SFR | — | 0.3 MM_sun/yr | Low, disturbed |
...
$$
\begin{aligned}
  & g_grav = 6.6743e-11 \times 1.989e41 / (2.83e20)2 = 1.657e-10 m/s2 \\
  & H(z)\times t = 2.269e-18 \times 1.578e17 = 0.358; factor = 1.358 \\
  & factor_sf = 1.015; factor_TRZ = 1.04 \\
  & \text{g\_grav\_total} = 1.657e-10 \times 1.358 \times 1.015 \times 1.04 = 2.386e-10 m/s2 \\
  & a_EM = 1.053e-3 m/s2 \\
...
$$
\begin{aligned}
  & R_freq = 1 + \kappa \times [SSq] = 1 + 0.0005 \times 0.57 = 1.000285 \\
  & g_res = g_comp \times 1.000285 = 1.053e-3 m/s2
\end{aligned}
$$
...
\begin{aligned}
  ...
  & g_primary (Compressed) = 1.053e-3 m/s2
\end{aligned}
$$
...
The Three-UQFF framework applied to NGC 4826 demonstrates that the counter-rotating inner disk —
despite its extraordinary kinematic anomaly — produces the same result across all three UQFF modes.
The $\kappa$ $\times$ [SSq] resonance correction is only 2.85$\times$10-4, negligibly small at standard mass/velocity
parameters. The buoyancy correction is effectively zero at galactic density contrasts. This
confirms: for standard mass/velocity galaxies, Three-UQFF converges to the single-UQFF result, with
mode distinctions only becoming significant at extreme parameters. NGC 4826's infamous
counter-rotation does not alter the UQFF electromagnetic ground state.

---

## 6. Conclusions

Three-UQFF applied to NGC 4826 yields g_primary $\approx$ 1.053$\times$10-3 m/s2 across all three modes
(compressed, resonant, buoyancy). The counter-rotating disk kinematics confirm that UQFF mode
convergence holds for standard galaxy-scale systems regardless of unusual kinematic configurations.
