...
# Ni-62 example
Z, N, A = 28, 34, 62
E_{bind\_per\_nuc} = 8.7945e6 * eV  # J (per nucleon)
E_bind = E_{bind\_per\_nuc} * A
dp = delta_pair(Z, N)
shell = S_shell(Z, N)
...
# H (Z=1)
Z, N, A = 1, 0, 1; E_{bind\_H} = 0.0  # hydrogen: no nuclear binding
print(f"H   (Z=1):  A_res = {k_A * 1 * (1/A_H) * (1+0.0):.3f} V")

# Pb-208 (doubly magic)
Z, N, A = 82, 126, 208
E_{bind\_Pb} = 7.8675e6 * eV * A
shell_Pb = S_shell(Z, N)
A_{res\_Pb} = k_A * 82 * (208 / A_H) * (1 + 1.0)
...
A_{res\_isl} = k_A * Z_isl * (A_isl / A_H) * (1 + 1.0)
print(f"Z=120 Island: A_res = {A_res_isl:.0f} V, S_shell = {S_isl:.1f}")
print(f"Ratio Z=120/Pb = {A_res_isl/A_{res\_Pb}:.1f}x")
```
