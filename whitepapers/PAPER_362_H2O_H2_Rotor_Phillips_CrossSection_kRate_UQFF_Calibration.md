...
The H2O/H2 rotational excitation cross-section from the Phillips (1994) energy-dependent formula s(E) = a·(1 - e^{-bE}) is connected to the UQFF framework. The calibrated parameters a = 15.28 Å² (saturation cross-section) and b = 0.00387 cm (energy slope parameter) reproduce s(300 cm?¹) = 10.50 Å². The UQFF rate constant k_rate = s·v_therm links the molecular cross-section to the vacuum buoyancy scale via the U_UA coupling established in PAPER_341.
...
At T = 300 K, µ = reduced mass of H2O/H2 system:
...
|----------|---------|-------|
| a | Saturation s | 15.28 Å² |
| b | Energy slope | 0.00387 cm |
| s(300 cm?¹) | Phillips formula | 10.50 Å² |
| v_therm | v(8kT/pµ) | ~3600 m/s |
...
This paper connects UQFF to laboratory molecular physics for the first time via an experimentally verified cross-section formula. The s(300 cm?¹) = 10.50 Å² value links PAPER_339 (t_rot coupling, s_CS = 10.50 Å²) and PAPER_341 (f_Ub = U_UA·s) into a consistent molecular-scale UQFF chain. The k_rate = s·v_therm formula has direct applications in astrochemical modeling of molecular clouds where H2O/H2 rotational collisions drive the thermal balance (e.g., protostellar envelopes, cometary comae).

---

## 5. Deduplication Note

- **vs. PAPER_339 (UmRotor):** PAPER_339 connected t_rot to s_CS = 10.50 Å²; PAPER_362 derives s(E) analytically via the Phillips formula.
