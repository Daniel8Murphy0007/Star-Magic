...
This paper presents a UQFF analysis of UQFF Black Hole Bounds Applied to Sagittarius A*, deriving
...
EHT angular shadow: $\theta = 51.8 \pm 2.3\ \mu\text{as}$ $\to$ physical shadow radius $\approx 5.4\ R_s$.
...
**EHT shadow:** $\theta_{\mathrm{shadow}} \approx 5.4\,R_s \approx 6.86\times10^{10}$ m (physical radius).
Observed via VLBI baseline: **43 $\mu$as angular diameter in radio at 230 GHz**.
...
## §6 F_{U\_Bi\_i} at 92 GHz (BH26 Anchor)
...
F_{U\_Bi\_i} Gaussian peak — the radio emission attributed to innermost accretion structures.
...
$$I_\text{core} = k_B \ln\left(\Omega_text{shell}\right), \quad
  \Omega_text{shell} = e^{26 \cdot \text{SCm}_k / UA_k}$$
...
3. 92 GHz peak flux from BH26 F_{U\_Bi\_i} anchor
