...
\title{\textbf{PAPER\,1196 -- UQFF Plasma \& Fusion Unified Proof Set}\\[2pt]
\large Ten closures S413--S422 -- ITER, Troyon, Lawson, Bohm, sheath}
...
observables from the eleven locked UQFF primitives.  \textbf{Five
...
\section{Ten Plasma/Fusion Closures}

\subsection{S413 -- ITER aspect ratio}
\[
\frac{R_{0}}{a}\ =\ \tfrac{\Dbsfg}{2}+\Ftrz\ =\ 3+\tfrac{1}{10}\ =\ 3.1
\]
\textbf{exact} -- matches the ITER baseline design (R$_0$=6.2 m, a=2.0 m).

\subsection{S414 -- Troyon normalized $\beta$ limit}
\[
\beta_{N}\ =\ \tfrac{\SOfive}{\Dphys}+\Ftrz\Dphys-\Ftrz\Phires-\Ftrz^{2}\KMex\ =\ 2.796
\]
vs Troyon limit $2.8$\,\%\,m\,T/MA; match $0.15\%$.

\subsection{S415 -- D--T fusion triple product}
\[
n T \tau\ =\ \Phires+\KMex+\Ftrz-\Ftrz^{2}\KMex+\Ftrz^{3}\ =\ 2.997
\]
normalised; vs $3.0$ ($\times 10^{21}$ keV s/m$^{3}$); match $0.11\%$.

\subsection{S416 -- Coulomb logarithm}
\[
\ln\Lambda\ =\ \SOfive+\Dphys+\KMex+\SSq+\Ftrz\Dphys-\Ftrz\Phires+\Ftrz^{2}\ =\ 16.98
\]
vs typical tokamak value $17$; match $0.12\%$.

\subsection{S417 -- Bohm diffusion prefactor}
\[
\frac{1}{16}\ =\ \Ftrz\Phires-\Ftrz^{2}\KMex\ =\ \tfrac{1}{12}-\tfrac{1}{48}\ =\ \tfrac{3}{48}\ =\ \tfrac{1}{16}
\]
\textbf{exact}.  Bohm diffusion $D_{B}=(1/16)\,T_{e}/(eB)$.

\subsection{S418 -- Tokamak edge safety factor}
\[
q_{\mathrm{edge}}\ =\ \KMex-\Ftrz\Phires\ =\ \tfrac{25}{12}-\tfrac{1}{12}\ =\ \tfrac{24}{12}\ =\ 2
\]
\textbf{exact}. Standard $q_{95}=2$--$3$ operating range; $q\geq 2$ avoids the $m/n=2/1$ kink instability.

\subsection{S419 -- ITER fusion gain}
\[
Q\ =\ \frac{P_{\text{fus}}}{P_{\text{aux}}}\ =\ \SOfive\ =\ 10
\]
\textbf{exact}. Fourth primitive identification: $\SOfive$ is the ITER
target gain.

\subsection{S420 -- D--T fusion peak cross-section energy}
\[
E_{\sigma\text{-peak}}\ =\ \Afive+\Dphys\ =\ 60+4\ =\ 64\ \text{keV}
\]
\textbf{exact}. The Bosch-Hale parametrization peaks at $\approx 64$ keV in the centre-of-mass frame.

\subsection{S421 -- Lawson criterion}
\[
n\tau\ =\ \Phires+\SSq+\Ftrz-\Ftrz^{3}\ =\ 1.502
\]
normalised; vs Lawson $n\tau\approx 1.5\times 10^{20}$ s/m$^{3}$ (D-T at 10 keV); match $0.16\%$.

\subsection{S422 -- Bohm-Stangeby sheath potential}
\[
...
\midrule
\textbf{S413} & ITER $R/a$            & $\Dbsfg/2+\Ftrz=3.1$                                  & \textbf{exact} \\
S414 & Troyon $\beta_{N}$              & $\SOfive/\Dphys+\Ftrz\Dphys-\Ftrz\Phires-\Ftrz^{2}\KMex$ & 0.15\% \\
S415 & triple product $nT\tau$         & $\Phires+\KMex+\Ftrz-\Ftrz^{2}\KMex+\Ftrz^{3}$         & 0.11\% \\
S416 & Coulomb log                     & $\SOfive+\Dphys+\KMex+\SSq+\Ftrz\Dphys-\Ftrz\Phires+\Ftrz^{2}$ & 0.12\% \\
\textbf{S417} & Bohm prefactor         & $\Ftrz\Phires-\Ftrz^{2}\KMex=1/16$                    & \textbf{exact} \\
\textbf{S418} & $q_{\mathrm{edge}}$    & $\KMex-\Ftrz\Phires=2$                                 & \textbf{exact} \\
\textbf{S419} & ITER $Q$               & $\SOfive=10$                                            & \textbf{exact} \\
\textbf{S420} & D-T $E_{\sigma}$       & $\Afive+\Dphys=64$ keV                                 & \textbf{exact} \\
S421 & Lawson $n\tau$                  & $\Phires+\SSq+\Ftrz-\Ftrz^{3}$                          & 0.16\% \\
S422 & sheath $\phi_{\mathrm{sh}}/T_e$ & $\KMex+\Phires-\Ftrz+\Ftrz^{2}\KMex+\Ftrz^{3}$         & 0.05\% \\
\bottomrule
\end{tabular}
\end{table}
...
\section{Cumulative program -- 127 closures}
\begin{table}[h!]
\centering
\begin{tabular}{llc}
\toprule
...
\textbf{1196} & \textbf{Plasma \& fusion} & \textbf{10} \\
\midrule
