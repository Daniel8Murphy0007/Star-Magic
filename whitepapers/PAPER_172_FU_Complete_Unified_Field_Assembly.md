---
paper_id: PAPER_172
title: "F_U Complete Unified Field Assembly"
session: 0
date: 2025-01-01
author: "Daniel T. Murphy"
status: production
cvw: "v2.0.0"
tags: [quasar, AGN, buoyancy, black-hole, UQFF]
                  ...
                  eta, g_{mu\_nu})
{
    ...
    A_{mu\_nu} = compute_{A\_mu\_nu}(t, tn, g_{mu\_nu}, eta, T_s00=Ts_surface)

    return (Ug1+Ug2+Ug3+Ug4) + (Ubi1+Ubi2+Ubi3+Ubi4) + Um + trace(A_{mu\_nu})
}
```
...
$$
F_U = \sum_{i=1}^{4} \left[ k_i \cdot Ug_i(r,t) - \beta_i \cdot Ug_i \cdot \frac{\Omega_g
M_{bh}}{d_g}(1+\varepsilon_{sw}\rho_{sw})[UA]\cos(\pi t_n) \right]
+ \sum_j \left[ \frac{\mu_j}{r_j}(1-e^{-\gamma t \cos(\pi t_n)}) \hat{\phi}_j \right] P_{SCm}
E_{react}
    + \text{tr}(g_{\mu\nu} + \eta T_s^{\mu\nu})
$$
...
$$
U_{b\_i}(r,t) = \rho_{\text{vac}}\,V_{\text{eff}}\,g_{\text{loc}}\cdot[SSq]\,e^{-\kappa t}, \quad
[SSq]=0.57,\;\kappa=5.0\times10^{-4}\,\text{day}^{-1}
$$
...
`simulate_{quasar\_jet}()` in main.cpp runs a temporal evolution for SgrA*:
