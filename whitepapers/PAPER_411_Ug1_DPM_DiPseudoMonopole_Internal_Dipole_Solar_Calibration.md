...
```cpp
double compute_{mu\_s}(double t, double Bs, double omega_c, double Rs, double SCm_contrib = 1e3) {
...
double compute_Ug1(const CelestialBody& body, double r, double t, double tn,
    ...
    double mu_s = compute_{mu\_s}(t, body.Bs_avg, body.omega_c, body.Rs);
    double grad_{Ms\_r} = compute_{grad\_Ms\_r}(body.Ms, body.Rs);
    double defect = 1.0 + delta_def * std::sin(0.001 * t);
    return k1 * mu_s * grad_{Ms\_r} * std::exp(-alpha * t) * std::cos(PI * tn) * defect;
}
```
...
```python
def test_{Ug1\_solar\_calibration}():
    ...
    grad_{Ms\_r} = G * Ms / Rs**2                 # \approx 274
    defect = 1.0
    expected = k1 * mu_s * grad_{Ms\_r} * math.exp(-alpha * t) * math.cos(math.pi * tn) * defect
    # \approx 1.39e26
