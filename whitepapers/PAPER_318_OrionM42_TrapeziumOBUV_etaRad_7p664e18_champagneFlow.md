...
### Surface Area at r
$$A_{\mathrm{trap}} = 4\pi r^2 = 4\pi \times (1.18\times10^{17})^2 = 1.748\times10^{35}\ \text{m}^2$$

### Radiation Pressure
$$P_{\mathrm{rad}} = \frac{L_{\mathrm{trap}}}{A_{\mathrm{trap}} \cdot c} = \frac{7.656\times10^{31}}{1.748\times10^{35} \times 2.998\times10^8} = 1.461\times10^{-12}\ \text{Pa}$$

### Radiation Pressure Acceleration (PAPER_318 Key Result)
$$a_{\mathrm{rad}} = \frac{P_{\mathrm{rad}}}{\rho_{\mathrm{fluid}}} = \frac{1.461\times10^{-12}}{10^{-20}} = \boxed{1.461\times10^{8}\ \text{m/s}^2}$$

### UV Radiation–Gravity Dominance Ratio
$$\eta_{\mathrm{rad}} = \frac{a_{\mathrm{rad}}}{g_{\mathrm{base}}} = \frac{1.461\times10^8}{1.907\times10^{-11}} = \boxed{7.664\times10^{18}}$$

### Champagne Flow Condition
$$\eta_{\mathrm{rad}} = 7.664\times10^{18} \gg 1 \implies \text{ionized gas escapes freely (champagne flow)}$$
...
The MHD Alfvén term in ORION_{UQFF\_MODULE}.cpp shares the same WOLFRAM_{TERM\_ORION\_TRAPEZIUM\_RAD} macro
...
```cpp
#define WOLFRAM_{TERM\_ORION\_TRAPEZIUM\_RAD}(val) (val)
// rad = L_trap/(4pi*r^2*c*rho)   [PAPER_318 Trapezium UV; eta_rad=7.664e18]
// em_MHD = B^2/(2*mu0*rho*r)     [Alfven companion; same macro]
```

*Series first: FIRST UQFF sub-pc compact HII Trapezium OB cluster UV radiation dominance parameter.
Establishes the SECOND entry in the UQFF OB-cluster radiation class (after Lagoon Nebula Session 87,
PAPER_306).*
...
$$p_{\mathrm{DVP}} = 67, \quad n_{\mathrm{channel}} = 7/26$$
