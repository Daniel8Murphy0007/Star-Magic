...
Ug1 = k1 \times \mu_s(t) \times (G\timesMs/Rs2) \times exp(-a\timest) \times cos(p\timest?) \times (1 + d_def\timessin(0.001\timest))

where:
  \mu_s(t)  = (Bs + 0.4\timessin(?_c\timest) + 1e3) \times Rs3   [stellar DPM moment]
  d_def   = 0.01                                   [defect amplitude]
  a       = 0.001                                  [temporal decay rate]
  t?      = negative time index (p-cycle modulator)
  k1      = 1.5
The defect factor `(1 + d_def\timessin(0.001\timest))` introduces quantum surface 
...
Ug2 = k2 \times (QA + QUA) \times Ms/r2 \times S(r-Rb) \times (1 + d_sw\timesv_sw) \times H_SCm \times E_react
...
where:
  ...
  E_react = (?_SCm \times v_SCm2/?_A) \times exp(-?t)
  k2      = 1.2
--- 
...
where:
  Bj(t)    = 1e-3 + 0.4\timessin(?_c\timest) + SCm  [string field, time-varying]
  ?_s(t)   = ?_s - 0.4e-6\timessin(?_c\timest)       [modulated spin rate]
...
Ug4 = k4 \times ?_v \times C_conc \times Mbh/dg \times exp(-a\timest) \times cos(p\timest?) \times (1 + f_feedback)
...
Um = S? [ \mu?(t)/r? \times (1 - exp(-?\timest\timescos(p\timest?))) \times f^ ] \times PSCm \times E_react
