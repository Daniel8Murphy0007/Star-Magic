...
```
Ug1 = k1 × μ_s(t) × (G×Ms/Rs²) × exp(-α×t) × cos(π×tₙ) × (1 + δ_def×sin(0.001×t))

where:
  μ_s(t)  = (Bs + 0.4×sin(ω_c×t) + 1e3) × Rs³   [stellar DPM moment]
  δ_def   = 0.01                                   [defect amplitude]
  α       = 0.001                                  [temporal decay rate]
  tₙ      = negative time index (π-cycle modulator)
  k1      = 1.5
```

The defect factor `(1 + δ_def×sin(0.001×t))` introduces quantum surface
...
```
Ug2 = k2 × (QA + QUA) × Ms/r² × S(r−Rb) × (1 + δ_sw×v_sw) × H_SCm × E_react
...
where:
  ...
  δ_sw    = 0.01    [solar wind coupling coefficient]
...
```
Ug3 = k3 × Bj(t) × cos(ω_s(t) × t × π) × Pcore × E_react

where:
  Bj(t)    = 1e-3 + 0.4×sin(ω_c×t) + SCm  [string field, time-varying]
  ω_s(t)   = ω_s − 0.4e-6×sin(ω_c×t)       [modulated spin rate]
...
rotation periodicity and its π-cycle quantum gating.
...
```
Ug4 = k4 × ρ_v × C_conc × Mbh/dg × exp(−α×t) × cos(π×tₙ) × (1 + f_feedback)
...
```
Um = Σⱼ [ μⱼ(t)/rⱼ × (1 − exp(−γ×t×cos(π×tₙ))) × φ̂ ] × PSCm × E_react

where:
  μⱼ(t)   = Bj(t) × Rs³                  [time-varying string moment]
  rⱼ       = string path distance          [field parameter]
  φ̂        = unit vector (infinity curve)  [disk plane direction]
  PSCm     = body.PSCm                     [SCm pressure]
  γ        = 0.00005                       [reciprocation decay; near-zero]
  N_strings = 1e9                          [total string count]
```

The near-zero γ ensures minimal energy loss, consistent with SCm
superconductivity.
