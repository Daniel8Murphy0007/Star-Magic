...
check("32 public calculate_* functions (16 prior + 6 BUCKET 0 + 1 B + 1 C + 1 D + 7 buckets E-K)",
...
check("BUCKET B: calculate_paradox() returns full inventory of 8 paradoxes",
      full['value']['inventory']['total_paradoxes'] == 8)
check("BUCKET B: paradox inventory has all 8 named paradoxes",
      set(full['value']['inventory']['paradox_names']) == {
...
# Each individual paradox returns a finite value via _paradox_proof routing
for name in ['yang_mills_mass_gap', 'riemann_hypothesis', 'bsd_conjecture',
    ...
    check(f"BUCKET B: calculate_paradox({name!r}) returns finite value + NOT REPLACEMENT",
          ok, "actual=%s" % one)
...
# Suite structure
check("BUCKET C: cosmology suite has 27 observables (was 18 + 9 w(z) live)",
      cos['total_count'] == 27,
