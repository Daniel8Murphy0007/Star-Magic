...
try:
    ...
    _urs, _r5 = None, {}
check("R5 BACKEND — uqff_registry_status imports; calculate_status_report() computes program statistics LIVE from UNIFIED_REGISTRY.csv + UNIFIED_REGISTRY_GRAPH.csv + uqff_registry_primitives (nothing hand-maintained)", _urs is not None and _r5.get("registry_rows") == 2544 and _r5.get("graph_edges") == 656)
...
check("XGEO RECLASSIFICATION — registry drains R1 debt: zero SYMBOLIC_PENDING_R1 rows remain; all 1,044 former GAP rows carry XGEO campaign statuses per Daniel's 2026-07-24 ruling (full derivation campaign; each cell = independent re-derivation of the observable in a geometry that does not own it — no mechanical fill, no value-coincidence numerology)", _xst.get("SYMBOLIC_PENDING_R1", 0) == 0 and _xst.get("CROSS_GEOMETRY_PENDING", 0) + _xst.get("XGEO_ROUTED_IDENTITY", 0) == 1044)
...
check("XGEO REMAINING (post-batch-1 accounting, superseded by batch 2) — 71 qcalcgeom deferred + 42 d26 unreachable + 111 opaque-formula cells were CROSS_GEOMETRY_PENDING after batch 1", _xst.get("CROSS_GEOMETRY_PENDING", 0) + _xst.get("XGEO_ROUTED_IDENTITY", 0) == 1044)
