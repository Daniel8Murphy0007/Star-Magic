...
check("R5 RESULTS TABLE — 14 derived constants composed LIVE from the 9 independent primitives at build time (table cannot drift from code): 7 EXACT (mu_0, kappa, B_crit, k_spring, lambda_vac, D_BSFG, K_MEX) + honest residuals G 0.075 pct, c 0.10 pct, k_B 0.0011 pct, hbar 0.027 pct, T_SCm 0.007 pct, Lambda 0.90 pct", _r5.get("derived_constants_live") == 14 and _r5.get("derived_constants_exact") == 7 and _r5.get("independent_primitives") == 9)
...
check("R5 ARTIFACTS — UNIFIED_REGISTRY_STATUS_REPORT.md + UNIFIED_REGISTRY_RESULTS_TABLE.csv (header + 14 constant rows) + UNIFIED_REGISTRY_RESULTS_TABLE.md emitted idempotently (identical sha across consecutive runs, no timestamps)", _os.path.exists("UNIFIED_REGISTRY_STATUS_REPORT.md") and _os.path.exists("UNIFIED_REGISTRY_RESULTS_TABLE.md") and sum(1 for _ in open("UNIFIED_REGISTRY_RESULTS_TABLE.csv", encoding="utf-8")) == 15)
