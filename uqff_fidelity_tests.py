...
check("R3 MODULE — uqff_registry_primitives.py single-source-of-truth exists; c DUAL EXPOSURE per Daniel sec-6.2 verdict (C_UQFF_DERIVED = 2.99497e8 live PAPER_592 form + C_OBSERVED 2.998e8; consumers migrate per-class on instruction)", _rp.C_OBSERVED == 2.998e8 and abs(_rp.C_UQFF_DERIVED - 2.99497e8) < 1e4)
...
check("R5 WORST RESIDUAL IS THE HUBBLE TENSION — H0 grid (2*SO_5+2)*F_TRZ^19 vs local observed anchor: 3.08 pct residual retained deliberately per PAPER_2125 (the residual is not an error; it is the physics; PAPER_1156 1/12 EXACT tilt closure addresses it)", _r5 and 3.0 < _r5.get("worst_residual_pct", 0.0) < 3.2)
