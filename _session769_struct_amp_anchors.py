"""
SESSION 769 -- complete z=0.61 BAO/RSD + structure-amplitude anchors.

  (a) LXXVIII  D_M(z=0.61)       ~ 2283 Mpc       [r_drag shell]
  (b) LXXIX    f*sigma_8(z=0.61) ~ 0.436          [direct]
  (c) LXXX     sigma_8(z=0)      ~ 0.811          [direct]
  (d) LXXXI    Omega_m * h^2     ~ 0.1430         [direct]
...
print("="*80); print("SESSION 769 -- D_M(0.61) LXXVIII; f*s8(0.61) LXXIX; sig8(0) LXXX; Om*h2 LXXXI"); print("="*80)
...
# ---------------------------------------------------------------------------
banner("TRACK (a) -- Class LXXVIII: D_M(z=0.61) = 2283 Mpc (BOSS Lya cross)")
DM061_obs = 2283.0
DM061_pred, DM061_lab, DM061_err = report_shell("D_M(0.61)", DM061_obs, r_drag, "r_drag")

# ---------------------------------------------------------------------------
banner("TRACK (b) -- Class LXXIX: f*sigma_8(z=0.61) = 0.436 (RSD growth amplitude)")
fs8_obs = 0.436
fs8_pred, fs8_lab, fs8_err = report_direct("f*sigma_8(0.61)", fs8_obs)

# ---------------------------------------------------------------------------
banner("TRACK (c) -- Class LXXX: sigma_8(z=0) = 0.811 (RSD/Planck anchor)")
s8_obs = 0.811
s8_pred, s8_lab, s8_err = report_direct("sigma_8(0)", s8_obs)

# ---------------------------------------------------------------------------
banner("TRACK (d) -- Class LXXXI: Omega_m * h^2 = 0.1430 (physical matter density)")
omh2_obs = 0.1430
omh2_pred, omh2_lab, omh2_err = report_direct("Omega_m*h^2", omh2_obs)
...
print()
st_a = write_ledger("classLXXVIII_D_M_z061_session769",   DM061_pred,  DM061_obs,  DM061_err)
st_b = write_ledger("classLXXIX_fsigma8_z061_session769", fs8_pred,    fs8_obs,    fs8_err)
st_c = write_ledger("classLXXX_sigma8_z0_session769",     s8_pred,     s8_obs,     s8_err)
st_d = write_ledger("classLXXXI_Om_h2_session769",        omh2_pred,   omh2_obs,   omh2_err)

print()
print("-"*80); print("DECISION GATE"); print("-"*80)
print(f"  LXXVIII  D_M(0.61)        pred={DM061_pred:.4f}  err={DM061_err:+.4f}%  ({st_a})")
print(f"  LXXIX    f*sigma_8(0.61)  pred={fs8_pred:.4f}    err={fs8_err:+.4f}%  ({st_b})")
print(f"  LXXX     sigma_8(0)       pred={s8_pred:.4f}     err={s8_err:+.4f}%  ({st_c})")
print(f"  LXXXI    Omega_m*h^2      pred={omh2_pred:.4f}    err={omh2_err:+.4f}%  ({st_d})")

artifact = os.path.join(ROOT, "_session769_result.json")
with open(artifact, "w", encoding="utf-8") as fh:
    json.dump({
        "session":769,
        "anchors":{"r_drag_LIV_Mpc":r_drag},
        "tracks":{
            "LXXVIII_D_M_z061":{"obs":DM061_obs,"pred":DM061_pred,"err_pct":DM061_err,"label":DM061_lab,"status":st_a},
            "LXXIX_fsigma8_z061":{"obs":fs8_obs,"pred":fs8_pred,"err_pct":fs8_err,"label":fs8_lab,"status":st_b},
            "LXXX_sigma8_z0":{"obs":s8_obs,"pred":s8_pred,"err_pct":s8_err,"label":s8_lab,"status":st_c},
            "LXXXI_Om_h2":{"obs":omh2_obs,"pred":omh2_pred,"err_pct":omh2_err,"label":omh2_lab,"status":st_d},
        },
        "cvw":"v2.0.0",
        "sm_anchor":"CVW v2.0.0 -- G1 + G3 + G6 + G7 SM Anchor Gate compliant",
    }, fh, indent=2)
