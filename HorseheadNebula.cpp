 ...
 *              upgraded to UQFF 2.0 + 3-tier buoyancy pipeline on March 16, 2026).
 * 
 ...
 *          erosion E(t) [simultaneous co-action with buoyancy — E(t) unique to dark nebula erosion physics],
 *          UQFF Ug components with f_TRZ, Lambda, quantum uncertainty, scaled EM with [UA],
 *          fluid dynamics, oscillatory waves, DM/density perturbations, stellar wind feedback,
 *          plus 3-tier UQFF buoyancy pipeline (CP3/PAPER_198): term_Ubi (Tier-1 static half-gravity),
 *          term_F_UBii (Tier-2 dynamic compact cosine over static M), term_Ub_i (Tier-3 CP1 outer-frame
 *          via Sgr A* ~8.5 kpc from Horsehead Nebula, Orion). 10→13-term MUGE.
 *          UQFF 2.0: logging_enabled, dynamic_params, setEnableLogging, exportState, cross_validate<>.
 ...
 * Date: October 08, 2025 (UQFF 2.0 upgrade: March 16, 2026)
...
private:
    ...
    // External gravitational body: Sgr A* (~8.5 kpc from Horsehead Nebula, Orion)
    double M_GC;            // Sgr A* mass ~4e6 M_sun (kg)
    double r_GC;            // Distance Horsehead to GC ~8.5 kpc in meters
...
public:
    ...
    void initializeDefaults() {
        ...
        r_GC    = 2.623e20;      // Horsehead Nebula to GC ~8.5 kpc in meters
    ...
    double compute_g_Horsehead(double t) const {
        ...
        // ── UQFF buoyancy pipeline (CP3/PAPER_198 canonical) ─────────────────────────
        // Note: static M; E(t) erosion co-present with buoyancy (both act simultaneously)
        // Tier-1: term_Ubi = 0.5 * ug1_base (CP3/PAPER_198 static half-gravity buoyancy)
        double term_Ubi = 0.5 * ug1_base;
        // Tier-2: term_F_UBii — PAPER_198 dynamic compact, cosine modulation over static M
        double term_F_UBii = -beta_i * ug1_base * omega_g * (M / r) * U_UA * cos(M_PI * t);
        // Tier-3: term_Ub_i — CP1 outer-frame via Sgr A* (~8.5 kpc from Horsehead Nebula, Orion)
        double term_Ub_i = -beta_i * ug1_base * omega_g * (M_GC / r_GC) * U_UA * cos(M_PI * t);
        // FU diagnostic (not summed into result)
        double FU_diag = -(term2 + term_Ubi) * ug1_base;

        // Total g_Horsehead (13 terms: 10 original + 3 UQFF buoyancy tiers)
        double result = term1 + term2 + term3 + term4 + term_q + term_fluid
                      + term_osc + term_DM + term_wind
                      + term_Ubi + term_F_UBii + term_Ub_i;
        if (logging_enabled)
            std::cout << "[LOG] compute_g_Horsehead(t=" << t << ") = " << result
                      << "  [t1=" << term1 << " t2=" << term2 << " t3=" << term3
                      << " t4=" << term4 << " tq=" << term_q << " tf=" << term_fluid
                      << " tosc=" << term_osc << " tdm=" << term_DM
                      << " twind=" << term_wind << " Et=" << Et
    ...
    void printParameters(std::ostream& os = std::cout) const {
        ...
        os << "M_GC: " << M_GC << " (Sgr A* ext body), r_GC: " << r_GC << " (~8.5 kpc)" << std::endl;
    }
    ...
    void exportState(const std::string& filename = "HorseheadNebula_state.txt") const {
        std::ofstream ofs(filename);
        if (!ofs) {
            std::cerr << "Error: Cannot open '" << filename << "' for state export." << std::endl;
            return;
        }
        ofs << "# HorseheadNebula (UQFF 2.0) state export\n";
    ...
    double cross_validate(const OtherModuleT& /*other*/, double t_seconds = 0.0) const {
        double g_here  = compute_g_Horsehead(t_seconds);
        double g_other = 0.0;
