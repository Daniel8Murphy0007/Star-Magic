...
%           had been propagating an unmoored "5970 GeV" value through 610
...
\verb|return 5970.0|, with no matching derivation chain in the cited
source PAPER\_1005 (whose internal formula
$\Delta_{\text{YM}} = \Lambda_{\text{QCD}}\cdot e^{-1/(\alpha_s N_c)}\cdot S_{26}^{(3)}$
evaluates to $\sim 10^{24}\,\mathrm{GeV}$, not 5970, when the
$[\mathrm{UA}]$ suppression is omitted). The $5970\,\mathrm{GeV}$ value
...
closure; the prior $5970\,\mathrm{GeV}$ value is not the framework's
...
\begin{enumerate}
  \item \textbf{Numerical agreement with lattice QCD.} Both UQFF
        ...
        on the SU(3) glueball mass; the prior $5970\,\mathrm{GeV}$
