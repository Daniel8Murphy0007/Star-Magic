"""Session 273 raw derivative-function dump.

For every entry in _PA_S273_CLOSURE_REGISTRY whose form() is a real callable
(not None), print:
  - S_id
  - paper_tag
  - label
...
def find_source_line(s_id: str) -> str:
    """Return the verbatim source line for this S_id from the registry."""
    needle = f'"{s_id}":'
    for line in SRC:
        if needle in line and "1086" in line or needle in line and "1087" in line:
            return line.strip()
    for line in SRC:
        if needle in line:
            return line.strip()
...
def extract_form_text(src_line: str) -> str:
    """Pull the form= argument (lambda or None) from the registry tuple text."""
    # tuple text is: ("paper_tag", "label", <form>, predicted, observed, "unit"),
    # we want the 3rd comma-separated field at the top level of the tuple.
    m = re.search(r'\("[^"]+",\s*"[^"]+",\s*(.+?),\s*[\-0-9eE\.+]', src_line)
    if not m:
...
def main() -> None:
    ...
    print(f"Session 273 transcribed lambdas: {len(transcribed)} / {len(reg)} entries")
