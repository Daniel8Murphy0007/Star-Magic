...
// conflicts with source10's own struct definitions
...
public:
    ...
    void runSimulation(double t_start, double t_end, int steps, Func computeFunc) {
        if (enable_logging) {
            std::cout << "[UQFFModule10] Running simulation: t=" << t_start << " to " << t_end << " (" << steps << " steps)" << std::endl;
        }
        ...
        for (int i = 0; i <= steps; ++i) {
            ...
            if (enable_logging) {
                std::cout << "[UQFFModule10] t=" << t << ": g=" << result << std::endl;
            }
        }
    }
...
int main() {
    ...
    double test_g = g_values.empty() ? 9.81 : g_values[0];
    ...
    DualMethodValidator validator("source10_dual_physics.log");
    
    // Validate first system
    if (!system_names.empty()) {
        const auto& sys = module.getSystems().begin()->second;
        
        UQFFDualPhysics::CelestialBody body(sys.name, sys.M, sys.r * 0.01, sys.B0);
        UQFFDualPhysics::MUGESystem muge(sys.name, sys.M, sys.r);
        muge.B0 = sys.B0;
        muge.omega = sys.omega0;
        muge.T = sys.T;
        
        auto result = validator.validate(body, muge, sys.t, sys.theta_deg * M_PI / 180.0);
        result.print();
    }
