"""
gen_muge_horsehead.py — Generator for HorseheadNebula.h
Module 14: Horsehead Nebula (Barnard 33).
Unique physics: Decaying erosion E(t)=E_0*exp(-t/tau_erosion)
                applied as (1-E(t)) on BOTH term1 AND Ug (same pattern as Pillars).
                Distinct parameters: M=1000 Msun, r=2.5 ly, tau_erosion=5 Myr.

Run:  python gen_muge_horsehead.py
Output: HorseheadNebula.h
"""
import os

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
OUTPUT_FILE = os.path.join(SCRIPT_DIR, "HorseheadNebula.h")


def get_content():
    return r"""/**
 * ================================================================================================
 * Header: HorseheadNebula.h
 *
 * Description: C++ Module for Horsehead Nebula (Barnard 33) Class (Module 14)
 *              UQFF simulations — dark nebula pillar with photoerosion dynamics.
 *
 * Unique Terms:
 *   - Decaying erosion: E(t) = E_0 * exp(-t / tau_erosion)
 *   - Applied as (1-E(t)) to BOTH term1 AND Ug
 *   - Pillar mass loss by UV photoionization from nearby sigma Orionis cluster
 *
 * Key Parameters:
 *   M=1000 M_sun, r=2.5 ly, E_0=0.1, tau_erosion=5 Myr
 *
 * Author: Encoded by Grok (xAI), based on Daniel T. Murphy's UQFF manuscript.
 * Date: October 08, 2025
 * Copyright: Daniel T. Murphy
 * ================================================================================================
 */

#ifndef HORSEHEAD_NEBULA_H
#define HORSEHEAD_NEBULA_H

#include <iostream>
#include <cmath>
#include <iomanip>
#include <string>

namespace UQFF {

class HorseheadNebula {
private:
    double G, M0, r;
    double B, B_crit;
    double Lambda, c_light, H0;
    double q_charge, gas_v, f_TRZ;
    double E_0, tau_erosion;
    double rho_fluid;
    double rho_vac_UA, rho_vac_SCm, scale_EM, proton_mass;
    double hbar;
    double t_Hubble, t_Hubble_gyr;
    double delta_x, delta_p, integral_psi;
    double A_osc, k_osc, omega_osc, x_pos;
    double M_DM_factor, delta_rho_over_rho;
    double ug1_base;

public:
    HorseheadNebula() { initializeDefaults(); }
    ~HorseheadNebula() {}

    void initializeDefaults() {
        G = 6.6743e-11;
        double M_sun = 1.989e30;
        M0 = 1000.0 * M_sun;
        double ly = 9.461e15;
        r = 2.5 * ly;
        B = 2.0e-10;
        B_crit = 1.0e11;
        Lambda = 1.1e-52;
        c_light = 3.0e8;
        H0 = 2.184e-18;
        q_charge = 1.602e-19;
        gas_v = 5.0e4;
        f_TRZ = 0.1;
        E_0 = 0.1;
        tau_erosion = 5.0e6 * 3.156e7;   // 5 Myr
        rho_fluid = 1.0e-20;
        rho_vac_UA = 7.09e-36;
        rho_vac_SCm = 7.09e-37;
        scale_EM = 1.0e-12;
        proton_mass = 1.673e-27;
        hbar = 1.0546e-34;
        t_Hubble = 13.8e9 * 3.156e7;
        t_Hubble_gyr = 13.8;
        delta_x = 1.0e-10;
        delta_p = hbar / delta_x;
        integral_psi = 1.0;
        A_osc = 1.0e-10;
        k_osc = 1.0 / r;
        omega_osc = 2.0 * M_PI / (r / c_light);
        x_pos = r;
        M_DM_factor = 0.05;
        delta_rho_over_rho = 1.0e-5;
        updateCache();
    }

    void updateCache() { ug1_base = (G * M0) / (r * r); }

    // Decaying erosion: E(t) = E_0*exp(-t/tau_erosion)
    double E_t(double t) const { return E_0 * std::exp(-t / tau_erosion); }

    bool setVariable(const std::string& varName, double v) {
        if (varName == "M0")         { M0 = v; }
        else if (varName == "r")     { r = v; }
        else if (varName == "E_0")   { E_0 = v; }
        else if (varName == "tau_erosion") { tau_erosion = v; }
        else if (varName == "B")     { B = v; }
        else if (varName == "H0")    { H0 = v; }
        else if (varName == "f_TRZ") { f_TRZ = v; }
        else if (varName == "rho_fluid") { rho_fluid = v; }
        else { std::cerr << "Unknown variable '" << varName << "'.\n"; return false; }
        updateCache();
        return true;
    }

    double getVariable(const std::string& varName) const {
        if (varName == "M0") return M0;
        if (varName == "r")  return r;
        if (varName == "E_0") return E_0;
        if (varName == "tau_erosion") return tau_erosion;
        std::cerr << "Unknown '" << varName << "'.\n";
        return 0.0;
    }

    double compute_Ug(double Et) const {
        // DPM-emergent: mu_s x grad(M_s/r) (not Newtonian GM/r^2)\ndouble ug1 = (G * M0) / (r * r);
        double corr_B = 1.0 - B / B_crit;
        return (ug1 + ug1 * corr_B) * (1.0 + f_TRZ) * (1.0 - Et);
    }

    double compute_V() const { return (4.0 / 3.0) * M_PI * r * r * r; }

    double compute_g_Horsehead(double t) const {
        if (t < 0.0) { std::cerr << "Error: t must be non-negative.\n"; return 0.0; }

        double Et = E_t(t);
        double ug1_t = (G * M0) / (r * r);

        // Term 1 with (1-E) erosion correction
        double corr_H = 1.0 + H0 * t;
        double corr_B = 1.0 - B / B_crit;
        double term1 = ug1_t * corr_H * corr_B * (1.0 - Et);

        // UQFF Ug with (1-E)
        double term2 = compute_Ug(Et);

        // Lambda
        double term3 = (Lambda * c_light * c_light) / 3.0;

        // EM
        double cross_vB = gas_v * B;
        double em_base = (q_charge * cross_vB) / proton_mass;
        double corr_UA = 1.0 + (rho_vac_UA / rho_vac_SCm);
        double term4 = em_base * corr_UA * scale_EM;

        // Quantum
        double sqrt_unc = std::sqrt(delta_x * delta_p);
        double term_q = (hbar / sqrt_unc) * integral_psi * (2.0 * M_PI / t_Hubble);

        // Fluid
        double V = compute_V();
        double term_fluid = (rho_fluid * V * ug1_t) / M0;

        // Oscillatory
        double term_osc = 2.0 * A_osc * std::cos(k_osc * x_pos) * std::cos(omega_osc * t)
                        + (2.0 * M_PI / t_Hubble_gyr) * A_osc * std::cos(k_osc * x_pos - omega_osc * t);

        // DM
        double M_dm = M0 * M_DM_factor;
        double term_DM = ((M0 + M_dm) * (delta_rho_over_rho + 3.0 * G * M0 / (r * r * r))) / M0;

        return term1 + term2 + term3 + term4 + term_q + term_fluid + term_osc + term_DM;
    }

    void printParameters(std::ostream& os = std::cout) const {
        os << std::fixed << std::setprecision(6);
        os << "Horsehead Nebula Parameters:\n";
        os << "  M0=" << M0 << "  r=" << r << "\n";
        os << "  E_0=" << E_0 << "  tau_erosion=" << tau_erosion << "\n";
    }

    double exampleAt5Myr() const { return compute_g_Horsehead(5.0e6 * 3.156e7); }
};

}  // namespace UQFF

#endif  // HORSEHEAD_NEBULA_H
"""


def main():
    print("gen_muge_horsehead.py — Generating HorseheadNebula.h ...")
    content = get_content()
    with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
        f.write(content)
    print(f"  Written: {OUTPUT_FILE}  ({len(content.splitlines())} lines)")


if __name__ == "__main__":
    main()
