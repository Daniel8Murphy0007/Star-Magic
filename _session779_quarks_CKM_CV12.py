"""
SESSION 779 -- SM-sector first-gen + CKM + CV-12 (the "19 anchor").

  (a) CXVIII  m_d/m_u (down-to-up quark mass ratio)        ~ 2.04
  (b) CXIX    Cabibbo lambda_W (Wolfenstein CKM parameter) ~ 0.22500
  (c) CXX     m_top / M_Z (top quark to Z boson mass)      ~ 1.894
  (d) CXXI    CV-12 architectural identity:  SO5 + N_ch = 10 + 9 = 19

  CV-12 atom-factorization:  SO5 + N_ch = 10 + 9 = 19   EXACT
  This identifies the architectural origin of the integer "19" that recurs in
  three independent sectors:
    - LXI  z_BAO = 19/50               (cosmology, EXACT pure-primitive)
    - CX   sin^2 theta_W ~ 3^4*19/(2^9*13)  (electroweak)
    - CXVI a_mu ~ 27*19/(2^6*5^4*11)       (QED radiative)
  CV-12 grounds 19 in the (SO5 algebra + channel) dimensional sum.
...
print("="*80)
print("SESSION 779 -- m_d/m_u CXVIII; lambda_W CXIX; m_t/M_Z CXX; CV-12 CXXI")
print("="*80); print()
...
# ---------------------------------------------------------------------------
banner("TRACK (a) -- Class CXVIII: m_d/m_u = 2.04 (down-to-up quark mass ratio)")
mdu_obs = 2.04
mdu_pred, mdu_lab, mdu_err = report_direct("m_d/m_u", mdu_obs)

# ---------------------------------------------------------------------------
banner("TRACK (b) -- Class CXIX: lambda_W = 0.22500 (Cabibbo / Wolfenstein CKM)")
lam_obs = 0.22500
lam_pred, lam_lab, lam_err = report_direct("lambda_W", lam_obs)

# ---------------------------------------------------------------------------
banner("TRACK (c) -- Class CXX: m_top/M_Z = 1.894 (top quark to Z boson mass)")
mtz_obs = 1.894
mtz_pred, mtz_lab, mtz_err = report_direct("m_top/M_Z", mtz_obs)

# ---------------------------------------------------------------------------
banner("TRACK (d) -- Class CXXI: CV-12 architectural identity SO5 + N_ch = 19")
lhs = SO5 + N_ch                   # 10 + 9
rhs = F(19)
print(f"  SO5           = 10 = {float(SO5):.4f}")
print(f"  N_ch          = 9  = {float(N_ch):.4f}")
print(f"  SO5+N_ch      = 10+9 = {lhs}  =  {float(lhs):.10f}")
print(f"  19            = {rhs}  =  {float(rhs):.10f}")
print(f"  Identity check: LHS == RHS ?  {lhs == rhs}")
cv12_pred = float(lhs); cv12_obs = float(rhs); cv12_err = (cv12_pred-cv12_obs)/cv12_obs*100.0
print(f"  err = {cv12_err:+.6f}%  ->  EXACT atom-factorization identity")
...
print()
st_a = write_ledger("classCXVIII_md_mu_session779",                  mdu_pred, mdu_obs, mdu_err)
st_b = write_ledger("classCXIX_lambda_W_session779",                 lam_pred, lam_obs, lam_err)
st_c = write_ledger("classCXX_mtop_MZ_session779",                   mtz_pred, mtz_obs, mtz_err)
st_d = write_ledger("classCXXI_CV12_SO5_plus_Nch_session779",        cv12_pred, cv12_obs, cv12_err)

print()
print("-"*80); print("DECISION GATE"); print("-"*80)
print(f"  CXVIII m_d/m_u          pred={mdu_pred:.5f}   err={mdu_err:+.4f}%  ({st_a})")
print(f"  CXIX   lambda_W         pred={lam_pred:.5f}   err={lam_err:+.4f}%  ({st_b})")
print(f"  CXX    m_t/M_Z          pred={mtz_pred:.5f}   err={mtz_err:+.4f}%  ({st_c})")
print(f"  CXXI   CV-12 atom-fact  pred={cv12_pred:.4f}    err={cv12_err:+.4f}%  ({st_d})")

artifact = os.path.join(ROOT, "_session779_result.json")
with open(artifact, "w", encoding="utf-8") as fh:
    json.dump({
        "session":779,
        "tracks":{
            "CXVIII_md_mu":   {"obs":mdu_obs,"pred":mdu_pred,"err_pct":mdu_err,"label":mdu_lab,"status":st_a},
            "CXIX_lambda_W":  {"obs":lam_obs,"pred":lam_pred,"err_pct":lam_err,"label":lam_lab,"status":st_b},
            "CXX_mtop_MZ":    {"obs":mtz_obs,"pred":mtz_pred,"err_pct":mtz_err,"label":mtz_lab,"status":st_c},
            "CXXI_CV12_SO5_Nch":{"obs":cv12_obs,"pred":cv12_pred,"err_pct":cv12_err,
                                "label":"SO5 + N_ch = 10 + 9 = 19",
                                "status":st_d},
        },
        "cvw":"v2.0.0",
        "sm_anchor":"CVW v2.0.0 -- G1 + G3 + G6 + G7 SM Anchor Gate compliant",
