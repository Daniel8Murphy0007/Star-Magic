...
Unique physics:
  - B(t)=B0*exp(-t/tau_B) decaying magnetic field
  - F(t)=F0*exp(-t/tau_fil) decaying filament factor, applied as (1+F) on term1 AND Ug
  - Cooling flow: term_cool = rho_cool*v_cool^2 / rho_fluid
  - Central BH: g_BH = G*M_BH / r_BH^2
  - H(z): full Hubble correction H0*sqrt(0.3*(1+z)^3+0.7)

Run:  python gen_muge_ngc1275.py
Output: NGC1275.h
"""
import os

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
OUTPUT_FILE = os.path.join(SCRIPT_DIR, "NGC1275.h")
...
def main():
    print("gen_muge_ngc1275.py — Generating NGC1275.h ...")
    content = get_content()
