...
private:
    ...
    // External gravitational body: R136 cluster (Tarantula/30 Dor core, ~650 ly from NGC 2014)
    double M_R136;          // R136 cluster mass ~1e5 M_sun (kg)
    double r_R136;          // Distance to R136, 650 ly in meters
...
public:
    ...
    bool setVariable(const std::string& varName, double newValue) {
        ...
        else if (varName == "M_R136") { M_R136 = newValue; }
        else if (varName == "r_R136") { r_R136 = newValue; }
        else {
    ...
    double getVariable(const std::string& varName) const {
        ...
        else if (varName == "M_R136") return M_R136;
        else if (varName == "r_R136") return r_R136;
        else {
            std::cerr << "Error: Unknown variable '" << varName << "'." << std::endl;
            return 0.0;
        }
    ...
    double compute_g_Starbirth(double t) const {
        ...
        // CP1 Tier-3: outer-frame Ub_i via R136 cluster (external body ~650 ly)
        double term_Ub_i = -beta_i * ug1_t * omega_g * (M_R136 / r_R136) * U_UA * cos(M_PI * t);
    ...
    void printParameters(std::ostream& os = std::cout) const {
        ...
        os << "M_R136: " << M_R136 << " (R136 ext body), r_R136: " << r_R136 << std::endl;
    }
