...
#include <QFormLayout>      // Form layout manager - two-column label/field layout
...
#include <QProcessEnvironment> // Process environment for Python/venv execution
...
#include "calculator_advanced/include/calculator_widget.h"  // Tab 22 - Advanced Calculator with focused Qt-only backend
...
#include "ipc/physics_service.h" // Phase 2: Physics Backend Service Mode
...
QString resolvePythonExecutableForBridge() {
...
QProcessEnvironment buildPythonProcessEnvironmentForBridge() {
    QProcessEnvironment env = QProcessEnvironment::systemEnvironment();
    const QString appDir = QCoreApplication::applicationDirPath();
    const QStringList venvCandidates = {
        QDir(appDir).filePath(".venv"),
        QDir(appDir).filePath("../.venv")
    };

    for (const QString& venvPath : venvCandidates) {
        if (QDir(venvPath).exists()) {
            env.insert("VIRTUAL_ENV", venvPath);
            env.insert("PATH", QDir::toNativeSeparators(QDir(venvPath).filePath("Scripts")) + ";" + env.value("PATH"));
...
QString resolveProjectScriptPath(const QString& scriptName) {
    const QString appDir = QCoreApplication::applicationDirPath();
    const QStringList candidates = {
        QDir(appDir).filePath(scriptName),
        QDir(appDir).filePath("../" + scriptName),
        QDir(appDir).filePath("../../" + scriptName)
    };
...
QJsonObject runAPIFetchBridgeRequest(
    ...
    const QString trimmedObjectName = objectName.trimmed();
    if (trimmedObjectName.isEmpty()) {
        if (errorMessage) {
            *errorMessage = "Target object name is required for APIFetch.";
        }
        return {};
    }

    const QString scriptPath = resolveProjectScriptPath("APIFetch.py");
    if (scriptPath.isEmpty()) {
        if (errorMessage) {
            *errorMessage = "APIFetch.py was not found in the application or project directories.";
        }
        return {};
    }
    ...
    process.setProcessEnvironment(buildPythonProcessEnvironmentForBridge());

    QStringList args;
    args << scriptPath
         << "--object" << trimmedObjectName
         << "--save"
         << "--output-dir" << QCoreApplication::applicationDirPath()
             << "--archives" << "gaia,mast,alma,chandra,jwst";

    if (!requiredParams.isEmpty()) {
        args << "--required-params" << requiredParams.join(',');
    }

    process.start(resolvePythonExecutableForBridge(), args);
    if (!process.waitForStarted(5000)) {
        if (errorMessage) {
            *errorMessage = "Failed to start Python interpreter for APIFetch.";
        }
        return {};
    }
    ...
    if (!process.waitForFinished(120000)) {
        ...
        if (errorMessage) {
            *errorMessage = "APIFetch.py timed out while gathering archive data.";
        }
        return {};
    }
    ...
    QString jsonText = stdoutText;
    const int jsonStart = jsonText.indexOf('{');
    if (jsonStart > 0) {
        jsonText = jsonText.mid(jsonStart);
    }
    ...
    if (parseError.error != QJsonParseError::NoError || !doc.isObject()) {
        if (errorMessage) {
            *errorMessage = QString("Invalid JSON from APIFetch.py: %1\n%2")
                .arg(parseError.errorString(), stderrText);
        }
        return {};
    }
    ...
    if (!payload.value("success").toBool()) {
        if (errorMessage) {
            *errorMessage = payload.value("error").toString(stderrText.isEmpty() ? "APIFetch.py reported failure." : stderrText);
        }
        return payload;
    }

    if (csvPath) {
        *csvPath = payload.value("csv_path").toString();
    }

    if (!stderrText.isEmpty()) {
        payload.insert("stderr", stderrText);
    }
...
public:
    ...
    /**
     * @brief Call QCalcGeom.py via uqff_server.js REST POST /qcalcgeom
     *
     * Session 230 (Gap #6): QCalcGeom v2.1.0 BSFG geometric pipeline wiring.
     * source2.cpp (Principal GUI) → uqff_server.js:3141 POST /qcalcgeom
     * → Python QCalcGeom.py calculator → JSON results back to GUI.
     *
     * Available calculators:
     *   "BSFGMetricCalculator"        - BSFG metric tensor (eps, R_scalar)
     *   "UniversalBuoyancyCalculator" - FUBi/FUBii simultaneous solver
     *   "HabitableZoneCalculator"     - r_hz iterative zone finder
     *   "UniversalGravityCalculator"  - F_U_total (26-layer DPM gravity)
     *   "MayanTimingCalculator"       - Baktun/generation timing windows
     *
     * VR/VM backend (source2(HEAD PROGRAM).cpp) receives QCalcGeom 3D results
     * via the same named pipe \\.\pipe\StarMagic_UQFF with type "QCALCGEOM_3D".
     *
     * @param calculator  One of the five QCalcGeom calculator names above
     * @param dataset     QJsonObject with M_kg, r_m (and optional parameters)
     * @return QJsonObject with result dict or error
     */
    QJsonObject sendQCalcGeomRequest(const QString& calculator, const QJsonObject& dataset) {
        QNetworkAccessManager mgr;
        QNetworkRequest req(QUrl("http://127.0.0.1:3141/qcalcgeom"));
...
public:
    VideoQueryHandler() : captureEnabled(false), activeDeviceId(0) {}
    ...
    bool captureFrame(const QString& outputPath) {
        ...
        cv::VideoCapture cap(activeDeviceId);
        if (!cap.isOpened()) {
            qWarning() << "VideoQueryHandler: Unable to open camera" << activeDeviceId;
            captureEnabled = false;
            return false;
        }
        ...
        cap.release();
        if (frame.empty()) {
            qWarning() << "VideoQueryHandler: Empty frame captured";
            return false;
        }

        if (!cv::imwrite(outputPath.toStdString(), frame)) {
            qWarning() << "VideoQueryHandler: Failed to write frame to" << outputPath;
            return false;
        }
    ...
    QString extractQueryContext() {
        ...
        QFileInfo frameInfo(lastFramePath);
        QString baseName = frameInfo.completeBaseName();
        QString objectHint;
        const QStringList tokens = baseName.split(QChar('_'), Qt::SkipEmptyParts);
        for (const QString& token : tokens) {
            if (token.startsWith("NGC", Qt::CaseInsensitive) ||
                token.startsWith("IC", Qt::CaseInsensitive) ||
                token.startsWith("M", Qt::CaseInsensitive) ||
                token.contains("Sgr", Qt::CaseInsensitive) ||
                token.contains("SGR", Qt::CaseInsensitive)) {
                objectHint = token;
                break;
            }
...
#ifdef NO_OPENCV
        return objectHint.isEmpty()
            ? QString("Captured frame ready for archive correlation: %1").arg(frameInfo.fileName())
            : QString("Captured frame suggests target %1; route to APIFetch/JWST-Chandra-ALMA validation.").arg(objectHint);
#else
        cv::Mat frame = cv::imread(lastFramePath.toStdString(), cv::IMREAD_COLOR);
        if (frame.empty()) {
            return QString("Captured frame stored at %1").arg(lastFramePath);
        }
        ...
        double edgeFraction = static_cast<double>(cv::countNonZero(edges)) /
                              static_cast<double>(std::max(1, edges.rows * edges.cols));
        QString brightnessLabel = brightness[0] > 170.0 ? "bright/stellar" : (brightness[0] > 90.0 ? "mixed diffuse" : "dark/high-contrast");
        QString structureLabel = edgeFraction > 0.08 ? "filamentary or high-structure field" : "smooth extended field";

        QStringList parts;
        if (!objectHint.isEmpty()) {
            parts << QString("target hint %1").arg(objectHint);
        }
        parts << brightnessLabel;
        parts << structureLabel;
        parts << QString("frame %1x%2").arg(frame.cols).arg(frame.rows);
        return QString("Video query context: %1").arg(parts.join(", "));
#endif
    }
    ...
    QString processGestureInput() {
        ...
        cv::inRange(hsvFrame, cv::Scalar(0, 20, 70), cv::Scalar(20, 255, 255), skinMask);
...
public:
    CoAnQiVisualCalculator(QWidget* parent = nullptr) 
        ...
        statusLabel = new QLabel("Status: Ready for archive-routed video/image preview", this);
        ...
        videoPreview = new QLabel("Load a video or image to extract a preview frame and route archive context.", this);
        videoPreview->setAlignment(Qt::AlignCenter);
        videoPreview->setMinimumSize(400, 300);
        videoPreview->setWordWrap(true);
        videoPreview->setStyleSheet("background-color: #1a1a2e; color: white; border: 2px solid #4a4a6a;");

        contextLabel = new QLabel("Archive context: none", this);
        contextLabel->setWordWrap(true);
        contextLabel->setStyleSheet("font-size: 10px; color: #D6E4FF; background-color: #0f172a; padding: 6px; border: 1px solid #334155;");
...
private slots:
    void loadVideo() {
        QString fileName = QFileDialog::getOpenFileName(this, "Load Video for Analysis",
            "", "Media Files (*.mp4 *.avi *.mkv *.mov *.png *.jpg *.jpeg *.bmp *.tif *.tiff);;All Files (*)");
        if (fileName.isEmpty()) {
            return;
        }

        loadedMediaPath = fileName;
        QString previewPath;
        const QString previewContext = preparePreviewFrame(fileName, &previewPath);

        if (!previewPath.isEmpty() && QFileInfo::exists(previewPath)) {
            QPixmap pixmap(previewPath);
            videoPreview->setPixmap(pixmap.scaled(videoPreview->size(), Qt::KeepAspectRatio, Qt::SmoothTransformation));
            videoPreview->setText(QString());
        } else {
            videoPreview->setPixmap(QPixmap());
            videoPreview->setText(previewContext);
        }

        archiveContextText = previewContext;
        QString objectHint = extractObjectHint(fileName);
        if (!objectHint.isEmpty()) {
            QString csvPath;
            QString errorMessage;
            const QJsonObject payload = runAPIFetchBridgeRequest(
                objectHint,
                {"mass", "distance", "redshift", "temperature", "luminosity", "density"},
                &csvPath,
                &errorMessage);

            if (payload.value("success").toBool()) {
                const QJsonObject result = payload.value("result").toObject();
                QStringList archiveSummary;
                const QJsonArray hits = result.value("archive_hits").toArray();
                for (const QJsonValue& value : hits) {
                    const QJsonObject hit = value.toObject();
                    archiveSummary << QString("%1(%2)")
                        .arg(hit.value("source").toString())
                        .arg(hit.value("match_count").toInt());
                }
                QStringList summaryParts;
                summaryParts << previewContext;
                summaryParts << QString("Object hint: %1").arg(objectHint);
                if (!archiveSummary.isEmpty()) {
                    summaryParts << QString("Archive hits: %1").arg(archiveSummary.join(", "));
                }
                if (!csvPath.isEmpty()) {
                    summaryParts << QString("CSV: %1").arg(QFileInfo(csvPath).fileName());
                }
                archiveContextText = summaryParts.join(" | ");
                statusLabel->setText(QString("Status: Routed %1 through APIFetch archives").arg(objectHint));
                statusLabel->setStyleSheet("color: #4ADE80;");
            } else if (!errorMessage.isEmpty()) {
                archiveContextText += " | Archive routing failed: " + errorMessage;
                statusLabel->setText("Status: Preview ready, archive routing failed");
                statusLabel->setStyleSheet("color: #F59E0B;");
            }
        } else {
            statusLabel->setText("Status: Preview ready, no object hint resolved from filename");
            statusLabel->setStyleSheet("color: #F59E0B;");
        }

        contextLabel->setText(archiveContextText);
        emit videoLoaded(fileName);
    }
    
    void addTrackPoint() {
        ++trackPointCount;
        statusLabel->setText(QString("Status: %1 track point(s) queued for export").arg(trackPointCount));
        statusLabel->setStyleSheet("color: #60A5FA;");
        contextLabel->setText(archiveContextText + QString(" | Track points: %1").arg(trackPointCount));
    }
    
    void toggleOverlay() {
        overlayEnabled = !overlayEnabled;
        statusLabel->setText(QString("Status: Overlay mode %1").arg(overlayEnabled ? "enabled" : "disabled"));
        statusLabel->setStyleSheet(overlayEnabled ? "color: #4ADE80;" : "color: #F59E0B;");
        contextLabel->setText(archiveContextText + QString(" | Overlay: %1").arg(overlayEnabled ? "on" : "off"));
    }
    
    void exportToPImath() {
        if (loadedMediaPath.isEmpty()) {
            QMessageBox::warning(this, "Export Unavailable", "Load a video or image before exporting a PImath package.");
            return;
        }

        QDir().mkpath(REPO_PATH + VIS_CALC_DIR);
        ...
        if (file.open(QIODevice::WriteOnly | QIODevice::Text)) {
            ...
            out << "media_path=" << loadedMediaPath << "\n";
            out << "preview_path=" << previewImagePath << "\n";
            out << "track_points=" << trackPointCount << "\n";
            out << "overlay_enabled=" << (overlayEnabled ? "true" : "false") << "\n";
            out << "archive_context=" << archiveContextText << "\n";
            file.close();
...
private:
    QString extractObjectHint(const QString& mediaPath) const {
        const QString baseName = QFileInfo(mediaPath).completeBaseName();
        ...
        for (int index = 0; index < tokens.size(); ++index) {
            ...
            if (token.compare("M", Qt::CaseInsensitive) == 0 && index + 1 < tokens.size()) {
                bool ok = false;
                tokens.at(index + 1).toInt(&ok);
                if (ok) {
                    return token + " " + tokens.at(index + 1);
                }
            }
            if (token.startsWith("M", Qt::CaseInsensitive) && token.size() > 1) {
                bool ok = false;
    ...
    QString preparePreviewFrame(const QString& mediaPath, QString* previewPath) {
        if (previewPath) {
            previewPath->clear();
        }
        ...
        const QString cachePath = REPO_PATH + VIS_CALC_DIR + info.completeBaseName() + "_preview.png";

        if (suffix == "png" || suffix == "jpg" || suffix == "jpeg" || suffix == "bmp" || suffix == "tif" || suffix == "tiff") {
            previewImagePath = mediaPath;
            if (previewPath) {
                *previewPath = mediaPath;
            }
            return QString("Image preview loaded from %1").arg(info.fileName());
        }

#ifdef NO_OPENCV
        previewImagePath.clear();
        return QString("Loaded %1, but OpenCV is unavailable for frame extraction.").arg(info.fileName());
#else
        cv::VideoCapture capture(mediaPath.toStdString());
        if (!capture.isOpened()) {
            previewImagePath.clear();
            return QString("Unable to open %1 for preview extraction.").arg(info.fileName());
        }
        ...
        if (frame.empty()) {
            previewImagePath.clear();
            return QString("No frame data found in %1.").arg(info.fileName());
        }

        cv::imwrite(cachePath.toStdString(), frame);
        previewImagePath = cachePath;
        if (previewPath) {
        ...
        cv::Mat edges;
        cv::Canny(gray, edges, 80.0, 180.0);
        const double edgeFraction = static_cast<double>(cv::countNonZero(edges)) /
            static_cast<double>(std::max(1, edges.rows * edges.cols));
        const QString brightnessLabel = brightness[0] > 170.0 ? "bright/stellar" : (brightness[0] > 90.0 ? "mixed diffuse" : "dark/high-contrast");
        const QString structureLabel = edgeFraction > 0.08 ? "filamentary" : "smooth";
        return QString("Preview extracted from %1 | %2 | %3 | %4x%5")
            .arg(info.fileName(), brightnessLabel, structureLabel)
            .arg(frame.cols)
            .arg(frame.rows);
#endif
    }
    ...
    QLabel* videoPreview;
    QLabel* contextLabel;
    QString loadedMediaPath;
    QString previewImagePath;
    QString archiveContextText;
...
public:
    explicit ScientificCalculatorWidget(QWidget* parent = nullptr)
        ...
        // BSFG Quick-Run button (Session 231) — calls QCalcGeom via uqff_server.js REST port 3141
        QPushButton* qcgBtn = new QPushButton("BSFG Geom", this);
        qcgBtn->setStyleSheet("background-color: #1565C0; color: white; font-weight: bold; padding: 8px;");
        qcgBtn->setToolTip("Run QCalcGeom BSFGMetricCalculator (Session 231)\nPOSTs to uqff_server.js on port 3141");
        connect(qcgBtn, &QPushButton::clicked, this, [this]() {
            QJsonObject dataset;
            dataset["M_kg"] = 1.989e30;
            dataset["r_m"]  = 6.96e8;
            dataset["SSq"]  = 0.57;
            dataset["t_n"]  = 0.0;
            QString expr = expressionInput->text().trimmed();
            if (!expr.isEmpty()) dataset["expr"] = expr;
            IPCClient client;
            QJsonObject result = client.sendQCalcGeomRequest("BSFGMetricCalculator", dataset);
            if (result.contains("error")) {
                outputDisplay->append("\n[QCalcGeom Error] " + result["error"].toString()
                    + "\n  Ensure uqff_server.js is running on port 3141");
            } else {
...
public:
    explicit QuantumDesignCalculatorWidget(QWidget* parent = nullptr)
        ...
        QHBoxLayout* archiveBridgeLayout = new QHBoxLayout();
        targetObjectEdit = new QLineEdit(this);
        targetObjectEdit->setPlaceholderText("Target object for APIFetch archive bridge");
        fetchDataButton = new QPushButton("Fetch Archive Data", this);
        fetchDataButton->setStyleSheet(
            "QPushButton { background-color: #3A6EA5; color: white; font-size: 10px; min-height: 28px; }"
            "QPushButton:hover { background-color: #4C82B8; }"
        );
        connect(fetchDataButton, &QPushButton::clicked, this, &QuantumDesignCalculatorWidget::fetchMissingInputsFromArchive);
        archiveBridgeLayout->addWidget(targetObjectEdit, 1);
        archiveBridgeLayout->addWidget(fetchDataButton);
        displayLayout->addLayout(archiveBridgeLayout);
...
private slots:
    void fetchMissingInputsFromArchive() {
        QString objectName = targetObjectEdit->text().trimmed();
        if (objectName.isEmpty()) {
            objectName = QInputDialog::getText(this, "APIFetch Archive Bridge", "Target object name:");
            if (objectName.trimmed().isEmpty()) {
                suggestionsLabel->setText("Archive bridge cancelled: no target object provided.");
                suggestionsLabel->setStyleSheet("color: #FFAA00; font-size: 10px;");
                return;
            }
            targetObjectEdit->setText(objectName.trimmed());
        }

        suggestionsLabel->setText(QString("Running APIFetch archive bridge for %1...").arg(objectName));
        suggestionsLabel->setStyleSheet("color: #88AAFF; font-size: 10px;");
        QApplication::processEvents();

        QString csvPath;
        QString errorText;
        const QJsonObject payload = runAPIFetchBridgeRequest(objectName, buildRequiredParameterList(), &csvPath, &errorText);
        if (payload.isEmpty() || !payload.value("success").toBool()) {
            suggestionsLabel->setText("Archive bridge failed: " + errorText.left(220));
            suggestionsLabel->setStyleSheet("color: #FF6666; font-size: 10px;");
            if (!errorText.isEmpty()) {
                QMessageBox::warning(this, "APIFetch Archive Bridge", errorText);
            }
            return;
        }

        const QJsonObject result = payload.value("result").toObject();
        applyFetchedParameters(result);

        QStringList archiveSummary;
        const QJsonArray archiveHits = result.value("archive_hits").toArray();
        for (const QJsonValue& value : archiveHits) {
            const QJsonObject hit = value.toObject();
            archiveSummary << QString("%1(%2)")
                .arg(hit.value("source").toString())
                .arg(hit.value("match_count").toInt());
        ...
        const QStringList stillMissing = collectMissingInputSuggestions();
        QString message = QString("APIFetch updated %1").arg(objectName);
        if (!archiveSummary.isEmpty()) {
            message += " | Archives: " + archiveSummary.join(", ");
        }
        if (!csvPath.isEmpty()) {
            message += " | CSV: " + QFileInfo(csvPath).fileName();
        }
        if (!stillMissing.isEmpty()) {
            message += " | Remaining: " + stillMissing.join(", ").left(140);
            suggestionsLabel->setStyleSheet("color: #FFAA00; font-size: 10px;");
        } else {
            suggestionsLabel->setStyleSheet("color: #00FF00; font-size: 10px;");
        }
        suggestionsLabel->setText(message.left(260));
    }
    ...
    void checkMissingInputs() {
        const QStringList missing = collectMissingInputSuggestions();
...
private:
    QStringList collectMissingInputSuggestions() const {
        QStringList missing;
        for (auto it = inputFields.begin(); it != inputFields.end(); ++it) {
            if (it.value()->value() == 0.0) {
                missing << buildDeepSearchSuggestion(it.key());
            }
        }
        return missing;
    }

    QStringList buildRequiredParameterList() const {
        QStringList required = {
            "mass", "distance", "radius", "redshift", "magnetic_field",
            "temperature", "luminosity", "density", "radial_velocity"
        };

        if (inputFields.value("M_DM") && inputFields.value("M_DM")->value() == 0.0) {
            required << "mass";
        }
        if (inputFields.value("ρ_fluid") && inputFields.value("ρ_fluid")->value() == 0.0) {
            required << "density";
        }
        if (inputFields.value("F_BH") && inputFields.value("F_BH")->value() == 0.0) {
            required << "xray_flux";
        }
        if (inputFields.value("F_rad") && inputFields.value("F_rad")->value() == 0.0) {
            required << "luminosity";
        }
        return required;
    }

    void applyFetchedParameters(const QJsonObject& result) {
        const auto findNumericValue = [&result](const QStringList& resultKeys, double* outValue) {
            for (const QString& resultKey : resultKeys) {
                const QJsonValue value = result.value(resultKey);
                if (value.isDouble()) {
                ...
                if (value.isString()) {
                    bool ok = false;
                    const double numeric = value.toString().toDouble(&ok);
                    if (ok) {
                        *outValue = numeric;
                        return true;
                    }
        ...
        const auto setField = [this, &findNumericValue](const QString& fieldName, const QStringList& resultKeys) {
            auto it = inputFields.find(fieldName);
            if (it == inputFields.end()) {
                return;
            }
            double numericValue = 0.0;
            if (findNumericValue(resultKeys, &numericValue)) {
                it.value()->setValue(numericValue);
            }
        };

        const auto setProxyField = [this, &findNumericValue](const QString& fieldName, const QStringList& resultKeys, double scale) {
            auto it = inputFields.find(fieldName);
            if (it == inputFields.end()) {
                return;
            }
            double numericValue = 0.0;
            if (findNumericValue(resultKeys, &numericValue) && numericValue > 0.0 && scale > 0.0) {
                const double proxy = std::max(0.0, std::min(10.0, numericValue / scale));
                it.value()->setValue(proxy);
            }
        };
        ...
        setField("M_vis", {"mass"});
        setField("M_ext", {"mass"});
        setField("r_0", {"radius", "distance"});
        setField("r_ext", {"distance", "radius"});
        setField("z", {"redshift"});
        setField("B_t", {"magnetic_field"});
        setField("v_r", {"radial_velocity", "velocity_dispersion"});
        setField("ρ_fluid", {"density", "gas_density", "SCm_density"});
        setField("ρ", {"density", "gas_density", "SCm_density"});
        setProxyField("F_rad", {"luminosity"}, 3.828e26);
        setProxyField("F_BH", {"xray_flux"}, 1e-12);
    }

    QString buildDeepSearchSuggestion(const QString& parameter) const {
        if (parameter == "M_0" || parameter == "M_ext" || parameter == "M_vis" || parameter == "M_DM") {
            return parameter + ": use Fetch Archive Data for Gaia, JWST, or exoplanet host-star mass proxies";
        }
        if (parameter == "z" || parameter == "r_0" || parameter == "r_ext") {
            return parameter + ": use Fetch Archive Data for Gaia/MAST/JWST distance-redshift catalogs";
        }
        if (parameter == "ρ_fluid" || parameter == "δ_ρ" || parameter == "ρ") {
            return parameter + ": use Fetch Archive Data for ALMA, 2MASS, or WISE gas-density and IR products";
        }
        if (parameter == "F_wind" || parameter == "F_rad" || parameter == "F_SN" || parameter == "F_BH") {
            return parameter + ": use Fetch Archive Data for MAST, HEASARC, JWST, and Chandra environment terms";
        }
        if (parameter == "B_t") {
            return parameter + ": use Fetch Archive Data for Gaia, Chandra, and radio magnetic diagnostics";
        }
        return parameter + ": use Fetch Archive Data via APIFetch (Gaia/MAST/JWST/ALMA/Chandra/ESO)";
    }
...
// ============================================================
// QCalcGeomWidget — Tab 23: BSFG Geometric Physics Panel
// Session 231 — calls all 5 QCalcGeom calculators via REST port 3141
// ============================================================
class QCalcGeomWidget : public QWidget {
    Q_OBJECT
public:
    QCalcGeomWidget(QWidget* parent = nullptr) : QWidget(parent) { setupUI(); }
private:
    QComboBox* calcCombo      = nullptr;
    QLineEdit* massInput      = nullptr;
    QLineEdit* radiusInput    = nullptr;
    QLineEdit* ssqInput       = nullptr;
    QLineEdit* tnInput        = nullptr;
    QTextEdit* resultsDisplay = nullptr;
    void setupUI() {
        QVBoxLayout* mainLayout = new QVBoxLayout(this);
        QLabel* title = new QLabel(
            "QCalcGeom BSFG  |  v1.4.0  |  5 Calculators  |  REST port 3141", this);
        title->setStyleSheet(
            "font-size: 12pt; font-weight: bold; color: #1565C0; padding: 6px;");
        mainLayout->addWidget(title);
        QGroupBox* selGroup = new QGroupBox("Calculator", this);
        QVBoxLayout* selLayout = new QVBoxLayout(selGroup);
        calcCombo = new QComboBox(this);
        calcCombo->addItem("BSFGMetricCalculator");
        calcCombo->addItem("UniversalBuoyancyCalculator");
        calcCombo->addItem("HabitableZoneCalculator");
        calcCombo->addItem("UniversalGravityCalculator");
        calcCombo->addItem("MayanTimingCalculator");
        calcCombo->setToolTip("QCalcGeom BSFG calculators (QCalcGeom.py v2.1.0 / Session 203)");
        selLayout->addWidget(calcCombo);
        ...
        massInput   = new QLineEdit("1.989e30", this);
        massInput->setToolTip("Body mass [kg]  (Solar mass = 1.989e30)");
        radiusInput = new QLineEdit("6.96e8", this);
        radiusInput->setToolTip("Body radius / distance [m]  (Solar radius = 6.96e8)");
        ssqInput    = new QLineEdit("0.57", this);
        ssqInput->setToolTip("[SSq] UQFF calibration constant (canonical = 0.57)");
        tnInput     = new QLineEdit("0.0", this);
        tnInput->setToolTip("Phase parameter t_n (dimensionless, default = 0.0)");
        paramForm->addRow("M_kg :", massInput);
        paramForm->addRow("r_m  :", radiusInput);
        paramForm->addRow("[SSq]:", ssqInput);
        paramForm->addRow("t_n  :", tnInput);
        mainLayout->addWidget(paramGroup);
        QHBoxLayout* btnRow = new QHBoxLayout();
        QPushButton* runBtn   = new QPushButton("Run QCalcGeom", this);
        QPushButton* clearBtn = new QPushButton("Clear", this);
        runBtn->setStyleSheet(
            "background-color: #1565C0; color: white; font-weight: bold; padding: 8px 20px;");
        clearBtn->setStyleSheet(
            "background-color: #616161; color: white; padding: 8px 16px;");
        btnRow->addWidget(runBtn);
        btnRow->addWidget(clearBtn);
        btnRow->addStretch();
        mainLayout->addLayout(btnRow);
        QGroupBox* resGroup = new QGroupBox("Results", this);
        ...
        resLayout->addWidget(resultsDisplay);
        mainLayout->addWidget(resGroup);
        connect(runBtn, &QPushButton::clicked, this, [this, runBtn]() {
            QString calc = calcCombo->currentText();
            QJsonObject dataset;
            dataset["M_kg"] = massInput->text().toDouble();
            dataset["r_m"]  = radiusInput->text().toDouble();
            dataset["SSq"]  = ssqInput->text().toDouble();
            dataset["t_n"]  = tnInput->text().toDouble();
            runBtn->setEnabled(false);
            runBtn->setText("Running...");
            QApplication::processEvents();
            IPCClient client;
            QJsonObject result = client.sendQCalcGeomRequest(calc, dataset);
            runBtn->setEnabled(true);
            runBtn->setText("Run QCalcGeom");
            QJsonDocument doc(result);
            QString ts = QDateTime::currentDateTime().toString("hh:mm:ss");
            if (result.contains("error")) {
                resultsDisplay->append(
                    QString("\n[%1] ERROR: %2\n  Ensure uqff_server.js is running on port 3141")
                    .arg(ts, result["error"].toString())
                );
            } else {
                resultsDisplay->append(
                    QString("\n[%1]  %2\n%3")
                    .arg(ts, calc, QString::fromUtf8(doc.toJson(QJsonDocument::Indented)))
                );
            }
        });
        connect(clearBtn, &QPushButton::clicked, resultsDisplay, &QTextEdit::clear);
    }
};
    ...
    QVBoxLayout *layout = new QVBoxLayout(centralWidget); // Vertical layout (top to bottom)
        ...
        for (int i = 0; i < MAX_WINDOWS; ++i)
        {
            ...
            // Special case: Tab 22 (index 21) reserved for Advanced Calculator (Grok Thread Extraction)
                ...
                CalculatorAdvanced::CalculatorWidget* advCalc = new CalculatorAdvanced::CalculatorWidget(this);
        ...
        // Tab 23 (index 22): QCalcGeom BSFG Geometric Physics Panel (Session 231)
        QCalcGeomWidget* qcgTab = new QCalcGeomWidget(this);
        tabs->addTab(qcgTab, "QCalcGeom BSFG");
        ...
        // Context menu: right-click UQFF results sidebar to re-run with QCalcGeom BSFG (Session 231)
        uqffResultsWidget->setContextMenuPolicy(Qt::CustomContextMenu);
        connect(uqffResultsWidget, &QWidget::customContextMenuRequested,
                this, [this](const QPoint& pos) {
            QMenu ctxMenu(uqffResultsWidget);
            QAction* actBsfg = ctxMenu.addAction("Run QCalcGeom BSFG...");
            actBsfg->setToolTip("Re-run through BSFGMetricCalculator via uqff_server.js port 3141");
            QAction* chosen = ctxMenu.exec(uqffResultsWidget->mapToGlobal(pos));
            if (chosen == actBsfg) {
                QJsonObject dataset;
                dataset["M_kg"] = 1.989e30;
                dataset["r_m"]  = 6.96e8;
                ...
                if (!loadedBodies.empty()) {
                    dataset["M_kg"] = loadedBodies[0].mass;
                    dataset["r_m"]  = loadedBodies[0].radius;
                }
                IPCClient client;
                QJsonObject result = client.sendQCalcGeomRequest("BSFGMetricCalculator", dataset);
                QJsonDocument doc(result);
                QMessageBox::information(
                    this, "QCalcGeom BSFG Result",
                    QString::fromUtf8(doc.toJson(QJsonDocument::Indented)).left(4096)
                );
            }
        });
...
bool MainWindow::ensureBodiesDataAvailable(const QString& systemName, const QStringList& requiredParams) {
    QString csvPath;
    QString errorText;
    const QJsonObject payload = runAPIFetchBridgeRequest(systemName, requiredParams, &csvPath, &errorText);
    if (payload.isEmpty() || !payload.value("success").toBool()) {
        QMessageBox::warning(this, "APIFetch Archive Bridge",
            QString("Failed to gather data for %1.\n\n%2")
                .arg(systemName, errorText));
        return false;
    }
    ...
    QMessageBox::warning(this, "APIFetch Archive Bridge",
        QString("APIFetch completed for %1, but no bodies_*.csv file was produced.")
            .arg(systemName));
    return false;
}
...
void MainWindow::computeUQFF(const QString& systemName) {
    ...
    try {
        ...
        if (bodies.empty()) {
            if (!ensureBodiesDataAvailable(systemName, {"mass", "distance", "temperature", "redshift", "magnetic_field"})) {
                return;
            }
            bodies = UQFF::CSVBodyReader::read_latest(".");
        }
    ...
    auto locateTargetBody = [&](const std::vector<UQFF::CelestialBodyCSV>& candidateBodies) {
        const UQFF::CelestialBodyCSV* match = nullptr;
    ...
    targetBody = locateTargetBody(bodies);
